...
void LoadSceneSound()
{
	...
	if (i != -1)
	// Cheatsurfer -->
	// Музыка и звуковая схема в джунглях не сбрасываются. Вообще везде, где юзается одинаковая музыка, тоже не мешало бы прописать такое. Потом...
	{
		if (CheckAttribute(pchar, "CSM.JungleContinuousMusic"))
		{
			if (CheckAttribute(loadedLocation, "type") && loadedLocation.type == "jungle")
			{
				// Музыкальные схемы есть только у обладателей лейблов "Village" и "mines", но и "Graveyard" зацепим тоже, на всякий.
				if (CheckAttribute(loadedLocation, "id.label") && loadedLocation.id.label != "Graveyard" && loadedLocation.id.label != "mines" && loadedLocation.id.label != "Village")
				{
					...
					{
						// ngMusicID > 0 = флаг нахождения ГГ глубоко в джунглях, задается в seadogs.c, #f OnLoad()
