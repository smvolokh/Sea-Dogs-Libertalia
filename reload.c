...
{
	//ResetSoundScheme();
	// CheatSurfMenu by Cheatsurfer --> ћузыка и звукова€ схема в джунгл€х не сбрасываютс€.
	bool bResetMusic = true;
	if (CheckAttribute(pchar, "CSM.JungleContinuousMusic"))
	{
		if (IsEntity(loadedLocation))
		{
			...
			{
				// ћузыкальные схемы есть только у обладателей лейблов "Village" и "mines", но и "Graveyard" зацепим тоже, на вс€кий.
				if (CheckAttribute(loadedLocation, "id.label") && loadedLocation.id.label != "Graveyard" && loadedLocation.id.label != "mines" && loadedLocation.id.label != "Village")
				{
					...
					{
						if (locations[reload_location_index].id.label == "Village" || locations[reload_location_index].id.label == "Graveyard" || locations[reload_location_index].id.label == "mines")
						{
							ResetSound();
							PauseAllSounds();
						}
	...
	if(reload_cur_island_index < 0)
	{
		...
		Partition_SetValue("after");// ƒележ добычи и уход на сушу
...
//    DetectBlades();
	...
	if(reload_cur_island_index < 0)
	{
		...
		if(reload_island_index < 0)
		{
			...
			// Dolphin -> мод "ƒезертиры"
			ref loc = &locations[reload_location_index];
			if(CheckAttribute(loc, "type") && loc.type == "town" && GetQuestPastDayParam("MainChrDesertirs_timer") > 1)
			{
				SaveCurrentQuestDateParam("MainChrDesertirs_timer");
				int morale = GetCharacterCrewMorale(pchar);
				float percent;
				int count;
				bool bSound = false;
				if(morale < 50)
				{
					percent = Bring2Range(1.0,5.0,0.0,100.0, 100.0 - makefloat(morale)); // 1-5% команды
					percent *= 0.01;

					count = makeint(GetCrewQuantity(pchar) * percent);
					
					if(count > 0)
					{
						RemoveCharacterCrew(pchar, count);
						if(count == 1) Log_info(" апитан, "+FindRussianSailorString(count, "No")+" дезертировал с нашего корабл€!");
						else Log_info(" апитан, "+FindRussianSailorString(count, "No")+" дезертировали с нашего корабл€!");
						bSound = true;
					}
				}

				// теперь дл€ личных компаньонов
				int cn;
				ref chref;
				for(int i=1; i<COMPANION_MAX; i++)
				{
					...
					if(cn!=-1 && GetRemovable(&Characters[cn]))
					{
						...
						{
							morale = GetCharacterCrewMorale(chref);
							if(morale < 50)
							{
								percent = Bring2Range(1.0,5.0,0.0,100.0, 100.0 - makefloat(morale)); // 1-5% команды
								percent *= 0.01;

								count = makeint(GetCrewQuantity(chref) * percent);
								
								if(count > 0)
								{
									RemoveCharacterCrew(chref, count);
									if(count == 1) Log_info(" апитан, "+FindRussianSailorString(count, "No")+" дезертировал с корабл€ '"+chref.ship.name+"'!");
									else Log_info(" апитан, "+FindRussianSailorString(count, "No")+" дезертировали с корабл€ '"+chref.ship.name+"'!");
				...
				if(bSound) PlaySound("interface\shipsbell.wav");
			}
			//<- мод "ƒезертиры"
		}
		else
...
int ReloadToLocation(int location_index, aref reload_data)
{
	...
	if (CheckAttribute(&Locations[location_index], "MustSetReloadBack"))
	{
		...
		// reload_data.FromSea = true; - есть только на острове, ведет к пл€жу заглушке
		...
		{   // переход суша-суша
			for (i=0; i<iNumReload; i++)
			{
				...
				if (arLocator.Name == reload_data.emerge)
				{
	                ...
	                {
	                	arLocator.label  = Locations[reload_cur_location_index].id.label; // как завать, куда вернутьс€
	                }
	                else
	                {
	                    arLocator.label  = "";
	...
	//сменить анимацию перед загрузкой локации, нужно дл€ подводных храмов.
	if (CheckAttribute(&locations[location_index], "changeAnimation")) 
	{
		...
		{
			// CheatSurfMenu by Cheatsurfer --> девчата научились плавать (+ Ѕелтроп с Ѕлейзом за компанию)
			if (locations[location_index].changeAnimation == "swim")
			{
				if (HasSubStr(mc.model.animation, "BalthropPlayer"))
				{
					mc.model.animation = "_CheatSurfMenu_\swimBalthrop";
				}
				else
				{
					if (HasSubStr(mc.model.animation, "MaryPlayer"))
					{
						mc.model.animation = "_CheatSurfMenu_\swimBeatrice";
					}
					else
					{
						if (HasSubStr(mc.model.animation, "DannyPlayer") || HasSubStr(mc.model.animation, "Danielle") || HasSubStr(mc.model.animation, "Danny"))
						{
							mc.model.animation = "_CheatSurfMenu_\swimDanielle";
						}
						else
						{
							if (HasSubStr(mc.model.animation, "JessikaPlayer") || HasSubStr(mc.model.animation, "Jess"))
							{
								mc.model.animation = "_CheatSurfMenu_\swimJessika";
							}
							else
							{
								if (HasSubStr(mc.model.animation, "BlazeKCM") || HasSubStr(mc.model.animation, "man_kvl2Player"))
								{
									mc.model.animation = "_CheatSurfMenu_\swimK3man";
								}
								else
								{
									if (HasSubStr(mc.model.animation, "Ciri") || HasSubStr(mc.model.animation, "Chani") || HasSubStr(mc.model.animation, "Witch"))
									{
										mc.model.animation = "_CheatSurfMenu_\swimK2woman";
									}
									else
									{
										if (HasSubStr(mc.model.animation, "man_K2"))
										{
											mc.model.animation = "_CheatSurfMenu_\swimK2man";
										}
										else
										{
		...
		DoQuestFunctionDelay("CSMAnimationFix", 2.0); // CheatSurfMenu --> Ќапоминалка игроку, что, в случае чего, слетевшую анимацию можно исправить
	}
	
	// Warship. ¬ тавернах нельз€ использовать анимацию мушкетера, поэтому сбрасываем на стандартную
	...
	//мен€ем радиусы детекторов энкаунтеров
