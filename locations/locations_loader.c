...
bool LoadLocation(ref loc)
{
	...
	if(CheckAttribute(loc, "type"))
	{
		...
		PlaySound("Interface\DOOR_CLOSE01B.wav");
			res = false;
			break;
		case "tavern":			// таверна
		...
		case "dungeon":
			...
			break;
		case "Cannon":			// гандек
			ExecuteTechnique("amb_Cannon");
                PlaySound("Interface\DOOR_CLOSE01B.wav");
			res = false;
			break;
		case "Hold":			// трюм
			ExecuteTechnique("amb_Hold");
           PlaySound("Interface\DOOR_CLOSE01B.wav");
			res = false;
			break;
		}
	...
	if (CheckAttribute(Location, "locators.reload.boat"))
	{
		if (bMainCharacterHere && iMainCharacterShipType != SHIP_NOTUSED)
		{
			if (CreateEntity(&locShips[n], "ship"))
			{
				...
				if (!CheckAttribute(rPlayer, "Flags.Pirate")) rCharacter.Flags.Pirate = rand(3);
