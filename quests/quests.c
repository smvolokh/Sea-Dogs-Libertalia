...
#include "quests\start_line.c"
#include "quests\Gothic_F.c"
#include "quests\Gothic_R.c"
#include "quests\Gothic_D.c"
...
void AddQuestRecordEx(string idQuest,string idReferenceQuest,string idText)
{
	...
	// покраска новой СЖ
	SetQuestHeaderColor(idQuest, argb(255,255,255,0));
}
...
void AddQuestRecordInfo(string idQuest, string idText)
{
	...
	// покраска новой СЖ
        SetQuestHeaderColor(idQuest, argb(255, 255, 165, 0));
...
void QuestToSeaLogin_Launch()
{
	...
	SendMessage(&fader, "ls",FADER_PICTURE,"loading\sea_0" + rand(6) + ".tga");
...
void CompleteQuestName(string sQuestName, string qname)
{
	...
	else
	{
		...
		StartlineQuestComplete(sQuestName, qname);
		GothicFQuestComplete(sQuestName, qname);
		GothicRQuestComplete(sQuestName, qname);
		GothicDQuestComplete(sQuestName, qname);
	}
}
