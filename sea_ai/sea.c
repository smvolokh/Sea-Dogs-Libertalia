...
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_23.wav");
	...
	if(iColony != -1 && sti(Colonies[iColony].nation) != PIRATE && colonies[iColony].nation != "none" && colonies[iColony].id != "FortOrange" && colonies[iColony].id != "LostShipsCity") // Не берем сбор на необитайках, в пиратских колониях, в ГПК и в Форт Оранже
	{
		if(pchar.questTemp.sbormoney != Sea_FindNearColony())
		{
			...
			Log_Info("Оплачен портовый сбор в размере "+ sbormoney +" песо");
...
void Sea_MapLoad()
{
    ...
    if (!ok && !CheckAttribute(pchar, "CSM.FreeSailing")) // <-- CheatSurfMenu by Cheatsurfer
    {
    ...
    if (!CheckEnemyCompanionDistance2GoAway(true) && !CheckAttribute(pchar, "CSM.FreeSailing")) return; // <-- CheatSurfMenu by Cheatsurfer // && !bBettaTestMode  табличка выхода из боя
	...
	SendMessage(&SeaFader, "ls", FADER_PICTURE, "loading\sea_0" + rand(6) + ".tga");
...
void SeaLogin(ref Login)
{
	...
	// Sea Fader start
	if (!CheckAttribute(&Login,"ImageName")) { Login.ImageName = "loading\sea_0" + rand(6) + ".tga"; }
...
//            		Group_SetXZ_AY(sGName, x, z, ay);
		...
		if (iNumFantomShips) 
		{
			for (j=0; j<iNumFantomShips; j++)
			{
				...
				if(iEncounterType == ENCOUNTER_TYPE_WAR_KAPER) {rFantom.DontRansackCaptain = true;} // не сдаемся
