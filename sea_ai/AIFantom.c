...
void Fantom_SetGoods(ref rFantom, string sFantomType)
{
	...
	switch (sFantomType)
	{
		case "war":
			iStart 	= GOOD_BALLS;
			if( CheckAttribute(rFantom, "RealEncounterType") && sti(rFantom.RealEncounterType) == ENCOUNTER_TYPE_WAR_KAPER)
			{
...
void SetShipToFantom(ref _chr, string _type, bool _setgoods)
{
	...
	switch (_type)
	{
		case "trade":		
			...
			if (Rank > 20)
			{
				if (Nation == SPAIN) ShipType = RandFromThreeDight(SHIP_CARACCA, SHIP_NAVIO, SHIP_HEAVYGALEON);
		...
		case "pirate":
			...
			if (Rank > 20)
			{
				ShipType = RandFromThreeDight(SHIP_CORVETTE, SHIP_HEAVYGALEON, SHIP_LINESHIP);
			}
		break;
		...
		case "war":
			...
			if (Rank > 20)
			{
				switch (Nation)
				{
					...
					case SPAIN	  : ShipType = RandFromFiveDight(SHIP_POLACRE, SHIP_HEAVYGALEON, SHIP_FRIGATE_H, SHIP_HEAVYGALEON, SHIP_LINESHIP); break; 					
