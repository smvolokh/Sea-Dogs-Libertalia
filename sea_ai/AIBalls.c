...
void Ball_AddBall(aref aCharacter, float fX, float fY, float fZ, float fSpeedV0, float fDirAng, float fHeightAng, float fCannonDirAng, float fMaxFireDistance)
{
	...
	{
		if (fCannons > rand(100) && !IsCharacterEquippedArtefact(aCharacter, "talisman3") && !CheckAttribute(aCharacter, "CSM.NoCannonsBoom"))
		{
	...
	// Sticksъ -> рандумные звуки выстрелов
	if (CheckAttribute(pchar, "CSM.ArtillerySoundMod")) {
		Play3DSound(CSM_GetRandomCannonSoundByCaliber(rCannon.caliber), fX, fY, fZ);
		return;
	}
	// <- Sticksъ
