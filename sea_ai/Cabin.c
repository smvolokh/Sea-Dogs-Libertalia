...
void Return2SeaAfterCabin()
{
	...
	SendMessage(&boarding_fader, "ls", FADER_PICTURE, "loading\sea_0" + rand(6) + ".tga");
...
{
	string boat_pic = "loading\boat_"+rand(4)+".tga";	
	if(CheckAttribute(pchar, "TownEscape")) boat_pic = "loading\Start_Loading_" + rand(1) + ".tga";
...
void DeckBoat_Start()
{
 //ResetSoundScheme();
	...
	{
		PlaySound("interface\_NaVodu"+rand(1)+".wav");// шлюпки на воду!
	}	
	ref mchr = GetMainCharacter();
