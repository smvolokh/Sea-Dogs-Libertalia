...
void ProcessDialogEvent()
{
  ...
  //--> Jason, Ложный след----------------------Диалог первый - первая встреча
		case "First time":
PlaySound("VOICE\Russian\GOTHIC\GG_HALLO_M\GG_HALLO_M_" + rand(15) + ".wav");
		...
		case "FalseTrace_Prisoner_7":
			...
			link.l1 = "Что-то не внушает доверия мне ваш план... Сдается мне, что это ловушка, и не успею я отойти от " + XI_ConvertString(pchar.questTemp.FalseTrace.TargetShore+"Gen") + " на полстони миль, как за мной будет отряжена погоня. Нет, капитан, я не намерен рисковать своей головой. На этом разговор окончен.";
