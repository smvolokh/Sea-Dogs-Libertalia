void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Saxon":
			...
			if(sti(pchar.reputation.nobility) > 71)//при низкой репутации - в сад
			{
		...
		case "Saxon_check":
			...
			if(sti(pchar.reputation.nobility) < 71)//повторная проверка репутации, для второго и далее заходов
			{
		...
		//--> плохиш-пират
		case "Lostpirate":		
			if(sti(pchar.reputation.nobility) < 36)//при повышенной репутации - в сад
			{
				...
				link.l1 = "Что ты делаешь один в этом уединённом месте? Изучаешь повадки морских птиц?";
		...
		case "Lostpirate_check":
            if (FindFreeRandomOfficer() == -1)//если перебор с офицерами
            {
                dialog.text = "Хм. Я хотел бы пойти к вам на службу... Но вижу, у вас полный комплект офицеров, увы. А тесниться в кубрике с матросами я не намерен. Так что придется мне на этом уютном острове.";
    			link.l1 = "Да, вакансий у меня нет. Возможно, навещу тебя позже.";
    			link.l1.go = "exit";
				NextDiag.TempNode = "OnceAgainLP";
    			break;
            }
			if(sti(pchar.reputation.nobility) > 36)//повторная проверка репутации, для второго и далее заходов
			{
				dialog.text = "Хм... Знаете, кэп, что-то вы мне не нравитесь. Еще сдадите властям, чего доброго... от святош можно чего угодно ждать. Лучше я останусь здесь, на этом уютном острове.";
			...
			link.l1 = "Почему бы и нет? Надеюсь, в моей команде ты ни с кем не станешь спорить. А бакланов оставь себе или проставься перед ребятами.";
		...
		case "Exit_hire_Lostpirate":
			...
			NextDiag.CurrentNode = NextDiag.TempNode;
			DialogExit();
			locations[FindLocation("Shore126")].DisableEncounters = false; //энкаутеры в бухте откроем
		break;
		//<-- найм
