void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "ShoreGuard":
			...
			dialog.text = "Эй, ты! Здесь чужаки не нужны. Убирайся в море пират, подобру-поздорову!";
		...
		case "ShoreGuard_1":
			...
			dialog.text = "Ах, ты грязный пират. Как бы ты "+ GetSexPhrase("сам не докукарекался, петушок","сама не докудахталась, курочка") +"! Не повернешь назад добром - заставим!";
			link.l1 = "А кто может мне запретить ходить там, где я хочу?";
			link.l1.go = "ShoreGuard_2";
		break;
		case "ShoreGuard_2":
 	                PlaySound("VOICE\Russian\GOTHIC\SVM_5.wav");
			dialog.text = "Я порву тебя на части пират. Три дюйма стали меж ребер!";
		...
		case "DMCitizHead":
			...
			{
				dialog.text = "Здравствуйте, капитан! Что скажете?";
				link.l1 = "Я отравил"+ GetSexPhrase("ся","ась") +" на поиски корвета 'Черная Жемчужина'. Похоже, что это последний шаг к вашему освобождению.";
				link.l1.go = "exit";
			}
			if (pchar.questTemp.MC == "over")
			{
				dialog.text = "Это вы сняли проклятие? В какой-то момент у нас все нормализовалось!";
				link.l1 = "Да. Я наш"+ GetSexPhrase("ел","ла") +" корвет 'Черная Жемчужина' и все сделал"+ GetSexPhrase("","а") +". Рад"+ GetSexPhrase("","а") +", что сумел"+ GetSexPhrase("","а") +" вам помочь.";
		...
		break;
		case "DMHeadOver_1":
			dialog.text = "Двадцать тысяч песо! Может быть для вас это немного, но наш город за проклятое время здорово обеднел, так что это все, что у нас есть.";
		...
		break;
		case "DMSkelHead_2":
			dialog.text = "Как-то причалил к нашему мысу пиратский корвет 'Черная Жемчужина'. Пираты здесь рабов контрабандистам продали. Договариваются между собой...\n"+
				"Продали пираты рабов, а потом у них между собой ссора какая-то вышла. Часть команды здесь осталась, а остальные с капитаном на корвете в море ушли. С той поры все и началось.";
		...
		case "DMSkelHead_8":
			...
			link.l1 = "Плохо... Может попытаться разыскать этот корвет, на котором ушли пираты? Похоже, что это единственная ниточка.";
		...
		break;
		case "DMSkelHead_12":
			dialog.text = "Да, однако обитает она отчего на французском Сент-Кристофере. Я признаю, что дал маху, отпустив ее на все четыре стороны. Надо было допытаться до всего, что она знает.\nСейчас я прошу вас найти ее и выяснить все, что ей известно об этих проклятых пиратах и корвете 'Черная Жемчужина'.";
		...
		case "LizBegin_3":
			...
			link.l1 = "Помогите мне разыскать корвет 'Черная Жемчужина.";
			link.l1.go = "LizBegin_4";
		break;
		case "LizBegin_4":
			dialog.text = "Зачем ему нужен этот корвет?";
			link.l1 = "Дело в том, юная леди, что жители города связывают с этим корветом... возникновение внезапной эпидемии, поразившей город.";
		...
		break;
		case "LizBegin_6":
			dialog.text = "Конечно знаю! Дело в том, что капитан корвета 'Черная Жемчужина' - очень дорогой мне человек. Последнее время я только тем и занимаюсь, что пытаюсь распутать этот клубок и развеять дьявольские чары.";
			link.l1 = "Получается?";
			link.l1.go = "LizBegin_7";
		break;
		case "LizBegin_7":
			dialog.text = "Отчасти. Дело в том, что я дочь известного среди пиратов человека. Мой отец, капитан Барбосса, был одним из ближайших соратников Николаса Шарпа и одним из пиратских баронов!";
			link.l1 = "О-о, это очень достойно, мисс...";
			link.l1.go = "LizBegin_8";
		break;
		case "LizBegin_8":
			dialog.text = "Так вот, не имея другого выхода, я обратилась за помощью к Торусу. Надеюсь, вы знаете, кто это?";
			link.l1 = "Еще бы!";
		...
		break;
		case "LizBegin_14":
			dialog.text = "Тем не менее, я оттолкнула близкого мне человека. Тогда я твердо решила, что узнаю о несчастье, постигшем капитана 'Черной Жемчужины' и освобожу его!";
		...
		case "LizBegin_SeekSkull":
			...
			link.l1 = "Понятно. Если мне удасться раздобыть этот череп, то я первым делом отправлюсь сюда, в Капстервиль.";
		...
		case "LizCheckSkull_1":
			dialog.text = "Теперь у нас есть инструмент для снятия проклятия. Вам нужно отыскать корвет 'Черная Жемчужина' и, собственно, снять проклятие с капитана.";
		...
		case "LizCheckSkull_2":
			dialog.text = "Полагаю, что нужно просто доставить череп в каюту к капитану 'Черной Жемчужины'. В этом случае проклятие должно исчезнуть само по себе.";
			link.l1 = "Может быть... А где искать этот корвет?";
			link.l1.go = "LizCheckSkull_3";
		break;
		case "LizCheckSkull_3":
			dialog.text = "Не знаю. Скорей всего, капитан блуждает по морю вдали от городов. Смотрите внимательней вокруг, этот корвет вы не пропустите.";
		...
		case "LizSeekQueen":
			...
			{
				dialog.text = "Капитан, вы нашли корвет 'Черная Жемчужина'?";
				link.l1 = "Нет еще... Напомните, где искать этот корвет?";
				link.l1.go = "LizCheckSkull_3";
			}
		break;
		...
		case "LizOver_1":
			...
			link.l1 = "Понятно. Вы - дочь своего отца, это заметно... Кстати, Аврора поступила ко мне на службу офицером.";
		...
		break;
		case "DanAbordage_1":
			dialog.text = "Нам не о чем разговаривать. Ты напал"+ GetSexPhrase("","а") +" на мой корвет!";
			link.l1 = "Мне нужно было попасть в капитанскую каюту, иного варианта у меня не было!.. Подождите, разве этот корвет ваш?";
		...
		case "DanAbordage_3":
			...
			link.l1 = "Именно по ее поручению я нахожусь сейчас здесь. А вы, как я понимаю, ее старшая сестра Аврора Шеппард?";
			link.l1.go = "DanAbordage_4";
		break;
		case "DanAbordage_4":
			dialog.text = "Точно! Позвольте вас спросить, кто вы так"+ GetSexPhrase("ой","ая") +"?";
			link.l1 = "Меня зовут капитан " + GetFullName(pchar) + ". Я разыскал"+ GetSexPhrase("","а") +" ваш корвет для того, чтобы снять проклятие. Расскажите мне, как получилось, что на вас пало проклятие ацтекского бога мертвых.";
			link.l1.go = "DanAbordage_5";
		break;
		case "DanAbordage_5":
			dialog.text = "Так вот кто это был! А я до сих пор и не знала, что это за чучело.\nДело было на мысе Несбывшихся надежд, в заливе Маракайбо, у Дес-Мойнеса. Вы, наверное, знаете, что я дочь капитана Барбоссы. Несмотря на то, что являюсь женщиной, я сумела, после пропажи отца, отстоять свое право быть капитаном 'Черной Жемчужины'.\n"+
				"Как-то раз на Исла-Тесоро я наняла шкипера, по имени Пьер Кабаль. Это было ошибкой, такого негодяя свет еще не видывал. У берегов Блуфилдс он схватил какого-то молодого индейца. К сожалению, я узнала об этом, когда мы уже подходили к Маракайбо. Негодяй решил продать его, а деньги забрать себе. Мотивировал он это тем, что индеец - лично его законная добыча.\n"+
				...
				"Однако судьба распорядилась иначе. Как только индеец со стоном свалился в траву, откуда ни возьмись появилось ужасное чучело в перьях, схватило мертвого индейца, и громовым голосом заявило, что я и моя команда теперь прокляты, поскольку капитан Чёрной Жемчужины нарушил уговор. Потом чучело исчезло.";
			link.l1 = "Узнаю Миктлантекутли собственной персоной.";
			link.l1.go = "DanAbordage_6";
		break;
		case "DanAbordage_6":
			dialog.text = "Все стояли, как громом пораженные. Ни о каком поединке не могло быть и речи. Но и взять Кабаля на борт корвета тоже было невозможно. Поэтому, с большей частью команды я ушла в море. С Пьером остались только те, кто ему симпатизировал.";
			link.l1 = "А потом вы поняли, что проклятие - не шутка...";
			link.l1.go = "DanAbordage_7";
		break;
		case "DanAbordage_7":
			dialog.text = "В ту же ночь. Отец рассказывал мне в детстве, что он когда-то был проклят и по ночам превращался в страшного скелета, а всё из-за проклятого золота ацтеков. Избавился он от проклятья договорившись с самим Богом Мёртвых. Но я была маленькой и думала, что папа просто рассказывал мне страшную сказку. Представьте себе мой ужас, когда сказка стала былью! ";
		...
		case "DanAbordage_8":
			...
			link.l1 = "Вам повезло, Аврора, что Элизабет узнала вас в скелете, что приходил к ней ночью. И она начала действовать.";
		...
		case "DanAbordage_10":
			...
			link.l1 = "Похоже, что дело уже сделано. Мне нужно было доставить нефритовый череп Миктлантекутли поближе к капитану корвета 'Черной Жемчужины', что я сделал"+ GetSexPhrase("","а") +".";
	...
	PlaySound("VOICE\Russian\GOTHIC\GG\OPENADANOSTEMPLE.wav");
		...
		case "DanAbordage_14":
			...
			sld.monster = false; // признак нежити
			sld.sex = "woman"; 
			sld.viper = false;
...
void SetAvroraInWorld()
{
	...
	sld.Ship.Mode = "Skel";
	FantomMakeCoolSailor(sld, SHIP_FRIGATE_QUEST, "Черная Жемчужина", CANNON_TYPE_CANNON_LBS32, 90, 90, 90);
...
sld.MultiShooter = 2.5;
	...
	sld.city = "Maracaibo"; //определим колонию, из бухты которой с мушкетом выйдет
	...
	sld.mapEnc.Name = "Корвет 'Черная Жемчужина'";
	...
	sTemp = sld.id;
	NullCharacter.capitainBase.(sTemp).quest = "Avrora"; //идентификатор квеста
