...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "ClimeUsurer_1":
			dialog.text = "Тем не менее, это так. Ты долж"+ GetSexPhrase("ен","на") +" отдать мне немедленно " + FindRussianMoneyString(sti(pchar.questTemp.LSCC_climeUsurer.qtyMoney )) + ". У тебя есть такие деньги?";
		...
		case "InGreateTemple":
			...
			link.l1.go = "InGreateTemple_1";
	                                   DoQuestFunctionDelay("LoginDeadmansGod_00", 2.0);
		break;
		...
		case "InGreateTemple_3":
			...
			link.l1.go = "exit";
			AddDialogExitQuestFunction("Tenoc_fightInTemple");
		break;
		...
		case "WinInTemple_6":
			chrDisableReloadToLocation = false;
	                                    ...
	                                    LAi_SetActorType(npchar);
                                                      LAi_ActorGoToLocation(npchar, "quest", "quest1", "none", "", "", "", -1);
			DoQuestCheckDelay("Tenoc_GoDeadmansGod", 3.0);
