...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "HeadMan":
			...
			{
				link.l2 = "Я прибыл"+ GetSexPhrase("","а") +" за наградой, сударь.";
		...
		break;
		case "SharpSeekSpy_6":
			dialog.text = "Хм, ну давайте обсудим. Итак, я мог бы предложить вам жемчуг и деньги, но я предложу нечто, на мой взгляд, более интересное для вас.\nЭто комплект вооружения. В него входит уникальный саксонский двухзарядный пистолет, английский смоллсворд, рейтарская секира и траншейный доспех! Поверьте мне, один доспех дорогого стоит!";
		...
		break;
		case "SharpSeekSpy_9":
			GiveItem2Character(pchar, "talisman1");
		...
		case "SharpSeekSpy_12":
			...
			link.l1 = "Точно так, уважаемый.";
			link.l1.go = "SharpSeekSpy_13";
		break;
		case "SharpSeekSpy_13":
			dialog.text = "Тогда извольте получить. Все это отныне ваше.";
			link.l1 = "Очень рад"+ GetSexPhrase("","а") +", что смог"+ GetSexPhrase("","ла") +" оказаться вам "+ GetSexPhrase("полезен","полезна") +". Прощайте, сударь.";
			link.l1.go = "exit";
			GiveItem2Character(pchar, "blade_16");
			GiveItem2Character(pchar, "pistol9");
		...
		// ==> часовой у поселения
		case "GuardMan":
	                   PlaySound("VOICE\Russian\GOTHIC\SVM\BDT_FIRSTWARN.wav");
			NextDiag.TempNode = "GuardMan";
			dialog.text = NPCStringReactionRepeat("А, всё в порядке, можешь проходить! Позвольте представиться, меня зовут " + GetFullName(npchar) + ". Моя задача - предупредить поселение ловцов жемчуга о возможном нападении. Пираты, знаете ли... А вы кто?", 
				"Приветствую еще раз!", 
		...
		//продажа жемчуга
		case "IndPearlMan_2":
			if (!CheckAttribute(npchar, "quest.DestinyGiftPearl")) npchar.quest.DestinyGiftPearl = rand(1);
			if (sti(npchar.quest.pearl) == 0 || CheckAttribute(pchar, "quest.DestinyGiftPearlToShore_ForSale.id"))
			{
		...
		case "IndPearlMan_3":
			...
			if (npchar.location == "DestinyGiftPearl_town_2")
			{
				LAi_ActorRunToLocation(NPChar, "reload", "reload1", "Shore51", "goto", "goto1", "PearlToShore_ForSale", 10.0);
			}
			else
			{
				LAi_ActorRunToLocation(NPChar, "reload", "reload1", "Shore49", "goto", "goto1", "DestinyGiftPearlToShore_ForSale", 10.0);
			}
			pchar.quest.DestinyGiftPearlToShore_ForSale.idx = npchar.index;
			npchar.quest.DestinyGiftPearl.bigQty = rand(30) + 10;
			npchar.quest.DestinyGiftPearl.smallQty = rand(60) + 20;
		...
		case "IndPearlMan_Sale_2":
			dialog.text = npchar.quest.DestinyGiftPearl.bigQty + " большая, " + npchar.quest.DestinyGiftPearl.smallQty + " маленькая.";
			link.l1 = "Хорошо, беру все!";
			link.l1.go = "IndPearlMan_Sale_3";
		break;
		case "IndPearlMan_Sale_3":
			int iTemp = (sti(npchar.quest.DestinyGiftPearl.bigQty)*10) + (sti(npchar.quest.DestinyGiftPearl.smallQty)*5);
			if (sti(pchar.money) >= iTemp)
			{
				dialog.text = "Духи улыбнулись мне, я доволен...";
				link.l1 = "Бывай, приятель...";
				AddMoneyToCharacter(pchar, -iTemp);
				TakeNItems(pchar, "jewelry11", sti(npchar.quest.DestinyGiftPearl.bigQty));
				TakeNItems(pchar, "jewelry12", sti(npchar.quest.DestinyGiftPearl.smallQty)); 
			}
			else
			{
