...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				link.l1 = "Послушай, ты дела Моргана ведешь. Не слышал ничего про ограбление банка?";
		...
		//********************* Месть корсара *********************
   		...
   		case "RevengeCorsair_Oliver":
			dialog.text = "Что за вопросы такие странные?! Может, и знаю, а почему я тебе об этом должен говорить?";
			link.l1 = "Ты не кипятись. Я просто хочу помочь Генри Моргану, найти тех кто взял банк.";
			link.l1.go = "RevengeCorsair_Oliver_1";
		break;

   		case "RevengeCorsair_Oliver_1":
			dialog.text = "После ограбления таких соискателей как ты - полно, шёл бы ты своей дорожкой от сюда.";
			link.l1 = "Может и полно, но не таких как я. Я Барон " + GetFullName(pchar) + " - смекаешь.";
			link.l1.go = "RevengeCorsair_Oliver_2";
		break;

   		case "RevengeCorsair_Oliver_2":
			dialog.text = "Барон.";
			link.l1 = "Вот именно.";
			link.l1.go = "RevengeCorsair_Oliver_3";
		break;

   		case "RevengeCorsair_Oliver_3":
		dialog.text = "Так вот Барон - не знаю...";
		link.l1 = "Сразу не мог сказать, ладно приятно было познакомиться.";
		link.l1.go = "exit";
		npchar.quest.RevengeCorsair_Oliver = "true";
		pchar.questTemp.RevengeCorsair_Oliver = sti(pchar.questTemp.RevengeCorsair_Oliver)+1;
		if (sti(pchar.questTemp.RevengeCorsair_Oliver) == 2) 
		{
