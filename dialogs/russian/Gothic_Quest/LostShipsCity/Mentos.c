	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				if (pchar.questTemp.LSCC == "toSeekGoods" && npchar.id == "Ment_6" && CheckAttribute(pchar, "questTemp.LSCC.additional.powder") && sti(pchar.questTemp.LSCC.additional.powder))
				{
					link.l8 = "Слушай, мне очень нужны порох и оружие. Адмирал не хочет мне ничего давать, а мне очень нужно...";
					link.l8.go = "SeekGoods";
				}
				//поиски команды на корвет
				if (pchar.questTemp.LSCC == "toSeekPeopleInCrew" && npchar.id == "Ment_6" && !CheckAttribute(pchar, "questTemp.LSCC.crew.ment_6"))
				{
				...
				if (pchar.questTemp.LSCC == "MechanicIsArrest_toResidence" && npchar.id == "Ment_6" && CheckAttribute(npchar, "quest.LSCCofficer"))
				{
		...
		case "SeekGoods_4":
			...
			pchar.questTemp.LSCC.additional.powder = false;
		break;
		//поиски команды на воздушный шар
		case "SeekCrew":
			dialog.text = "Отлично! Когда?";
			link.l1 = "Я сейчас как раз набираю команду на воздушный шар, так что добро пожаловать. Отправляйся на 'Сан Габриэль'. Хенрик Ведекер, который там находится, переправит тебя на мой воздушный шар.";
		...
		case "SeekCrew_3":
			...
			pchar.questTemp.LSCC.crew.ment_6 = true; 
			npchar.quest.LSCCofficer = true; 
		...
		case "MentOffIsLife_3":
			...
			link.l1 = "Спасибо, ты мне здорово помог. Сейчас отправляйся на 'Сан Габриэль', там есть возможность попасть на мой воздушный шар. Остальные граждане Города, пожелавшие уйти со мной, уже там. Ждите нас с Ведекером.";
			link.l1.go = "MentOffIsLife_4";
		break;
		case "MentOffIsLife_4":
			dialog.text = "Хорошо, капитан. Я только осмотрюсь здесь немного...";
			link.l1 = "Хорошо, но не опаздывай на воздушный шар.";
