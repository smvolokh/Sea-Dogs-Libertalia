	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				if (pchar.questTemp.LSCC == "toSeekGoods" && !CheckAttribute(npchar, "quest.bombs"))
				{
					link.l8 = "Вы знаете, я ищу некоторые товары. Не могли бы вы мне помочь?";
					link.l8.go = "SeekGoods";
				}
				if (pchar.questTemp.LSCC == "toSeekGoods" && CheckAttribute(npchar, "quest.bombs") && sti(npchar.quest.bombs))
				{
		...
		break;
		case "SeekGoods_3":
			dialog.text = "Принесите мне 10 имбирных корней, три бутылки вина и лютню. У меня хорошее настроение, я буду петь и учиться играть.";
		...
		case "TakeBombs":
            ...
            if (CheckCharacterItem(pchar, "Mineral35") && CheckCharacterItem(pchar, "potionwine") && sti(pchar.items.potionwine) >= 3 && CheckCharacterItem(pchar, "potion5") && sti(pchar.items.potion5) >= 10)
            {				
		...
		break;
		case "SeekCrew_2":
			dialog.text = "Понятно... Ну что же, я согласна идти к вам на воздушный шар. Что я должна сделать?";
			link.l1 = "Отправляйтесь на 'Сан Габриэль'. Хенрик Ведекер переправит вас на мой воздушный шар. И не медлите!";
