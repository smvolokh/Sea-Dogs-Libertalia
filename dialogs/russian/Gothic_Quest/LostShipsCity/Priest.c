	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				квесты
				if (pchar.questTemp.LSCC != "AdmiralIsWaiting" && !CheckAttribute(npchar, "quest.takeCandles"))
				{
				...
				if (pchar.questTemp.LSCC == "toSeekGoods" && !CheckAttribute(npchar, "quest.takeCandles"))
				{
					link.l8 = "Святой отец, я хочу спросить, нельзя ли мне приобрести у вас некоторые товары?";
					link.l8.go = "SeekGoods";
				}
				if (pchar.questTemp.LSCC == "toSeekGoods" && CheckAttribute(npchar, "quest.takeCandles") && npchar.quest.takeCandles == "seek")
				{
					link.l8 = "Святой отец, я хочу спросить, нельзя ли мне приобрести у вас некоторые товары?";
					link.l8.go = "SeekGoods";
				}
				if (pchar.questTemp.LSCC == "toSeekGoods" && CheckAttribute(npchar, "quest.takeCandles") && npchar.quest.takeCandles == "found")
				{
		...
		break;
		//поиск товаров на воздушный шар не принеся свечи
		case "SeekGoods":
		...
		case "SeekCrew":
			...
			link.l1 = HeroStringReactionRepeat("Я набираю команду на свой воздушный шар и предлагаю вам присоединиться ко мне.", 
				"Да... Извините, запамятовал"+ GetSexPhrase("","а") +".",
                "Как знаете...", 
