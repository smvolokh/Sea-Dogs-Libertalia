	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				//муж Элис Тейлор
				if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "begin")
				{
				...
				if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "toElisGood")
				{
		...
		case "SeekGoods_3":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.additional.balls = true;
		...
		case "SeekCrew_2":
			...
			link.l1 = "Хорошо. Сейчас отправляйтесь на 'Сан Габриэль'. Хенрик Ведекер переправит вас на мой воздушный шар.";
		...
		case "ELTHusb_begin_disagree":
			...
			pchar.questTemp.LSCC.ElisHusband = "over"; //закончим
		break;
		case "ELTHusb_begin_4":
			...
			pchar.questTemp.LSCC.ElisHusband = "seekFreeMotod";
		...
		case "ELTHusb_good_3":
			...
			CloseQuestHeader("ISS_ElisHusband");
			pchar.questTemp.LSCC.ElisHusband = "over"; //закончим
			SetTimerFunction("LSCC_ElisHusbandSet", 0, 0, 1);
		break;
