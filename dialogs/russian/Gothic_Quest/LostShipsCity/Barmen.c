	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				if (pchar.questTemp.LSCC == "toWineCellarGone" && npchar.location.locator == "stay1")
				{
					dialog.Text = "А-а, вот и ты!";
					link.l4 = "Именно! Теперь хотелось бы послушать тебя.";
					link.l4.go = "TizerCellar";
					PChar.quest.LSCC_returnBarmen.over = "yes"; //снимаем таймер на опоздание, разговор состоялся
					break;
				}	
				//после завала Хилла Брюннера
				if (pchar.questTemp.LSCC == "barmenIsDead" && !CheckAttribute(npchar, "quest.HillNB_1"))
				{
					dialog.Text = "Слушаю тебя внимательно.";
					link.l4 = "Какого это дьявола ты тут делаешь? А где Брюннер?";
					link.l4.go = "NB_1";
					pchar.questTemp.LSCC = "barmenIsDeadYouKnow";
					break;
				}
				if (pchar.questTemp.LSCC == "barmenIsDeadYouKnow" || pchar.questTemp.LSCC == "toAdmiralBarmenDead")
				{
				...
				if (pchar.questTemp.LSCC == "afterFightInProtector")
				{
					link.l4 = "Послушай, тут такое дело...";
					link.l4.go = "AfterProtector_1";
				}
				if (pchar.questTemp.LSCC == "toAdmNarvalRing" && CheckCharacterItem(pchar, "DOjeronRing"))
				{
					link.l4 = "Я сделал"+ GetSexPhrase("","а") +", как ты сказал. Кольцо 'нарвалов' у меня.";
					link.l4.go = "NarvalRing_1";
				}
				if (pchar.questTemp.LSCC == "toTavernNarvalDestroyed")
				{
					link.l4 = "Ты знаешь, все получилось!";
					link.l4.go = "AfterNarvalDesrt_1";
				}
				if (pchar.questTemp.LSCC == "waitInterception" && GetQuestPastDayParam("questTemp.LSCC") > 2 && GetHour() < 20)
				{
					link.l4 = "Ну, как дела? Выяснил, что планировал?";
					link.l4.go = "Interception";
				}
				if (pchar.questTemp.LSCC == "InterceptionLate")
				{
				...
				if (pchar.questTemp.LSCC == "InterceptionYouSeen")
				{
				...
				if (pchar.questTemp.LSCC == "waitInterceptionResult")
				{
		...
		case "hall_night_wait":
			DialogExit();
			NextDiag.CurrentNode = NextDiag.TempNode;
			TavernWaitDate_LSCC("wait_night");
		break;
		case "hall_day_wait":
			DialogExit();
			NextDiag.CurrentNode = NextDiag.TempNode;
			TavernWaitDate_LSCC("wait_day");
		break;
		//вопросы
		case "int_quests":
			...
			{
				link.l4 = "Что интересного здесь можно увидеть, и как вы здесь живете, черт возьми?..";
			...
			if (!CheckAttribute(NextDiag, NodeName+".l7") && pchar.questTemp.LSCC == "AdmiralIsWaiting" && CheckCharacterItem(pchar, "keyQuestLSCC"))
			{
			...
			if (!CheckAttribute(NextDiag, NodeName+".l12") && CheckActiveQuest("LSCC_findDekster"))
			{
		...
		break;
		case "ansewer_5_1":
			dialog.text = "Этот шторм, грозы здесь постоянное явление, другой погоды здесь не бывает. У нас положено по прибытии отмечаться у адмирала, так что если ты не явишься к нему, то он найдет тебя сам.";
			link.l1 = "Адмирал?";
		...
		break;
		case "ansewer_4":
	                                PlaySound("VOICE\Russian\GOTHIC\GG_TAVERN\GG_TAVERN_9.wav");
			dialog.text = "Здесь, много чего, а живем вполне нормально. Хотя, знаешь, надоело все до смерти... А вообще, Город живет на основании Закона. О его главных положениях тебе лучше узнать у адмирала Каппера.";
			link.l1 = "И...";
		...
		case "ansewer_6_5":
			...
			link.l1 = "Понял"+ GetSexPhrase("","а") +". И что нам делать дальше?";
			link.l1.go = "ansewer_6_06";
		break;
		case "ansewer_6_06":
	                                PlaySound("VOICE\Russian\GOTHIC\GG\WEITER_00.wav");
			dialog.text = "";
			link.l1 = "";
			link.l1.go = "ansewer_6_6";
		break;
		case "ansewer_6_6":
			dialog.text = "Дальше, я пока пораскину мозгами об этой твоей новости...";
			link.l1 = "Ну ладно, пораскинь.";
			link.l1.go = "int_quests";
			AddQuestRecord("ISS_MainLine", "4");
			AddQuestUserData("ISS_MainLine", "sSex", GetSexPhrase("", "а"));
			SaveCurrentQuestDateParam("questTemp.LSCC"); //запомним дату базара
		break;
		case "ansewer_7":
		...
		break;
		case "ansewer_8":
 	                                PlaySound("VOICE\Russian\GOTHIC\GG_TAVERN\GG_TAVERN_20.wav");
		...
		case "Tizer_2":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toWineCellar";
		...
		case "TizerCellar":
			...
			pchar.quest.LSCC_returnBarmen_2.win_condition.l1 = "ExitFromLocation";
			pchar.quest.LSCC_returnBarmen_2.win_condition.l1.location = pchar.location;
			pchar.quest.LSCC_returnBarmen_2.function = "LSCC_returnBarmen_2";
		break;
		case "TizerCellar_1":
		...
		case "TizerCellar_7":
			...
			if (CheckCharacterItem(pchar, "letter_LSCC_2"))
			{
		...
		case "TizerCellar_11":
			...
			pchar.questTemp.LSCC = "toDiffindoor"; //флаг линейки ГПК
		break;
		...
		case "TizerCellarOk_1":
			...
			TakeItemFromCharacter(pchar, "letter_LSCC_2");
		break;
		...
		case "TizerCellarOk_8":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toAndreLabor"; //флаг линейки ГПК
		...
		case "AfterProtector_8":
			...
			pchar.questTemp.LSCC = "toKillOneNarval";
			pchar.quest.LSCC_KillOneNarval.win_condition.l1 = "locator";
			pchar.quest.LSCC_KillOneNarval.win_condition.l1.location = "VelascoShipInside11";
			pchar.quest.LSCC_KillOneNarval.win_condition.l1.locator_group = "reload";
			pchar.quest.LSCC_KillOneNarval.win_condition.l1.locator = "reload2";
			pchar.quest.LSCC_KillOneNarval.function = "LSCC_KillOneNarval";
		break;
		...
		case "AfterNarvalDesrt_7":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "waitInterception";
			SaveCurrentQuestDateParam("questTemp.LSCC"); //запомним дату базара
		...
		case "Interception_3":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toInterception";
			pchar.questTemp.LSCC.itemState = false; //флаг нахождения в локаторе итем в таверне
		...
		case "InterceptionBadWork_4":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "waitInterceptionResult";
			AddQuestRecord("ISS_MainLine", "21");
			SetTimerFunction("LSCC_changeBarmen", 0, 0, 15);
		break;
		...
		case "InterceptionOk":
			...
			if (sld.location == "FleuronTavernn")
		...
		case "InterceptionOk_8":
			...
			SetTimerFunction("LSCC_changeBarmen", 0, 0, 3);
		break;
		//смерть Брюннера
		case "NB_1":
		...
		case "NB_4":
			...
			link.l1.go = "exit";
			if (pchar.questTemp.LSCC == "barmenIsDeadYouKnow")
			{
				pchar.questTemp.LSCC = "toArmo_BrunnerDead";
			}
