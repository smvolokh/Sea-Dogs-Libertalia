void ProcessDialogEvent()
{
	...
	{
		case "First time":
			if (npchar.location == "SanAugustineResidencee")
			{
			...
			if (npchar.quest.meeting == "0")
			{
				...
				pchar.questTemp.LSCC = "AdmiralFoundHero";
			...
			if (pchar.questTemp.LSCC == "NarvalDestroyed")
			{
			...
			//муж Элис Тейлор
			if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC != "AdmiralIsWaiting" && pchar.questTemp.LSCC.ElisHusband == "toElis")
			{
				link.l5 = "Адмирал, скажи, пожалуйста, что Максим Тейлор наделал тут у тебя?";
				link.l5.go = "ELTHusb_begin";
			}
			if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "toAdmiral")
			{
				link.l5 = "Адмирал, я хотел"+ GetSexPhrase("","а") +" бы помочь Максиму Тейлору.";
				link.l5.go = "ELTHusb_SF";
			}
			if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "toAdmiralGood")
			{
			...
			if (CheckAttribute(pchar, "questTemp.LSCC.lostDecster") && pchar.questTemp.LSCC != "AdmiralIsWaiting" && pchar.questTemp.LSCC.lostDecster == "toAdmiral")
			{
		...
		break;
		case "FT_10":
			if (pchar.questTemp.LSCC == "AdmiralFoundHero")
			{
		...
		case "FT_16":
			...
			AddDialogExitQuestFunction("LSCC_admiralTakeAll");
			NextDiag.TempNode = "First time";
			NextDiag.CurrentNode = NextDiag.TempNode;
			DialogExit();
		break;
		...
		case "FoundHero":
			npchar.quest.meeting = "1";
			NextDiag.TempNode = "First time";
			pchar.questTemp.LSCC = "AdmiralNotFoundHero";
		...
		case "NarvalRing_13":
			...
			pchar.questTemp.LSCC = "toDestoyAllNarval";
			int idxMent;
			pchar.questTemp.LSCC.qtyOfficers = 0;
			string sOffName;
			for (i=1 ; i<=3; i++)
			{
				...
				if (idxMent != -1)
				{
					ChangeCharacterAddressGroup(&characters[idxMent], "LostShipsCityy_town", "goto", "goto16_"+i);
					...
					pchar.questTemp.LSC.qtyOfficers = sti(pchar.questTemp.LSCC.qtyOfficers) + 1;
				}
			}
			pchar.quest.NavalEnterOfficers.win_condition.l1 = "location";
			pchar.quest.NavalEnterOfficers.win_condition.l1.location = "VelascoShipInside11";
		...
		case "DestrNarval_1":
			...
			{
				if (iTemp == sti(pchar.questTemp.LSCC.qtyOfficers))
				{
				...
				DeleteAttribute(pchar, "questTemp.LSCC.qtyOfficers");
			}
		break;
		...
		case "DestrNarval_Bad1":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toTavernNarvalDestroyed";
		...
		case "Interception":
			...
			if (sti(sld.questTemp.LSCC.itemState) && bOk)
			{
				dialog.text = "Рад тебя видеть, адмирал.";
				link.l1 = "Как дела у клана 'Каспер?'";
				link.l1.go = "Interception_1";				
				sld.questTemp.LSCC = "InterceptionOk";
				sld.questTemp.LSCC.knowMechanic = true; //флаг ГГ знает о Механике
			}
			else
			{
				dialog.text = "Рад тебя видеть, адмирал.";
				...
				link.l1.go = "exit";				
				sld.questTemp.LSCC = "InterceptionYouSeen";
		...
		case "BarmenDead_6":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toSeekOldCitizen";
			...
			pchar.quest.LSCC_SaveSesilGalard.win_condition.l1          = "location";
			pchar.quest.LSCC_SaveSesilGalard.win_condition.l1.location = "AvaShipInside33";
			pchar.quest.LSCC_SaveSesilGalard.function                  = "LSCC_SaveSesilGalard";
		break;
		//замочили касперов
		case "CasperDead_1":
		...
		case "CasperDead_14":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "seekMillionAndHalf";
		break;
		//поиск товаров на корвет
		case "SeekGoods":
		...
		case "SeekGoods_10":
			...
			AddQuestUserData("ISS_MainLine", "sSex", GetSexPhrase("", "а"));
			pchar.questTemp.LSCC.additional.powder = true; //флаг на дачу квеста ментом
		break;
		//найм команды
		case "SeekCrew":
		...
		case "fightTalking_3":
			...
			AddDialogExitQuestFunction("LSCC_figtInResidence");
            DialogExit();
		...
		case "ELTHusb_SF_5":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.ElisHusband = "toWaitress";
		...
		case "ELTHusb_good_3":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.ElisHusband = "toElisGood";
		...
		case "LostDecster_6":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.lostDecster = "seekBox";
			AddQuestRecord("LSCC_findDekster", "5");	
		break;
