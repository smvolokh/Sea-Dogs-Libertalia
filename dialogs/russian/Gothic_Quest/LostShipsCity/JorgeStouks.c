...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "FT_8":
			dialog.text = "Ну, может и так... Продолжу. К адмиралу на регистрацию нужно явиться как можно быстрей. Сейчас шторм, поэтому народу на палубах маловато. Но как только он спадет - Каппер сам вас найдет в любом случае. И я советую не тянуть с регистрацией.";
			link.l1 = "Хорошо, это я понял"+ GetSexPhrase("","а") +". И когда, по вашему мнению, утихнит этот шторм?";
			link.l1.go = "FT_9";
		break;
		case "FT_9":
			dialog.text = "Завтра к вечеру точно успокоится. Шторм уже пошел на убыль.";
		...
		case "int_quests":
			...
			if (!CheckAttribute(NextDiag, NodeName+".l4") && CheckCharacterItem(pchar, "keyQuestLSCC"))
			{
		...
		//найм команды
		case "SeekCrew":
			dialog.text = "Команду на воздушный шар?";
			link.l1 ="Да.";
		...
		case "SeekCrew_2":
			...
			link.l1 = "Отлично! Тогда немедленно отправляйтесь на 'Сан Габриэль'. Хенрик Ведекер переправит вас на мой воздушный шар.";
