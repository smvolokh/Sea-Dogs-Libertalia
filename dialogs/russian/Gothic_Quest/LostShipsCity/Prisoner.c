	...
	switch(Dialog.CurrentNode)
	{
		case "prisonerHead":
			...
			else
			{
				...
				if (CheckAttribute(pchar, "questTemp.LSCC.lostDecster") && !CheckAttribute(npchar, "quest.decster"))
				{
					if (pchar.questTemp.LSCC.lostDecster == "seekBox" || pchar.questTemp.LSCC.lostDecster == "toAdmiral")
					{
				...
				//муж Элис Тейлор
				if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "toElis")
				{
					link.l7 = "Не подскажете, а за что сюда попал Максим Тейлор?";
					link.l7.go = "ELTHusb_begin";
				}
				if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "seekFreeMotod")
				{
		...
		case "prisoner":
			...
			else
			{
				...
				//квест со скелетом Декстера
				if (CheckAttribute(pchar, "questTemp.LSCC.lostDecster") && npchar.chr_ai.type == LAI_TYPE_SIT && !CheckAttribute(npchar, "quest.decster"))
				{
					if (pchar.questTemp.LSCC.lostDecster == "seekBox" || pchar.questTemp.LSCC.lostDecster == "toAdmiral")
					{				
				...
				if (CheckAttribute(pchar, "questTemp.LSCC.LegroHelp") && pchar.questTemp.LSCC.LegroHelp == "toTomasBoil" && CheckCharacterItem(pchar, "potionwine"))
				{
				...
				if (CheckAttribute(pchar, "questTemp.LSCC.LegroHelp") && pchar.questTemp.LSCC.LegroHelp == "toSecondToPrison")
				{
					link.l2 = "Приятель, это опять я. И снова по поводу Доминика Легро.";
					link.l2.go = "SecondLogro";
				}
				if (CheckAttribute(pchar, "questTemp.LSCC.LegroHelp") && pchar.questTemp.LSCC.LegroHelp == "to3ToPrison")
				{
				...
				iTemp = GetCharacterIndex("LSCCwaitress");
				if (CheckAttribute(pchar, "questTemp.LSC.LegroHelp") && pchar.questTemp.LSCC.LegroHelp == "over" && CheckAttribute(pchar, "questTemp.LSCC.LegroHelp.Armo") && iTemp != -1 && characters[iTemp].name != "Армо" && pchar.questTemp.LSCC == "toSeekMechanik")
				{
		...
		case "LostDecster_1":
			...
			link.l1 = "Никогда!";
			link.l1.go = "exit";
			AddQuestRecord("LSCC_findDekster", "4");	
			npchar.quest.decster = true; //не повторять базар
		break;
		...
		case "WineLogro_2":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.LegroHelp = "toFackDominic";
		...
		break;
		case "SecondLogro_2":
			iTemp = GetCharacterIndex("LSCCCwaitress");
			if (iTemp != -1 && characters[iTemp].name == "Армо")
			{
				...
				pchar.questTemp.LSCC.LegroHelp.Armo = true; //флаг на то, что квест пошел с участием Армо
			}
			else
			{
		...
		case "SecondLogro_3":
			...
			link.l1 = "Ладно...";
			link.l1.go = "exit";
			pchar.questTemp.LSCC.LegroHelp = "toArmoDominic";
		...
		case "LastLegro_3":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.LegroHelp = "toKeyDominic";
