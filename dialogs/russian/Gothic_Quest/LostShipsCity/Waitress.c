void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.questTemp.LSCC == "barmenIsDead")
			{
			...
			if (pchar.questTemp.LSCC == "barmenIsDeadYouKnow" || pchar.questTemp.LSCC == "toArmo_BrunnerDead")
			{
			...
			else
			{
				...
				if (pchar.questTemp.LSCC == "toSeekMechanik" && !CheckAttribute(pchar, "questTemp.LSCC.Armo"))
				{
					link.l4 = "Послушай, Армо, у меня есть некоторые мысли по поводу смерти Хилла.";
					link.l4.go = "AboutCasper_1";
				}
				//наводка на первого
				if (pchar.questTemp.LSCC == "toSeekMechanik" && CheckAttribute(pchar, "questTemp.LSCC.Armo") && sti(pchar.questTemp.LSCC.Armo) == 0)
				{
					link.l4 = "Армо, есть какие-нибудь новости по первому свиданию?";
					link.l4.go = "Casper1";
				}
				//ждем втогого
				if (pchar.questTemp.LSCC == "toSeekMechanik" && CheckAttribute(pchar, "questTemp.LSCC.Armo") && sti(pchar.questTemp.LSCC.Armo) == 2 && CheckAttribute(pchar, "questTemp.LSCC.Armo.waiting"))
				{
				...
				//ждем третьего
				if (pchar.questTemp.LSCC == "toSeekMechanik" && CheckAttribute(pchar, "questTemp.LSCC.Armo") && sti(pchar.questTemp.LSCC.Armo) == 4 && CheckAttribute(pchar, "questTemp.LSCC.Armo.waiting"))
				{
				...
				if (pchar.questTemp.LSCC == "toSeekMechanik" && CheckAttribute(pchar, "questTemp.LSCC.Armo") && sti(pchar.questTemp.LSCC.Armo) == 11 && npchar.name == "Armo")
				{
					link.l4 = "Армо, прошу прощения, но я, кажется, все-таки опоздал"+ GetSexPhrase("","а") +" к свиданию.";
					link.l4.go = "CasperLate";
				}
                //опоздал или упустил вызванного каспера
				if (pchar.questTemp.LSCC == "toSeekMechanik" && CheckAttribute(pchar, "questTemp.LSCC.Armo") && sti(pchar.questTemp.LSCC.Armo) == 15 && npchar.name == "Armo")
				{
					link.l4 = "Армо, тут такое дело получилось...";
					link.l4.go = "CasperAllFight"; 
				}
				if (pchar.questTemp.LSCC == "seekMillionAndHalf" && npchar.name == "Армо" && !CheckAttribute(npchar, "quest.giveMoney"))
				{
					link.l4 = "Слушай, Армо, у меня тут проблемы финансовые...";
					link.l4.go = "askMoney";
				}
				//освобождение мужа Элис Тейлор
				if (CheckAttribute(pchar, "questTemp.LSCC.ElisHusband") && pchar.questTemp.LSCC.ElisHusband == "toWaitress")
				{
				...
				//поиски команды на воздушный шар
		...
		case "int_quests":
			...
			if (!CheckAttribute(NextDiag, NodeName+".l6") && CheckActiveQuest("LSCC_findDekster"))
			{
				link.l6 = "Послушай, " + npchar.name + ", ты что-нибудь знаешь о Лейтоне Декстере?";
				link.l6.go = "ansewer_6";
			}
			if (!CheckAttribute(NextDiag, NodeName+".l7") && CheckAttribute(pchar, "questTemp.LSCC.LegroHelp") && pchar.questTemp.LSCC.LegroHelp == "over" && npchar.name == "Армо")
			{
		...
		case "NB_14":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC = "toAdmiralBarmenDead";
			npchar.quest.meeting = "1";
			if (CheckAttribute(pchar, "questTemp.LSCC.knowMechanic"))
			{
				...
				DeleteAttribute(pchar, "questTemp.LSCC.knowMechanic");
			}
			else
			{
		...
		case "AboutCasper_9":
			...
			pchar.questTemp.LSCC.Armo = 0; //флаг на мочилово касперов. четные - готовность Армо, нечетные - флаг questAction 
			SaveCurrentQuestDateParam("questTemp.LSCC"); //запомним дату базара
		break;
		//первый каспер
		case "Casper1":
			if (GetQuestPastDayParam("questTemp.LSCC") > 0 && GetHour() < 20)
			{
		...
		case "Casper1_1":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.Armo = 1; //чтобы сработал questAction на полночь
		...
		case "WaitCasper1_1":
			...
			link.l1.go = "exit";
			DeleteAttribute(pchar, "questTemp.LSCC.Armo.waiting");
			DeleteAttribute(pchar, "questTemp.LSCC.Armo.Interrapt");
			SaveCurrentQuestDateParam("questTemp.LSCC"); //запомним дату базара
		break;
		//второй каспер
		case "Casper2":
		...
		case "Casper2_1":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.Armo = 3; //чтобы сработал questAction на полночь
		...
		case "Casper3_1":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.Armo = 5; //чтобы сработал questAction на полночь
		...
		case "CasperLate":
			...
			link.l1 = "Эх, жаль...";
			link.l1.go = "exit";
			pchar.questTemp.LSCC.Armo = 12; //Отмазала девка
		...
		case "CasperAllFight":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.Armo = 16; //чтобы диалога больше не было
		break;
		//Армо мертва
		case "ArmoIsDead":
		...
		case "ELTHusb_SF_3":
			...
			pchar.questTemp.LSCC.ElisHusband = "toZhilBaru";
		break;
