void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "FT_24":
			dialog.text = "Дело в том, что у меня есть воздушный шар. Пару лет назад я задумал смастерить воздушный шар из обломков кораблей. Пока у меня была такая возможность...";
		...
		break;
		case "FT_26":
			dialog.text = "Сначала вы должны купить у меня воздушный шар. Затем достать со дна кое-что, без чего мы не сможем освободить корвет от стискивающих его кораблей.";
		...
		break;
		case "FT_27":
			dialog.text = "Ну, я же не знаю точно, когда случится катастрофа. К тому же, я вижу, что вы "+ GetSexPhrase("человек смелый и деятельный","девушка смелая и деятельная") +", и вам вполне по силам будет выполнить мои условия. Тем более, что я много не прошу за ноу-хао технического прогресса, всего лишь полтора миллиона за воздушный шар и возможность один раз спуститься под воду.";
		...
		case "OffM_GiveMoney_1":
			...
			{
				dialog.text = "Ну что же, дело сделано. Я получил требуемую сумму, теперь воздушный шар ваш.";
				link.l1 = "Очень хорошо. Что дальше?";
				link.l1.go = "FreeCorvett";
			}
		break;
		//освобождние воздушного шара
		case "FreeCorvett":
			dialog.text = "Теперь воздушный шар нужно освободить. Дело в том, что он находится на внешнем кольце корабельного хлама острова и стиснут обломками со всех сторон.";
			link.l1 = "Ну и как это сделать?";
			link.l1.go = "FreeCorvett_1";
		break;
		case "FreeCorvett_1":
			dialog.text = "Я уже все подготовил. У меня на платформе стоит лебедка, к ней я прикрепил обломки вокруг воздушного шара. Так что стоит только привести механизм в действие - и вуаля, воздушный шар свободен!";
		...
		case "FreeCorvett_7":
			...
			pchar.questTemp.LSCC.immersions = 0; //количество совершенных погружений
			pchar.questTemp.LSCC.immersion = "true"; // погружение возможно
			...
			pchar.quest.LSCC_takeUnderwater.function = "LSCC_takeUnderwater"; // выдача костюма

			pchar.questTemp.LSCC.immersions.pinion = false; //флаг найденной шестеренки
...
pchar.quest.LSCC_UnderwaterDolly.win_condition.l1 = "locator";
			...
			pchar.quest.LSC_UnderwaterDolly.function = "LSCC_FindUnderwaterDolly"; // нашли статую

			AddQuestRecord("ISS_MainLine", "42");
			pchar.Ship.Type = GenerateShipExt(SHIP_BALLOON, false, pchar);
			SetBaseShipData(pchar);
			pchar.Ship.name = "Лунный огонёк";
			...
			AddSimpleRumourCityTip("Вы знаете, в Городе появился некий Механик. И говорят, что он старейший житель! Надо же...", "LostShipsCityy", 10, 1, "LSCC", "");
			AddSimpleRumourCityTip("Вы слышали о Хенрике Ведекере? Вот так дела! Оказывается, его держали взаперти на 'Сан Габриэле' много лет.", "LostShipsCityy", 10, 1, "LSCC", "");
			AddSimpleRumourCityTip("Говорят, что новый-старый житель Города, некий Хенрик Ведекер, очень способный человек.", "LostShipsCityy", 10, 1, "LSCC", "");
			AddSimpleRumourCityTip("А я не верю, что этот Механик так важен для Города. Сколько жили без него, проживем и еще...", "LostShipsCityy", 10, 1, "LSCC", "");		
		...
		case "readyArmor":
			...
			LocatorReloadEnterDisable("LostShipsCityy_town", "reload72", false); //открываем вход в Феникс
	                                          LocatorReloadEnterDisable("LostShipsCityy_town", "reload73", true);
			pchar.questTemp.LSCC.immersion = "true"; // погружение возможно
			pchar.quest.LSCC_takeUnderwater.win_condition.l1 = "locator";
			pchar.quest.LSCC_takeUnderwater.win_condition.l1.location = "FenixPlatformm";
		...
		case "FoundPinion_1":
			...
			pchar.questTemp.LSCC.immersions.pinion = true; //нашел шестеренку
		...
		break;
		case "endImmersion_2":
			dialog.text = "Для начала обеспечьте воздушный шар всем необходимым. Весь набор боеприпасов, продовольствие, оружие... Ну, не мне вас учить. Все найденное доставляйте на 'Сан Габриэль', а переправкой этого добра на воздушный шар я займусь сам.";
		...
		case "takeGoods":
			...
			npchar.quest.LSCC.Sailcloth = false; //магазин
			npchar.quest.LSCC.Planks = false; //магазин
			npchar.quest.LSCC.Powder = false;
			npchar.quest.LSCC.Food = false; //магазин
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " ядра,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " картечь,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " книппели,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " бомбы,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " парусину,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " доски,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " порох,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " провиант,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " оружие,";
			...
			npchar.quest.LSCC.(sTemp) = npchar.quest.LSCC.(sTemp) + " медикаменты,";
			if (iTemp < 10)
			{
				if (iTemp == 0)
				{
				...
				else
				{
					...
					link.l1 = "Кое-что уже есть. Удалось достать " + npchar.quest.LSCC.s1 + ". Осталось раздобыть " + npchar.quest.LSCC.s0 + ".";
				}
				link.l1.go = "takeGoods_again";
			}
			else
			{
				link.l1 = "Все, что нужно для воздушного плавания, у меня в трюме.";
				link.l1.go = "TakeCrew";
		...
		//Набор команды
		case "TakeCrew":
			dialog.text = "Прекрасно, "+ GetSexPhrase("друг мой","дорогая моя") +", все уже размещено в трюме вашего воздушного шара. Теперь вам нужно набрать команду из жителей Города. Сами понимаете, вдвоем мы с воздушным шаром не управимся.";
			link.l1 = "Хех, а раньше нельзя было об этом сказать? Я оббегал"+ GetSexPhrase("","а") +" весь Город в поисках товаров на воздушный шар, заодно мог"+ GetSexPhrase("","ла") +" бы и команду набрать.";
		...
		case "TakeCrew_4":
			...
			link.l1 = "Хорошо, я так и сделаю. Кстати, вы сумели освободить воздушный шар?";
			link.l1.go = "TakeCrew_5";
		break;
		case "TakeCrew_5":
			dialog.text = "Еще нет, но опробовал лебедку, она в полной готовности. Договаривайтесь с людьми, пусть они прибывают сюда, а я буду переправлять их на воздушный шар. Нам нужно хотя бы пятнадцать человек, чтобы улететь. Как только вы соберете необходимое количество - сразу же бегите сюда. По вашему прибытию на 'Сан Габриэль', мы задействуем лебедку и освободим воздушный шар.";
			...
			pchar.questTemp.LSCC.crew = 0; //количество уговоренных людей.
		...
		case "TakeCrewNew":
			...
			{
				dialog.text = "Вы еще не набрали достаточного количества людей для плавания. Всего ко мне прибыли " + FindRussianPeoplesString(sti(pchar.questTemp.LSCC.crew), "Acc") + ". Продолжайте, и прошу вас - быстрей...";
			}
			else
			{
		...
		break;
		case "inPrison_4":
			dialog.text = "Нет, это вряд ли. Да и не стоит рисковать воздушным шаром и жизнями стольких людей, что уже находятся на нем. Слушайте, что вам нужно сделать...";
		...
		break;
		case "inPrison_6":
			dialog.text = "Да, я так решил. А теперь слушайте, что вы должны сделать, чтобы попасть на свой воздушный шар.";
			link.l1 = "Я слушаю.";
			link.l1.go = "inPrison_7";
		break;
		case "inPrison_7":
			dialog.text = "Отправляйтесь ко второй мачте 'Фернадо Диффиндура', оттуда до корабля уже рукой подать. Я успел использовать лебедку до ареста и растащил корабли, окружающие воздушный шар. Теперь вы с легкостью оторветесь от Города.\nШторм набирает силу, боюсь, что сейчас вам придется нелегко... При выходе из 'Тартаруса', вы должны четко представлять себе, куда направиться - направо отсюда. Иначе в этом хаосе вы погибнете. Я еще раз повторю, что вы должны двигаться по направлению ко второй мачте 'Фернандо Диффиндура'.";
