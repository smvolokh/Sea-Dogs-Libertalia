	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				if (CheckActiveQuest("LSCC_findDekster") && !CheckAttribute(pchar, "questTemp.LSCC.lostDecster"))
				{
				...
				if (pchar.questTemp.LSCC == "toSeekGoods" && !CheckAttribute(npchar, "quest.grapes"))
				{
					link.l8 = "Послушайте, я ищу некоторые товары.";
					link.l8.go = "SeekGoods";
				}
				if (pchar.questTemp.LSCC == "toSeekGoods" && CheckAttribute(npchar, "quest.grapes") && sti(npchar.quest.grapes))
				{
		...
		case "int_quests":
			...
			if (!CheckAttribute(NextDiag, NodeName+".l2") && pchar.questTemp.LSCC == "toSeekOldCitizen")
			{
		...
		case "TakeGrapes":
            ...
            if (CheckCharacterItem(pchar, "Mineral010") && sti(pchar.items.Mineral010) >= 10)
            {				
		...
		case "TakeGrapes_yes":
			...
			TakeNItems(pchar, "Mineral010", -10);
		break;
		case "TakeGrapes_1":
		...
		case "LostDecster_8":
			...
			link.l1.go = "exit";
			pchar.questTemp.LSCC.lostDecster = "toAdmiral";
			AddQuestRecord("LSCC_findDekster", "3");	
		break;
	}
