...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
 		...
 		case "HaveRecomendation":
            ...
            if (npchar.quest.meeting == "0")
            {
				...
				link.l1 = "Меня зовут " + GetFullName(pchar) + ". А прислал меня к тебе хозяин магазина, что находится на руднике Лос Текес...";
                npchar.quest.meeting = "1";
            }
            else
            {
            	dialog.text = "Опять на горизонте появился наш грозный корсар. Что тебе нужно на этот раз?";
        		link.l1 = "На этот раз меня прислал к тебе хозяин магазина, что находится на руднике Лос Текес...";
            }
            link.l1.go = "Step_5";
		break;
 		...
 		case "Step_5":
			sld = characterFromId(pchar.questTemp.Ascold.TraderId);
        	dialog.text = "Хех, я понял о ком ты говоришь, приятель. Что ж, проходи, садись, давай выпьем водки.\n"+
    		...
    		link.l1 = "Мне сказали, что ты искал порошок мумии. У меня он есть.";
    		link.l1.go = "Step_6";
		break;
		...
		break;
 		...
 		case "No_Powder":
        	dialog.text = "Эй, " + GetSexPhrase("братишка","сестричка") + ", шалишь! Нет у тебя порошка. Людей, которые пытаются меня обмануть, я не признаю больше.\n"+
		...
		break;
 		...
 		case "Step_7":
    		...
    		link.l1 = "Не, мыть я ничего не буду, а вот выпить за знакомство я не против.";
    		link.l1.go = "vodka_1";
		break;

		case "vodka_1":
			dialog.text = "Ну, это по нашему! Вот и славненько! Посидим, побеседуем о жизни! Присаживайтесь, присаживайтесь, месье, Фадей принеси водочку для моего друга. Ох, какая же она будет вкусная, холодненькая!..";
			link.l1 = "...";
			link.l1.go = "vodka_2";
		break;
		...
		case "vodka_3":
                     dialog.text = "";
			link.l1 = "Ты вот лучше скажи мне, зачем тебе эта дрянь? Сколько голову ломал" + GetSexPhrase("","а") + " над этим, а понять не смог" + GetSexPhrase("","ла") + ". Абсолютно бесполезная и мерзкая вещица.";
			link.l1.go = "vodka_4";
		break;
		...
		case "vodka_5":
			...
			sld = ItemsFromID("lcheer");
			sld.shown = false;
			pchar.questTemp.CanDrink = true; // ГГ теперь знает секреты выпивки, это ему потом пригодится
		...
		break;
 		...
 		case "Step_10":
        	dialog.text = "Хорошо. Слушай теперь, что мне нужно разыскать.\n"+
                          "Мне нужна статуэтка Тлалока, бога дождя и грома, церемониальный сосуд и церемониальных нож.";
		...
		case "WalkTwoMounth":
            ...
            {
            	dialog.text = "О-о-о, кого я вижу?! " + GetSexPhrase("Мой друг","Моя дорогая") + " " + GetFullName(pchar) + ", рад видеть. Если ты по поводу работы, то обрадовать пока ничем не могу, сам сижу без гроша в кармане. А ты заглядывай как-нибудь позже.";
		...
		break;
 		...
 		case "AfterManowarsBattle":
            ...
            {
             	link.l1 = "Я потопил" + GetSexPhrase("","а") + " все мановары, но инвентарного списка на кораблях мне найти так и не удалось.";
            ...
            {
             	link.l1 = "Я уничтожил" + GetSexPhrase("","а") + " испанские мановары, инвентарный список тоже у меня.";
        		link.l1.go = "ManowarGoodWork";
            }
            if (pchar.questTemp.Ascold == "Ascold_ManowarsDead" && CheckAttribute(PChar, "questTemp.Ascold.List"))
            {
             	link.l1 = "Мановары я отправил" + GetSexPhrase("","а") + " на дно!";
        		link.l1.go = "ManowarGoodWork";
            }
		break;
		...
		break;
 		...
 		case "Step_22":
        	dialog.text = "Печальные новости. Тогда у меня к тебе просьба - ты долж" + GetSexPhrase("ен","на") + " уничтожить всех испанцев, находящихся близ острова Кайман. Все корабли должны пойти ко дну, а сама экспедиция у храма должна быть уничтожена.\n"+
                          "И мне нужен инвентарый список испанцев.";
        	link.l1 = "Я готов" + GetSexPhrase("","а") + ". Могу приступать хоть сейчас.";
    		link.l1.go = "Step_23";
        	link.l2 = "Не слишком ли много ты хочешь от меня, приятель? Поищи дур" + GetSexPhrase("ака","у") + " в другом месте.";
    		link.l2.go = "No_Work";
		break;
		...
		break;
 		...
 		case "Step_24":
    		...
    		link.l1.go = "Step_25";
            link.l2 = "Это без меня. Ищи дур" + GetSexPhrase("ака","у") + "!";
    		link.l2.go = "No_Work";
		break;
		...
		break;
 		...
 		case "Step_26":
        	dialog.text = "Список это хорошо, ты молодец. Но мне нужно, чтобы ты потопил" + GetSexPhrase("","а") + " все корали испанцев близ бухты Аматике, а также очистил" + GetSexPhrase("","а") + " храм. Никто не должен уйти оттуда живым.";
		...
		break;
 		...
 		case "Mummie_begin":
            ...
            {
            	dialog.text = "О-о-о, кого я вижу, " + GetSexPhrase("мой друг","моя дорогая") + " "+ GetFullName(pchar) + "...";
	...
	PlaySound("VOICE\Russian\GOTHIC\GG\LEE_PERM_2.wav");
		...
		break;
 		...
 		case "Step_34":
    		...
    		link.l1.go = "Step_35";
        	link.l2 = "Слушай, Аскольд, я что, похож" + GetSexPhrase("","а") + " на гробокопателя? В могилах рыться не собираюсь.";
    		link.l2.go = "No_Work";
		break;
		...
		break;
 		...
 		case "Seek_Grave":
            ...
            {
            	dialog.text = "А, добрый день, " + GetSexPhrase("друг мой","дорогая моя") + ". Как дела, чем порадуешь?";
	...
	PlaySound("VOICE\Russian\GOTHIC\GG\HOLDEINGOLD_1.wav");
		...
		break;
 		...
 		case "Step_50":
        	...
        	link.l1 = "Берусь, чего уж. Само" + GetSexPhrase("му","й") + " интересно, что там...";
    		link.l1.go = "Step_51";
		break;
	...
	pchar.quest.Ascold_LeifIsDead.function = "Ascold_LeifIsDead";
		...
		break;
 		...
 		case "GoodFriends":
        	dialog.text = "О-о-о, кого я вижу! Стар" + GetSexPhrase("ина","ушка") + " " + GetFullName(pchar) + "! Рад видеть тебя снова. По делу или просто заш" + GetSexPhrase("ел","ла") + "?";
		...
		break;
 		...
 		case "Step_71":
        	dialog.text = "С тебя я денег не возьму. Мы ведь с де ла Фонтеном должны тебе еще, сколько ты сделал" + GetSexPhrase("","а") + " для нас, ожидая награды. Будем считать, что мы в рассчете. А черепа забирай.";
