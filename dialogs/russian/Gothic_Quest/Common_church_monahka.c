...
void ProcessDialogEvent()
{
	...
	case "Exit_1":
		NextDiag.CurrentNode = NextDiag.TempNode;
		DialogExit();
		Log_info("Теперь на душе так легко... Я снова ощущаю в себе силу Господа!");
		...
		case "First time":
			if (LAi_grp_playeralarm > 0)
			{
       			dialog.text = NPCharRepPhrase(pchar, 
					...
					LinkRandPhrase("Что ты делаешь в церкви, падшая душа? Я требую, чтобы ты удалил"+ GetSexPhrase("ся","ась") +" немедленно, пока солдаты не ворвались и не устроили здесь резню!", "Немедленно покинь храм, безбожни"+ GetSexPhrase("к","ца") +"! Я не заступлюсь за так"+ GetSexPhrase("ого","ую") +", как ты!", "Немедленно покинь храм Божий, " + GetSexPhrase("еретик","ведьма") + "! Мне не нужны здесь убийства!!"));
				link.l1 = NPCharRepPhrase(pchar,
					RandPhraseSimple("Да ладно тебе, сестра...", "Да не нужна мне твоя помощь..."), 
					LinkRandPhrase("Сестра, лучше не путайся под ногами...", "Уйду, не беспокойся...", "Сестра, не суетись, не до тебя, ей богу"));
				link.l1.go = "fight";
				break;
			} 
            if (npchar.quest.BadMeeting != lastspeak_date)
			{
				if(!CheckAttribute(PChar, "") && !CheckAttribute(PChar, "") && NPChar.GenQuest.ChurchQuest_2.GiveQuestDateParam != iMonth && NPChar.GenQuest.ChurchQuest_2.GiveQuestDay != lastspeak_date && NPChar.location != "Panama_church" && NPChar.location != "Minentown_church" && rand(5) == 1 && !CheckAttribute(pchar, "questTemp.Sharlie.Lock")) // 280313
				{
					...
					link.l1 = RandPhraseSimple("Гм! Не помешал"+ GetSexPhrase("","а") +", сестра?", "Вот это спич!");
				...
				link.l1 = "Здравствуйте, сестра.";
				link.l1.go = "node_3";
				Link.l2 = "Ты мне не сестра, и не смей так больше обращаться.";
				Link.l2.go = "node_2";
		...
		case "node_3":
    		...
    		link.l1.go = "donation";
    		link.l2 = RandPhraseSimple("Думаю, настало время исповеди.","Мне нужно покаяться, "+RandPhraseSimple("сестра.", "святая сестра."));
    		link.l2.go = "ispoved";
    		link.l3 = RandPhraseSimple("У меня к вам дело, " + RandPhraseSimple("сестра.", "святая сестра."),
                                           "Я прибыл"+ GetSexPhrase("","а") +" по делу, " + RandPhraseSimple("сестра.", "святая сестра."));
    		link.l3.go = "work";
			...
			{
				link.l5 = "Страшные вещи творятся, святая сестра.";
		...
		case "AffairOfHonor_GodJudgement_1":
			dialog.text = "Не беспокойся, " + GetSexPhrase("сын","дочь") + " мо" + GetSexPhrase("й","я") + ". Господь увидет твои страдания и поможет тебе. Всегда носи с собой крест для укрепления веры.";
			Link.l1 = "Благодарю вас, Святая сестра.";
			Link.l1.go = "exit";
		...
		case "GenQuest_Church_2_Start_1":
			...
			link.l1 = LinkRandPhrase("Святая сестра", "Сестра", "Присвятая сестра") + ", не слишком ли вы усердствуете? Так недолго и удар получить.";
		...
		case "GenQuest_Church_2_Start_2":
			...
			link.l2 = LinkRandPhrase("Это все, конечно, печально, но я, пожалуй, пойду...", "Сочувствую, сестра, но мне уже пора...", "Наверное вы слишком понадеялись на Господа, нужно было замки ставить покрепче. Пойду я, пожалуй...");
			link.l2.go = "exit";
		break;
		...
		case "GenQuest_Church_2_Start_5":
			...
			link.l1.go = "exit";
			link.l2 = LinkRandPhrase("Святая сестра", "Сестра", "Присвятая сестра") + ", но ведь я никакими обетами не связан"+ GetSexPhrase("","а") +", и с готовностью помогу Господу приблизить час возмездия.";
		...
		case "GenQuest_Church_2_Start_5_1": //выше - выход из диалога без подтверждения - странно, однако
			dialog.text = "Я буду молиться за тебя, " + GetSexPhrase("сын","дочь") + " мо" + GetSexPhrase("й","я") + "! Иди и воздай этим мошенникам их заслугу!";
			link.l1 = "Можете не сомневаться, " + RandPhraseSimple("сестра.", "святая сестра.");
		...
		case "GenQuest_Church_2_Thief_2":
			dialog.text = "Ну, разве всеблагой Создатель нашлёт мор и глад на неразумных детей своих, дабы покарать за грехи их тяжкие...";
				link.l1 = "Например, за гордыню, или, допустим, алчность... Что это вы так побледнели, Сестра?";
		...
		case "GenQuest_Church_2_Thief_4_1_1":
			...
			link.l1 = LinkRandPhrase("Святая Сестра", "Сестра", "Мать Тереза") + ", успокойтесь. Главное, что они никому ничего не рассказали... и не расскажут. Кстати, они добровольно отдали мне чашу для причастия, которую я вам и привез"+ GetSexPhrase("","ла") +".";
		...
		case "GenQuest_Church_2_Thief_4_1_3":
			if(rand(1) == 0) // Священник хочет всучить ГГ бабло
			{
				...
				link.l1 = "Это лишнее, сестра " + NPChar.name + ". Я и так небед"+ GetSexPhrase("ен","на") +", а эти деньги используйте на благо ваших прихожан.";
				link.l1.go = "GenQuest_Church_2_Thief_4_1_4";
				// ГГ берет деньги
				link.l2 = "Благодарю, " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ", этого вполне достаточно.";
			...
			else
			{
				...
				link.l1 = "Спасибо за благословение, " + LinkRandPhrase("сестра", "мать Тереза") + ".";
				link.l1.go = "GenQuest_Church_2_Thief_4_1_4";
		...
		case "GenQuest_Church_2_Thief_4_1_6":
			...
			dialog.text = "Ох, дитя моё, совсем забыла... Надеюсь, эта скромная лепта в размере " + FindRussianMoneyString(iMoneyToCharacter) + " окупит Ваши расходы?";
			link.l1 = "Да, " + LinkRandPhrase("сестра", "мать Тереза") + ", спасибо.";
		...
		case "GenQuest_Church_2_Thief_4_2_2":
			...
			link.l1 = "Полегче, сестра, вы её не получите, пока не объяснитесь.";
		...
		case "GenQuest_Church_2_Thief_4_2_3":
			...
			{
				dialog.text = "Что я должна вам объяснять?! Вы же "+ GetSexPhrase("умный человек","умная девушка") +", и сами должны понимать - всё, что наговорили эти проходимцы о том, что я их якобы наняла - это всё наглая ложь! Господь покарает этих безбожников не только за кощунство, но и за лжесвидетельство!";
			...
			else
			{
				...
				link.l1 = "Что ж... я вижу, вы погрязли в грехе настолько, что не боитесь людского суда. Но есть Высший Суд, перед которым все мы рано или поздно предстанем. Возьмите вашу 'церковную утварь' и подумайте о том, достойны ли вы прикасаться к ней, сестра " + NPChar.name + ".";
				link.l1.go = "GenQuest_Church_2_Thief_4_2_4_2_1";
				link.l2 = "Простите мои сомнения, святая сестра, ибо сердце человеческое слабо, а душа в смятении. Возьмите чашу и благословите меня.";
				link.l2.go = "GenQuest_Church_2_Thief_4_2_4_2_2";
			}
		break;
			
		case "GenQuest_Church_2_Thief_4_2_4_1":
			dialog.text = "Э-э-э... "+ GetSexPhrase("Сын мой... Сударь... Господин","Дочь моя... Сударыня... Госпожа") +" " + PChar.name + "... я готова пойти вам навстречу... рассмотреть наши разногласия.... компенсировать ваши дорожные расходы вот этой скромной суммой в размере " + FindRussianMoneyString(iMoneyToCharacter) + "... конечно, в том случае, если вы не станете вмешивать в это дело посторонних...";
		...
		case "GenQuest_Church_2_Finally_Complete_1":
			...
			{
				link.l1 = "Известия неутешительные, " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ": грабители оказались столь коварны и изворотливы, что мне не удалось вернуть церковное золото...";
			...
			{
				link.l1 = "Известия неутешительные, " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ": мне удалось настичь похитителей, но моим единственным трофеем оказалась вот эта чаша для причастия.";
			...
			if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.Complete.With_All"))
			{
				...
				link.l2 = LinkRandPhrase("Святая сестра", "Сестра", "Мать Тереза") + ", я принес"+ GetSexPhrase("","ла") +" дурные вести... Мне пришлось потратить много времени, сил и средств, чтобы выследить мерзавцев, но, когда я всё-таки добрал"+ GetSexPhrase("ся","ась") +" до них, оказалось, что они успели пропить и промотать всё, кроме вот этой чаши для причастия...";
		...
		case "GenQuest_Church_2_Finally_Complete_1_3_1": // ГГ решил заныкать деньги...
			...
			else
			{
				...
				dialog.text = "Ох, "+ GetSexPhrase("сын мой","дочь моя") +". Ваши слова повергли меня в глубокую скорбь, но не в отчаяние, ибо отчаяние - смертный грех! Господь насылает на нас очередное испытание, чтобы укрепить нашу веру. Однако ваши усилия и затраты должны быть вознаграждены, хоть и не принесли плодов. Возьмите эту небольшую сумму - " + FindRussianMoneyString(iMoneyToCharacter) + " -  и продолжайте ваш праведный путь!";
					link.l1 = "Э-хм!... Спасибо, сестра, я направлю эти деньги на богоугодное дело.";
					link.l1.go = "GenQuest_Church_2_Finally_Complete_1_3_1_1";
					link.l2 = LinkRandPhrase("Святая сестра", "Сестра", "Мать Тереза") + "... вы ставите меня в неловкое положение. Я не могу принять от вас вознаграждение за невыполненную работу, лучше обновите на эти деньги алтарь.";
		...
		case "GenQuest_Church_2_Finally_Complete_1_4_1_2":
			...
			link.l1 = "Как же так?! " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + "!..";
		...
		case "GenQuest_Church_2_Finally_Complete_1_2_1":
			dialog.text = "Боже, благодарю Тебя! Вы послужили орудием Господа, дитя моё! Я знала, что Отец наш небесный направит вас верным путём и не оставит своей помощью! Я непрестанно молилась, и вот...";
			link.l1 = "Вообще-то, в таких делах больше пользы от оружия, чем от молитв, сестра. Но, видимо, Бог действительно услышал ваши молитвы.";
		...
		case "GenQuest_Church_2_Finally_Complete_1_2_2":
			...
			link.l1 = "Святая сестра, вы слишком преувеличиваете мои скромные заслуги. Оставляю вас возносить хвалу Господу...";
		...
		case "donation":
			...
			Link.l1 = "Простите, святая сестра, но пожертвования не будет.";
			...
			{
				Link.l5 = "Святая сестра, я хочу сделать пожертвование не от себя. Я делаю это по поручению.";
		...
		case "No donation":
			...
			Link.l1.go = "node_3";
			Link.l2 = "Извините, святая сестра, мне пора.";
			Link.l2.go = "exit";
		break;
		...
		case "donation paid_100":
			...
			Link.l1 = "Мне надо поговорить с вами, святая сестра.";
			Link.l1.go = "node_3";
			Link.l2 = "Извините, святая сестра, мне пора.";
			Link.l2.go = "exit";
...
case "ispoved":
			...
			{   
			link.l2 = "Сестра, дело в том, что я дал обет служения Господу, я проливал свою и чужую кровь как воду, чтобы искоренить зло и утвердить торжество Истинной Веры...";
	...
	case "ispoved_11":
		...
		case "ispoved_2":
			...
			link.l1 = "Нет, конечно, сестра.";
			link.l1.go = "ispoved_3";
			link.l2 = "Всякие были, сестра... всякие.";
			link.l2.go = "ispoved_4";
		break;

	case "ispoved_22":
			dialog.text = "Это непозволительно для служителя Церкви и Господа!";
			link.l1 = "Я знаю, сестра, поэтому и пришёл... Всё это было не ради наживы, а ради служения Вере...";
		...
		case "ispoved_3":
			...
			link.l1 = "Спасибо, сестра, вы меня очень утешили... я пойду.";
			link.l1.go = "exit";
		break;

	case "ispoved_33":
			dialog.text = "Если твои слова правдивы, то ты несмоненно пожертвуешь десятину от своего имущества во славу Бога и Церкви...";
			link.l1 = "Непременно, сестра.";
...
AddMoneyToCharacter(pchar, -iMoney);
		dialog.text = "Тогда Господь прощает тебе эти грехи. Ибо век наш жесток, и уничтожая недругов Церкви, ты спасаешь жизни и души христиан!";
		link.l1 = "Спасибо, сестра, вы меня очень утешили... я пойду.";
		link.l1.go = "exit_1";
		break;
        ...
        case "work":
			...
			link.l1 = "Я хотел"+ GetSexPhrase("","а") +" бы задать вам один вопрос, сестра...";
			...
			{
	            link.l3 = RandPhraseSimple("Святая сестра", "Сестра") + ", я хотел"+ GetSexPhrase("","а") +" бы поговорить с вами о финансовых вопросах."; //(пересылка в кредиты)
			...
			{
	            link.l7 = RandPhraseSimple("Святая сестра", "Сестра") + ", я здесь по поручению одного человека. Его зовут губернатор " + GetFullName(characterFromId(pchar.GenQuest.Intelligence.MayorId)) + ".";
	            link.l7.go = "IntelligenceForAll";
            }
			//--> Jason Церковная депеша
			if (CheckAttribute(pchar, "GenQuest.Monkletter") && npchar.city == pchar.GenQuest.Monkletter.City)
			{
	            link.l10 = "Сестра, я привез" + GetSexPhrase("","ла") + " для вас бумаги от монаха из города "+XI_ConvertString("Colony"+pchar.GenQuest.Monkletter.StartCity)+".";
			...
			{
	            link.l11 = "Святая сестра, я привез молитвенники для вашего прихода.";
		...
		case "prihod":
			if(!CheckAttribute(PChar, "GenQuest.ChurchQuest_1.StartQuest") && !CheckAttribute(PChar, "GenQuest.ChurchQuest_2.StartQuest") && NPChar.GenQuest.ChurchQuest_1.GiveQuestDateParam != iMonth && NPChar.GenQuest.ChurchQuest_1.GiveQuestDay != lastspeak_date && NPChar.location != "Panama_church" && rand(5) == 1)
		    {
				...
				link.l1 = LinkRandPhrase("Сестра", "Святая сестра", "Мать Тереза") + ", я истинно верующ"+ GetSexPhrase("ий","ая") +" христиан"+ GetSexPhrase("ин","ка") +", и мои намерения послужить матери-Церкви - искренни."+ GetSexPhrase(" Кто же станет её опорой, если не такие верные рыцари, как я?","") +"";
		...
		case "GenQuest_Church_1_Start_1":
			...
			link.l1 = LinkRandPhrase("Сестра", "Святая сестра", "Мать Тереза") + ", я буду счастлив"+ GetSexPhrase("","а") +" оказать вам любую услугу, даже если для этого мне придётся проскользнуть в саму преисподнюю! Не в церкви будь сказано.";
		...
		case "GenQuest_Church_1_Start_2":
			...
			link.l1 = "Боюсь, " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ", что не оправдаю вашего доверия. Дело слишком деликатное, и для его решения нужно быть изрядным дипломатом.";
			link.l1.go = "exit";
			link.l2 = LinkRandPhrase("Сестра", "Святая сестра", "Мать Тереза") + ", ваше поручение не представляется мне сложным. Я сегодня же отправлюсь в путь.";
		...
		case "GenQuest_Church_1_Dialog_1":
			if(CheckAttribute(PChar, "GenQuest.ChurchQuest_1.2A_Scene"))	// Сцена 2а
			{
				...
				link.l1 = "Однако, " + LinkRandPhrase("сестра", "святая сестра", "мать Тереза") + ", души прихожан отца " + PChar.GenQuest.ChurchQuest_1.PriestName + " столь же нуждаются в спасении, а без этих трудов он лишён поддержки столпов теософии и вдохновения в своих проповедях.";
			...
			else	// Сцена 2б-а и 2б-б
			{
				...
				link.l1 = "Именно, " + RandPhraseSimple("сестра", "святая сестра") + ", именно. Поэтому сестра " + PChar.GenQuest.ChurchQuest_1.PriestName + " и поручила мне доставить эти книги лично ему в руки.";
		...
		case "GenQuest_Church_1_Dialog_1_1": // Сцена 2а
			...
			link.l1 = "Да, святая сестра, вы правы, но в голову мне пришли другие библейские строки: 'не ищи лёгких путей, ибо на них подстерегает нас Диавол, и голос его тем вкрадчивей, а соблазн тем сильнее, чем легче путь!' А сестра " + PChar.GenQuest.ChurchQuest_1.PriestName + " уже намеревался архиепископу доложить о вашей забывчивости, едва я его отговорил"+ GetSexPhrase("","а") +", предложив свою помощь.";
			link.l1.go = "GenQuest_Church_1_Dialog_1_1_1";
		break;
			
		case "GenQuest_Church_1_Dialog_1_1_1":
			dialog.text = "А-а-архиепископу?! О такой мелочи!.. Ну зачем же, зачем же? Я и сам давно хотела отослать эти труды любезному брату моему во Христе, да как-то всё не попадалось надёжной оказии. Но в тебе, "+ GetSexPhrase("сын мой","дочь моя") +", я узрела достойного слугу Божьего. Вот, вручаю книги и рукописи, доставь их отцу " + PChar.GenQuest.ChurchQuest_1.PriestName + " в целости и сохранности и присовокупи мою глубочайшую благодарность.";
			link.l1 = "Всенепременнейше присовокуплю, " + LinkRandPhrase("сестра", "святая сестра", "мать Тереза") + ". Всенепременнейше.";
		...
		case "GenQuest_Church_1_Dialog_1_2":	// Сцена 2б-а и 2б-б
			...
			else // Сцена 2б-б
			{
				...
				link.l1 = LinkRandPhrase("Святая сестра", "Сестра", "Мать Тереза") + "... а вы уверены, что этому капитану можно доверять?.. И еще я хотел"+ GetSexPhrase("","а") +" бы узнать, как называется его судно.";
		...
		case "GenQuest_Church_1_Dialog_1_2_2_2":
			...
			link.l1 = "Это я уже слышал"+ GetSexPhrase("","а") +", " + LinkRandPhrase("сестра", "святая сестра", "мать Тереза") + ".";
		...
		case "GenQuest_Church_1_Dialog_1_2_2_3":
			...
			link.l1 = "Я понял"+ GetSexPhrase("","а") +", " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ", я понял"+ GetSexPhrase("","а") +"! Богобоязненный капитан " + PChar.GenQuest.ChurchQuest_1.CapFullName + ", известный всем Карибам картежник и пьяница, благодаря вашему внушению решился переименовать своё корыто?..";
		...
		case "GenQuest_Church_1_Dialog_1_2_2_4":
			...
			link.l1 = "Ох, " + RandPhraseSimple("святая сестра", "сестра") + "... благодарю вас за всё. И помолитесь за мою грешную душу...";
		...
		case "GenQuest_Church_1_Complete_1":
			...
			link.l1 = RandPhraseSimple("Святая сестра", "Мать Тереза") + ", на свидание со Святым Петром я пока, слава Создателю, не собираюсь. А живым, кроме обещаний райской благодати, надобно много разных материальных вещей...";
		...
		case "GenQuest_Church_1_Complete_2":
			...
			link.l1 = "Благодарю, " + LinkRandPhrase("святая сестра", "сестра", "мать Тереза") + ". Возьмите ваши долгожданные книги и рукописи, и помолитесь за мою грешную душу... Прощайте";
		...
		case "healthAdd_3":
			...
			else
			{
    			...
    			link.l1    = "Спасибо, "+RandPhraseSimple("сестра.", "святая сестра.");
    			link.l1.go = "exit";
			}
		break;
		...
		case "HostessChurch_call":
			if (isBadReputation(pchar, 5)) 
			{
				...
				else
				{
					...
					link.l1 = "Святая сестра, как же так?! Мы к вам со всей душой, а вы...";
		...
		case "HostessChurch_notCall":
			if (rand(10) < GetCharacterSPECIAL(pchar, "Charisma"))
			{
				...
				link.l1 = "Да, сестра, именно так. Примите пожертвование от чистого сердца!";
			...
			else
			{
				...
				link.l1 = "Святая сестра, на всех деньгах - кровь...";
		...
		case "Monkletter":
			if (CheckAttribute(pchar, "GenQuest.Monkletter.Late"))
			{
				...
				link.l1 = "Меня задержали непредвиденные обстоятельства, святая сестра.";
			...
			else
			{
				...
				link.l1 = "Спасибо, святая сестра. Был рад помочь святой церкви!";
		...
		case "Monkletter_1":
			...
			link.l1 = "Вот, возьмите, сестра. Всего доброго!";
		...
		case "Monkletter_3":
			...
			link.l1 = "До свидания, святая сестра.";
		...
		case "Churchbooks":
			...
			link.l1 = "Благодарю, сестра. До свидания!";
		...
		case "Churchbooks_2":
			if (CheckAttribute(pchar, "GenQuest.Churchbooks.Late"))
			{
				...
				link.l1 = "Меня задержали непредвиденные обстоятельства в пути, святая сестра.";
