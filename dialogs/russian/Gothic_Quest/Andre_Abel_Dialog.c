void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Andre_Abel_3":
			dialog.text = "Но за работу я готов заплатить не более 3000 песо, и мне нужно уложиться в 15 дней.";
		...
		case "Andre_Abel_7":
			dialog.text = RandPhraseSimple("Нам повезло с ветром, капитан " + GetFullName(PChar) + ".", "Саванна-ла-Мар - милый городишко, но за то время, пока я в нем торчал, он успел надоесть мне до чёртиков.");
		...
		case "Andre_Abel_15Days_Left_1": // Это на случай, если время закончиться тогда, когда Абель будет подходить к ГГ в порту Порт-о-Принса
			if(PChar.location == "FortOrange_tavern")	// Из таверны можно было не выходить fix 
			{
				dialog.text = PChar.Name + ", вы меня подвели! Не представляю, какие дела могли заставить свободного капитана проболтаться в этой чёртовой Саванна-ла-Мар целых полмесяца!";
				link.l1.go = "Andre_Abel_15Days_Left_In_Rosea";
				break;
			}
		...
		case "Andre_Abel_15Days_Left_In_Rosea":
...
//			LAi_ActorGoToLocatorFunc(NPChar, "reload", "reload6_back", "Andre_Abel_Quest_Delete_Andre", 15);
		...
		case "Andre_Abel_In_PortPax":
			dialog.text = "Спасибо, капитан, вы меня очень выручили. Я оплачу ваши расходы дублонами - не возражаете?";
...
//			LAi_ActorGoToLocatorFunc(NPChar, "reload", "reload6_back", "Andre_Abel_Quest_Delete_Andre", 15);
        ...
        case "Andre_Abel_In_PortPax_6":
			...
			ChangeCharacterAddressGroup(NPChar, "PortPax_tavern", "sit", "sit_front1");
			DoFunctionReloadToLocation("PortPax_tavern", "sit", "sit_base1", "Andre_Abel_Quest_Dialog_In_PortPax_Tavern");
		break;
			
		case "Andre_Abel_In_Tavern_1":
			dialog.text = "В общем так... Месяца четыре назад я, при не очень благоприятных обстоятельствах, встретился с Торусом...";
			link.l1 = "Постой! Не о том ли Торусе ты говоришь, который заправляет французкими пиратами?";
			link.l1.go = "Andre_Abel_In_Tavern_2";
		break;
			
		case "Andre_Abel_In_Tavern_2":
			dialog.text = "Именно о нём... Так вот, Торус, пользуясь ситуацией, предложил мне свою защиту в обмен на 50 процентов от всей моей прибыли. Я отказался. Во-первых: потому что это форменный грабёж, а во-вторых: я не думал, что Торус сам станет организовывать мне проблемы. Но я ошибся...\n"
                          ...
                          + "Еще через пару месяцев я встретил пиратскую эскадру возле берегов Антигуа... Если бы не английский сторожевой фрегат, патрулировавший прибрежные воды, мы бы здесь с тобой уже не разговаривали...\n"
                          + "Примерно пол месяца спустя, я в составе небольшого торгового каравана пришёл в Саванна-ла-Мар. Там разгрузил свой потрепанный галеон и продал его в надежде сбить людей Торуса с толку... Потом купил и оснастил флейт, взял фрахт до Порт-о-Принса и стал ждать попутчика. Но сроки по фрахту заставили меня искать эскорт...\n" 
                          + "А дальше ты и сам"+ GetSexPhrase("","а") +" всё знаешь...";
			link.l1 = "И это всё под носом Торуса? Да уж, приятель, влип ты в историю...";
			link.l1.go = "Andre_Abel_In_Tavern_3";
		break;
			
		case "Andre_Abel_In_Tavern_3":
			dialog.text = "Я много раз думал, стоило ли тогда соглашаться на условия Торуса. И знаешь, что я тебе скажу?.. Я уже и готов бы пойти на попятную, только какой ему теперь резон нянчиться с тем, чей бизнес рушится к чёртовой матери...\n" 
                          + "Слушай, выручи, а?.. Сходи на Доминику к Торусу.";
		...
		case "Andre_Abel_In_Tavern_4":
			...
			link.l2 = "Попробовать, конечно, можно, но до Доминики полмесяца пути, а у меня были совсем другие планы...";
		...
		case "Andre_Abel_In_Tavern_5":
			...
			link.l1 = "Торус сам отстанет, когда увидит, что ты на мели... Да не отчаивайся ты так.";
		...
		case "Andre_Abel_In_Tavern_7":
			...
			link.l1 = "А чёрт, была не была! Ладно, схожу к Торусу, авось что-то получится.";
			link.l1.go = "Andre_Abel_In_Tavern_8";
			link.l2 = "Нет, иметь дело с Торусом за такое сомнительное вознаграждение - это не по мне.";
		...
		case "Andre_Abel_In_Tavern_10":
			ref rChar = CharacterFromID("Torus");
			DialogExit();
			...
			LAi_SetSitType(NPChar);
			LAi_SetPlayerType(PChar);
			QuestSetCurrentNode("Torus", "Andre_Abel_Quest_Torus_Dialog_1");
			DoFunctionReloadToLocation("PortPax_tavern", "tables", "stay3", "");
		break;
			
		case "Andre_Abel_In_Tavern_Continue_Quest":
			dialog.text = "Ну что, говорил"+ GetSexPhrase("","а") +" с Торусом? Он согласился?";
			...
			{
				link.l1 = "Да, всё получилось. Торус тебе больше не угрожает.";
				link.l1.go = "Andre_Abel_In_Tavern_Quest_Complete_1";
			}
			break;
			
		case "Andre_Abel_In_Tavern_Quest_Complete_1":
			dialog.text = "Ну, слава Богу! Не зря я на тебя так надеялся... Вот твоя коллекция драгоценностей, держи... Ну, и торговая голландская лицензия.";
			link.l1 = "Отлично. Но откуда, ты же вроде как на мели?";
		...
		case "Andre_Abel_In_Tavern_Quest_Complete_3":
			dialog.text = "Хм, а ты думаешь, с какой радости Торус за мной по всем Карибам гонялся?..\n" 
	...
	link.l1.go = "Andre_Abel_In_Tavern_Quest_Complete_4";
		...
		case "Andre_Abel_In_Tavern_Quest_Complete_4":
			...
			Log_Info("Вы получили торговую лицензию сроком 90 дней");
			PlaySound("interface\important_item.wav");
