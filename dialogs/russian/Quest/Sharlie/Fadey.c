...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (CheckAttribute(pchar, "questTemp.Sharlie"))
			{
				if (pchar.questTemp.Sharlie == "fadey")
				{
					...
					link.l1 = "Добрый день, месье Фадей. Я, конечно, незнаком вам, но зато вы знаете моего друга. Позвольте представиться - " + GetFullName(pchar) + ", друг Мишеля де Монпе.";
					link.l1.go = "Sharlie";
					break;
				}
				if (pchar.questTemp.Sharlie == "takeknife" && !CheckAttribute(pchar, "questTemp.Persian"))
				{
					dialog.text = "А-а, опять вы, " + GetName(NPChar) + "! Что на этот раз?";
				...
				{
					dialog.text = "Что скажете, " + GetFullName(pchar) + "? Сходили посмотреть на это индейское чучело?";
				...
				{
					dialog.text = "Ха, да это сам месье " + GetFullName(pchar) + "! Проходите-проходите... Что вас привело в мой дом?";
					link.l1 = TimeGreeting()+", Фадей. Я к вам пришел по совету моего друга. Он сказал, что вы обязательно сумеете помочь мне найти какую-нибудь работу.";
					link.l1.go = "trial";
					break;
				}
				if (pchar.questTemp.Sharlie == "trial" && !CheckAttribute(npchar, "quest.vodka"))
				{
					dialog.text = "Ха, наш старый добрый знакомый! Какими судьбами, месье " + GetFullName(pchar) + "?";
					link.l1 = TimeGreeting()+", Фадей. Наш общий друг настаивал, чтобы я с вами об этом не беседовал, но я все-таки не удержался...";
					link.l1.go = "vodka";
					break;
				}
		...
		case "Sharlie":
			dialog.text = "А-а, так вы друг старого доброго Мишеля! Это в корне меняет дело! Проходите-проходите, месье, присаживайтесь. Может, по стаканчику водочки за знакомство? У меня еще осталось несколько бутылок, и я с радостью разопью одну из них с одним из друзей моего лучшего друга!";
			link.l1 = "Простите, месье Фадей, но выпивать мне сейчас совсем не с руки. Я бы предпочел сразу перейти к делам.";
			link.l1.go = "Sharlie_1";
		break;
		
		case "Sharlie_1":
			dialog.text = "Эх, жаль... Ром - ничто по сравнению с русской водкой! Вы не понимаете, от чего отказываетесь. Ладно, раз к делам - так к делам. Слушаю вас, " + GetFullName(pchar) + ".";
			link.l1 = "Месье, наверное то, о чем я сейчас скажу, вам не очень понравится, но я вынужден - увы! Мишель мне сообщил, что вы должны ему приличную сумму - карточный долг. Также он распорядился, чтобы вы передали деньги мне, ибо мой друг сейчас... не может лично явиться к вам.";
			link.l1.go = "Sharlie_2";
		break;
		
		case "Sharlie_2":
			dialog.text = "Ох, молодой человек! Я, безусловно, все помню и безоговорочно согласен с тем, что должен вашему другу. Карточный долг - святое, это известно каждому дворянину. Но я прошу вас войти в мое положение. У меня здесь недавно случилась пренеприятнейшая история - меня обокрали, обокрали самым наглым образом!\nПроклятые воры вынесли почти все, что находилось у меня в доме, все, что я нажил за долгие годы, украли мою бесценную коллекцию, вынесли все мои сбережения! Я просто не в состоянии сейчас погасить долг перед вашим другом деньгами\nНо, месье " + GetFullName(pchar) + ", я ни в коем случае не отказываюсь от своих обязательств! Я просто прошу вас пойти мне навстречу. Поскольку денег у меня сейчас все равно нет, я предлагаю вам в качестве оплаты долга вот этот великолепный раритетный кинжал. Взгляните на него!\nЭто произведение оружейного искусства, гордость моей бывшей коллекции. Кроме того, он заговорен индейскими шаманами и обладает очень сильными скрытыми свойствами. От индейцев он получил название 'Коготь вождя'\nЯ бы ни за что не согласился отдать его, поскольку я купил его за немалую сумму, намного превышающую размер моего долга, но учитывая обстоятельства, готов отдать его вам. Берите, это очень выгодная для вас сделка!";
		...
		case "Sharlie_9":
			dialog.text = "Ну, мне нравится собирать редкие вещи... Послушайте, " + GetFullName(pchar) + ", может, вы сможете мне помочь в этом вопросе с клинками...";
		...
		case "Sharlie_12":
			dialog.text = "Вот и отлично! Я надеюсь, удача улыбнется вам. А я в долгу не останусь. Да, вы представляете, " + GetFullName(pchar) + ", эта кража - еще не все. Буквально на днях - представляете, ко мне снова залез воришка\nНу это уж слишком! Но тут он от меня не ушел: я поймал его, задал хорошую взбучку и сдал коменданту. Думаю, его скоро повесят. И поделом ему!";
		...
		case "Sharlie_13":
			dialog.text = "Жаль, конечно. Но на нет - и суда, как говорится, нет. Да, вы представляете, " + GetFullName(pchar) + ", эта кража - еще не все. Буквально на днях - представляете, ко мне снова залез воришка\nНу это уж слишком! Но тут он от меня не ушел: я поймал его, задал хорошую взбучку и сдал коменданту. Думаю, его скоро повесят. И поделом ему!";
			link.l1 = "Вы думаете, он из тех, кто вынес ваши сабли?";
			link.l1.go = "Sharlie_14";
		...
		case "Sharlie_15":
			dialog.text = "Сходите-сходите, подивитесь на местные ходячие диковины... Ладно, " + GetFullName(pchar) + ", приятно поговорили - вы же сейчас назад, на Мартинику? Желаю вам счастливого пути.";
		...
		case "Tichingitu_3":
			dialog.text = "Эх, " + GetFullName(pchar) + "... Вы такой же, как и ваш друг... Ладно. Поступим по-деловому. Раз вам так нужен этот индеец - компенсируйте мне моральную неустойку за него и я напишу просьбу о помиловании, которую, конечно же, удовлетворят. Идет?";
			link.l1 = "Сколько?";
			link.l1.go = "Tichingitu_4";
		break;
		
		case "Tichingitu_4":
			dialog.text = "Ха, вы уже начинаете кое-что понимать в жизни на Карибах, " + GetFullName(pchar) + ". Сто пятьдесят золотых дублонов.";
		...
		case "Tichingitu_6":
			dialog.text = "И поторопитесь, " + GetFullName(pchar) + "! Дней через десять они уже не понадобятся...";
		...
		case "Tichingitu_8":
			...
			PlaySound("interface\important_item.wav");
			dialog.text = "Это спорный вопрос, " + GetFullName(pchar) + ". А если вы спасаете убийцу или насильника? Впрочем, Бог с вами. Ступайте к коменданту, пока вашего краснокожего друга не вздернули на виселицу...";
		...
		case "vodka_1":
			dialog.text = "Ох, месье " + GetFullName(pchar) + "... Ваш друг меня, право, удивил. Ладно - эти полуграмотные торгаши, но он, он! Вот скажите, " + GetFullName(pchar) + ", если бы вам кто-нибудь принес старый двуручный рыцарский меч времен короля Артура и предложил купить, сколько бы вы за него заплатили?";
		...
		case "vodka_3":
			dialog.text = "Вот! Как хорошо сказано! Вы молодец, " + GetFullName(pchar) + ", что не стали таить обиду и сразу пришли ко мне за разъяснениями. Надеюсь, теперь инцидент исчерпан?";
		...
		case "vodka_4":
			...
			dialog.text = "Постойте, " + GetFullName(pchar) + "! Скажу честно: вы мне нравитесь. Возможно, мы даже когда-нибудь станем с вами друзьями. И я предлагаю сделать первый шаг в этом направлении. Я только-только охладил в погребке в ключевой воде две бутылочки отличной русской водки. Предлагаю распить их за наше знакомство и за здоровье нашего друга Мишеля.";
		...
		case "vodka_no":
			dialog.text = "Ну и зря. Ром по сравнению с водкой - поганое дешевое пойло. Но раз спешите - не буду задерживать. Удачи, " + GetFullName(pchar) + "!";
			link.l1 = "И вам того же, месье Фадей...";
			link.l1.go = "exit";
		break;
		
		case "vodka_5":
			dialog.text = "Ай, молодца! Вот и славненько! Посидим, побеседуем о жизни! Присаживайтесь, присаживайтесь, месье, Аскольд принеси водочку для моего друга. Ох, какая же она будет вкусная, холодненькая!..";
			link.l1 = "...";
			link.l1.go = "vodka_6";
		break;
		...
		case "vodka_6":
			DialogExit();
			sld = ItemsFromID("Rumm");
			sld.shown = true;
			sld.startLocation = "BasTer_houseSp1";
			sld.startLocator = "bottle1";

			sld = ItemsFromID("Grog");
			sld.shown = true;
			sld.startLocation = "BasTer_houseSp1";
			sld.startLocator = "bottle2";
			...
			DoQuestReloadToLocation("BasTer_houseSp1", "sit", "sit1", "Sharlie_DrinkWithFadey");
			NextDiag.CurrentNode = "vodka_7";
		break;
		
		case "vodka_7":
			dialog.text = "";
			link.l1 = "Ох... В-вот-т, скажите м-мне, Фадей, н-ну, как так п-получается? Ик! Мы выпили с вами... д-две бут-тылочки в-водочки... кстати - отличная вещь! - и у вас с Аскольдом ни в од-дном глаз-зу, а я.. пьян, к-как сапожник? А?";
			link.l1.go = "vodka_8";
		break;
		
		case "vodka_8":
			dialog.text = "" + GetFullName(pchar) + ", ну вы же расслаблятесь, отдыхаете. Причем тут - как сапожник? Водочка у меня отличная, поспите немного - и будете как стеклышко, даже голова болеть не будет. Вот увидите.";
		...
		case "vodka_12":
			dialog.text = "Отдохните, " + GetFullName(pchar) + ". Славно мы с вами сегодня посидели! Удачи вам и до встречи!";
		...
		case "trial":
			...
			link.l1 = "Ну, примерно так. А разве вы не окажете помощь другу вашего лучшего друга? Тем более у Мишеля сейчас трудности и я помогаю ему с их решением. Тем паче, что я же не прошу у вас денег, я прошу предоставить возможность их заработать.";
			link.l1.go = "trial_1";
		break;
		
		case "trial_1":
			dialog.text = "Хм. Нет, не откажу, конечно. Хочешь накормить человека - не давай ему рыбу, а дай удочку, так, месье " + GetFullName(pchar) + "? Значит, вам нужна работа... Хорошо. Я поговорю с одним человеком и рекомендую ему вас. Ну, а остальное будет зависеть только от вашей исполнительности.";
		...
		case "guardoftruth":
			...
			link.l1 = "Местному приходу один капер подарил за отпущение грехов наперстный золотой крест с лазуритом. Уникальная и дорогая вещь. Беда в том, что этот крест был похищен из другого прихода. Причем при грабеже был убит священник.";
		...
		case "guardoftruth_15":
			dialog.text = "От вас ничего не утаишь, "+pchar.name+". Вы такой же проницательный, как и ваш друг. Да, я выиграл эту дагу в кости у старины Арчибальда. Кто он такой? Да я и сам толком не ведаю. Познакомились мы случайно, разговорились, выпили немного, он поведал о грехах своих тяжких...";
