...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		case "Che_nado":
			Dialog.Text = "Посылали за мной, кэп? Зачем я вам сдался?";
			Link.l1 = "Я хочу обсудить твое поведение на судне, матрос.";
		...
		case "Che_nado2":
			if (rand(1) == 0)
			{
				Dialog.Text = LinkRandPhrase("Эх... Вы по поводу той бутылки рому, что я стырил из трюма?","Ну, не бил я  этого идиота, он сам об мачту башкой треснулся!","У нас пол команды от работы отлынивают, когда палубу драить надо, чего я сразу крайний?");
				Link.l1 = "Любопытно, конечно, но я не об этом. А о том, что ты регулярно нарушаешь дисциплину и порядок на корабле!";
				Link.l1.go = "Che_nado3";
			}	
			else	
			{
				Dialog.Text = LinkRandPhrase("О чем мне с тобой разговаривать? Не нравится мое поведение - иди в таверне эль разносить.","Поведение? У нас тут теперь школа благородных девиц, а не корабль? Тоже мне капитан. Развел тут сопли.","Ха! Осуждай, на здоровье. Я всегда веду себя так как хочу. И ты мне не указ, пусть ты и капитан!");
				Link.l2 = "Вполне ожидаемо. Твоя дерзость привела тебя в могилу, матрос... Теперь готовьсься к смерти, приятель.";
		...
		case "Che_nado3":
			if (rand(1) == 0)
			{
				Dialog.Text = LinkRandPhrase("Чай, мы не на королевском флоте, к дисциплинам и порядкам не привыкли. Мы вольные моряки, капитан! И свобода для нас не пустой звук!","Капитан, Вы, верно, недавно с Европ всяких, верно? Вы много чего не знаете или не понимаете. Мы вольные моряки, а не матросы королевского флота. Всякие дисциплины, уставы, распорядки - это не к нам.","Кэп, мы не на королевском флоте. Я служил некогда на военном судне и все эти дисцилины, уставы у меня уже в печенках сидят! Не для этого я стал вольным моряком, чтобы снова подчинятся каким-то там надуманным правилам.");
				Link.l1 = "Меня это не волнует. Либо ты  ведешь себя как должно, либо отправляешься на тот свет.";
				Link.l1.go = "Che_nado4";
			}	
			else	
			{
				Dialog.Text = RandPhraseSimple("Виноват, кэп. Исправлюсь! Честное слово, такого больше не повторится!","Ах, это. Ну дык, исправлюсь. Вы не подумайте плохого, капитан, мы все же не на королевском флоте, а вольные моряки, к дисциплинам и порядкам разным не приучены.");
				Link.l2 = "Понимаю. Впередь, смотри у меня!";
				Link.l2.go = "Che_nado_ispravilsya";
				Link.l3 = "Другого шанса у тебя не будет, к сожалению... Пора тебе проведать Морского Дьявола!";
				Link.l3.go = "Gop_stope";
			}	
		break;		
		
		case "Che_nado_ispravilsya":
			Dialog.Text = RandPhraseSimple("Есть, кэп! Буду вести себя как следует.","Не волнуйтесь, больше такого не поворится!");
			Link.l2 = "Очень надеюсь. Ты свободен, матрос.";
		...
		case "Che_nado4":
			...
			{
				Dialog.Text = RandPhraseSimple("Эх, ладно... Впредь буду более дисциплированным, кэп.","Как прикажите, капитан.");
				Link.l2 = "Ты свободен, матрос.";
				Link.l2.go = "Odumalsya_matros";
			}
			else
			{
				Dialog.Text = RandPhraseSimple("Ха, черта с два! Еще бы мне какой-то пижон угрожал!","Ты хоть знаешь за какой конец сабли-то держаться?");
				Link.l2 = "Тебе пора на рандеву с Дейви Джоунсом, сынок!";
				Link.l2.go = "Gop_stope";	
			}
		break;
	...
	SetTimerCondition("NETRazborkiNaKorable", 0, 0, 0, false);
			AddCharacterExpToSkill(PChar, "Leadership", 700); // харизма повышается 
		...
		case "Zaplatka_matrosu":
			Dialog.Text = "Здрасьте, капитан.";
			Link.l1 = "Матрос, что ты тут делаешь?";
			Link.l1.go = "Zaplatka_matrosu2";
		break;

		case "Zaplatka_matrosu2":
			Dialog.Text = "Дык это, прибираюсь в каюте у Вас.";
			Link.l1 = "Хорошо. Смотри, чтобы все на своих местах потом осталось.";
			Link.l1.go = "exit";
		break;		
