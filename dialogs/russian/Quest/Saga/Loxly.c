	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				{
					link.l1 = "Вы мне и нужны. Я доверенное лицо некой юной особы по имени Элизы Мак Артур, родной внучки небезызвестного Николаса Шарпа. После смерти ее матери и дяди она единственная наследница своего деда, и претендует на право владения островом Исла-Тесоро, согласно его завещанию. У нас есть обе половины карты покойного, что является условием вхождения в наследство. Сколько вы хотите за то, чтобы помочь Элизе стать владелицей острова по закону?";
					link.l1.go = "saga";
				}
			}
			else
			{
				if (CheckAttribute(pchar, "questTemp.Saga"))
				{
					if (pchar.questTemp.Saga == "loxly") // первое обращение по саге
					{
						...
						link.l1 = "Да, потребовались. Я доверенное лицо некой юной особы по имени Элиза Мак Артур, родной внучки небезызвестного Николаса Шарпа. После смерти ее матери и дяди она единственная наследница своего деда, и претендует на право владения островом Исла-Тесоро. Сколько вы хотите за то, чтобы помочь Элизе стать владелицей острова по закону?";
						link.l1.go = "saga";
						break;
					}
					...
					if (pchar.questTemp.Saga == "court" && GetNpcQuestPastDayParam(npchar, "court_date") >= 1 && IsOfficer(characterFromId("Helena")))
					{
						...
						link.l1 = "Да. Позвольте представить - Элиза Мак Артур.";
						link.l1.go = "saga_26";
						break;
					}
					dialog.text = "Вам потребовались мои услуги, сэр? Уверяю, вы останетесь довольны результатом.";
		...
		case "saga":
			// третья проверка времени. Отбираем Элизу, чистим квестовый лут, даем флаг на Свенсона, вызываем перемены
			if (CheckAttribute(pchar, "questTemp.Saga.Late"))
			{
				RemoveItems(pchar, "map_sharp_full", 1);
		...
		case "saga_5":
			...
			link.l1 = "Но согласно законам Англии при наличии карты у Элизы все права на этот остров!";
			link.l1.go = "saga_6";
		break;
		...
		case "saga_6":
			...
			link.l1 = "Это фамилия ее приемного отца, но Элиза может носить фамилию Шарп, так как является родной дочерью покойной Беатрисс Шарп, дочери Николаса.";
			link.l1.go = "saga_7";
		break;
		
		case "saga_7":
			dialog.text = "Прекрасно! Но кто это сможет подтвердить под присягой? Приемные родители Элизы были лично знакомы с Беатрисс Шарп?";
		...
		case "saga_8":
			...
			link.l1 = "Но только у Элизы есть полная карта Николаса!";
			link.l1.go = "saga_9";
		break;
		...
		case "saga_10":
			...
			link.l1 = "Навряд ли. Столько лет прошло. Если верить легенде, отцом Элизы мог быть сам капитан Бучер. Быть может, бывший палач Сент-Джонса смог бы что-то сказать по этому поводу. Он был последний, кто видел ее отца.";
			link.l1.go = "saga_11";
		break;
		...
		case "saga_21":
			dialog.text = "Тут многое зависит от вас, сэр. Как только вы выплатите мне вторую часть гонорара - 450 дублонов, я подам документы в суд и максимально ускорю начало процесса. Не позже, чем через неделю после вашего взноса, Элиза войдет в права наследства.";
			if (GetCharacterItem(pchar, "gold_dublon") >= 450)
			{
		...
		case "saga_29":
			...
			{
				link.l1 = "Все бумаги, подтверждающие права Элизы на остров в порядке?";
				link.l1.go = "saga_30";
			}
			else
			{
		...
		case "saga_32":
			...
			link.l1 = "Вот и славно. А теперь разрешите откланяться - у нас с Элизой еще очень много дел.";
			link.l1.go = "saga_33";
		break;
		...
		case "saga_34":
			...
			link.l1 = "Понятно... Все бумаги, подтверждающие права Элизы на остров в порядке?";
			link.l1.go = "saga_30";
		break;
