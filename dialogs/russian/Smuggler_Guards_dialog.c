void ProcessDialogEvent() 
{
    ...
    switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{   
			    link.l4 = "Офицер, вы как никогда вовремя. По поручению губернатора, я вычислил банду контрабандистов. Однако я и мои офицеры - неважные фехтовальщики. Надеюсь вы поможете нам?";
				link.l4.go = "ThisTimeFree2";
			}
		break;

		case "ThisTimeFree2":
			dialog.snd = "Voice\COGU\COGU002";		
			dialog.text = "Не волнуйтесь, капитан " + Pchar.name + "! Мы разберемся с этими негодяями, а вам лучше поспешить с донесением к губернатору. И не забудьте упомянуть того, кто пришел к вам на помощь в тяжелую минуту!";
			Link.l1 = "Обязательно упомяну, офицер. Обязательно!";
			Link.l1.go = "ThisTimeFree_exit1";
		break;
		
		case "ThisTimeFree_exit1":
			Diag.CurrentNode = Diag.TempNode;
			NPChar.quest.meeting = NPC_Meeting;
