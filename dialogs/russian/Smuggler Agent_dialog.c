void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				link.l6 = "Слушай, мне позарез нужна сотня другая рабов. Уже и не знаю, к кому обращаться.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				else
				{
					...
					{
						dialog.text = "Вот это шанс прищучить наглеца! Это очень, очень любопытно. Давай так: я плачу тебе " + sti(pchar.questTemp.different.GiveShipLetters.price4) + " серебром, а ты делаешь вид, что забыл"+ GetSexPhrase("","а") +" документы на столе?";
						link.l1 = "Пожалуй, нет.";
						link.l1.go = "SM_ShipLetters_2";
						link.l2 = "Да, ты угадал, уже начинаю забывать!";
						link.l2.go = "SM_ShipLetters_3";
		...
		case "pirateStartQuest_Smuggler_3":
			dialog.text = "Это обойдется тебе в 30 000 песо, по 300 за раба. Есть столько?";
			link.l1 = "По рукам.";
			link.l1.go = "pirateStartQuest_Smuggler_4";
		break;
		case "pirateStartQuest_Smuggler_4":
			pchar.questTemp.pirateStartQuest.Shore = "Shore39";
			dialog.text = "Ладно. Приходи с деньгами в место под названием бухта Ле Марен, там и потолкуем...";
		...
		case "Meeting_3":
			...
			if (iTmp)
			{
				...
				dialog.text = NPCStringReactionRepeat("Ты бы еще на королевском линкоре явил"+ GetSexPhrase("ся","ась") +". Да твою посудину за милю видать из форта. Мы не будем рисковать своими головами. Приходи на меньшем корабле, и только на одном.", 
