...
// перехват основного диалога, только один раз
		...
		case "node_3":
			...
			Link.l2.go = "quests"; //(пересылка в файл города)
			 ...
			 if (cannonMax < 42 && !CheckAttribute(&RealShips[sti(Pchar.Ship.Type)], "Tuning.Cannon"))	 
                    {
                        Link.l41 = "Я хочу увеличить калибр орудий на своём судне.";
    			        Link.l41.go = "dogovor";  // if (sti(RealShips[sti(pchar.Ship.Type)].Class) >= 6) для опраты по классам
    			    }
		...
		{
          dialog.text = "Извините, но лодками я не занимаюсь. Удачи!";
           link.l1 = "Прошу прощения за беспокойство.";		  
		   link.l1.go = "exit";
		} 
		if (sti(RealShips[sti(pchar.Ship.Type)].Class) == 5)
		{
          dialog.text = "Обзаведитесь карабликом посерьёзнее, тогда и поговорим! кхе..кхе..";
           link.l1 = "Эх, жаль...";		  
		   link.l1.go = "exit";
		} 
		if (sti(RealShips[sti(pchar.Ship.Type)].Class) == 4)
		{
          dialog.text = "Увеличение калибра орудий на вашем корабле будет стоить один миллион песо. Деньги вперед.";
           link.l1 = "Договорились!";		  
		   link.l1.go = "oplata_4";                              // сменить на кейс перехода к оплате
		   link.l2 = "Увы, но у меня нет столько денег.";
		   link.l2.go = "exit";
		} 
		if (sti(RealShips[sti(pchar.Ship.Type)].Class) == 3)
		{
          dialog.text = "Увеличение калибра орудий на вашем корабле будет стоить два миллиона песо. Деньги вперед.";
           link.l1 = "Договорились!";		  
		   link.l1.go = "oplata_3";                              // сменить на кейс перехода к оплате
		   link.l2 = "Увы, но у меня нет столько денег.";
		   link.l2.go = "exit";
		}
        if (sti(RealShips[sti(pchar.Ship.Type)].Class) == 2)
		{
          dialog.text = "Увеличение калибра орудий на вашем корабле будет стоить три миллиона песо. Деньги вперед.";
           link.l1 = "Договорились!";		  
		   link.l1.go = "oplata_2";                              // сменить на кейс перехода к оплате
		   link.l2 = "Увы, но у меня нет столько денег.";
		   link.l2.go = "exit";
		} 
         if (sti(RealShips[sti(pchar.Ship.Type)].Class) == 1)
		{
          dialog.text = "Увеличение калибра орудий на вашем корабле будет стоить четыре миллиона песо. Деньги вперед.";
           link.l1 = "Договорились!";		  
		   link.l1.go = "oplata_1";                              // сменить на кейс перехода к оплате
		   link.l2 = "Увы, но у меня нет столько денег.";
		   link.l2.go = "exit";
		} 		
		break;
		// оплата за увеличение калибра 
		case "oplata_4":
		if (sti(Pchar.money) < 1000000)  // если нет денег
			{
			   dialog.text = "Приятель, у тебя и денег то нет..Вот когда появятся, тогда и будем говорить!";
               link.l1 = "Эх, жаль! Придется пока подождать..";
               link.l1.go = "exit";
               break;			   
			}
		       AddMoneyToCharacter(pchar, -1000000);
			   dialog.text = "Ну, теперь когда деньги получены, можно к делу приступить!";
               link.l1 = "Жду с нетерпением!";
               link.l1.go = "ship_tunning_cannon_complite";
		break;	
        case "oplata_3":
		if (sti(Pchar.money) < 2000000)  // если нет денег
			{
			   dialog.text = "Приятель, у тебя и денег то нет..Вот когда появятся, тогда и будем говорить!";
               link.l1 = "Эх, жаль! Придется пока подождать..";
               link.l1.go = "exit";
		...
		break;	
        case "oplata_2":
		if (sti(Pchar.money) < 3000000)  // если нет денег
			{
			   dialog.text = "Приятель, у тебя и денег то нет..Вот когда появятся, тогда и будем говорить!";
               link.l1 = "Эх, жаль! Придется пока подождать..";
               link.l1.go = "exit";
		...
		break;
        case "oplata_1":
		if (sti(Pchar.money) < 4000000)  // если нет денег
			{
			   dialog.text = "Приятель, у тебя и денег то нет..Вот когда появятся, тогда и будем говорить!";
               link.l1 = "Эх, жаль! Придется пока подождать..";
               link.l1.go = "exit";
		...
		case "quests":
			...
			if (pchar.questTemp.BlueBird == "toSeekBermudes") //бриг "Черная Горгулья"
			{
...
//*************************** Открывание двери ***************************
	...
	// ------------------------------------------- хронометр Алекса ----------------------------------------------
		...
		case "clock_begin_4":
			dialog.text = "Мне не хватает 200 единиц бакаута, 250 емкостей смол, 220 рулонов корабельного шелка и 180 бухт пеньковых канатов. Как думаете, разыщете мне все это месяца за четыре? Больше я навряд ли смогу ждать.";
			link.l1 = "По крайней мере - попытаюсь. Что насчет оплаты?";
			link.l1.go = "clock_begin_5";
		break;
		
		case "clock_begin_5":
			dialog.text = "Расценки таковы: бакаут - 35 дублонов за штуку, смолы - 12 дублонов за емкость, шелк - 24 дублона за рулон, канаты - 13 дублонов за бухту. И если вы все мне привезете, получите полезный подарок от меня.";
		...
		case "sandal": // бакаут
			...
			{
				iSum = 35*iSan;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" бакаута. Вы привезли "+FindRussianQtyString(iSan)+". Значит, осталось довезти "+FindRussianQtyString(iTemp)+". Итак, исходя из цены в 35 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
			...
			{
				iSum = 35*amount;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" бакаута. Вы привезли "+FindRussianQtyString(iSan)+". Значит, вопрос с бакаутом мы с вами закрыли! Итак, исходя из цены в 35 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо!";
				link.l1.go = "clock_1";
			...
			Log_Info("Вы получили "+FindRussianDublonString(iSum)+"");
		break;
		...
		case "oil": // смолы
			...
			{
				iSum = 12*iOil;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" смол. Вы привезли "+FindRussianQtyString(iOil)+". Значит, осталось довезти "+FindRussianQtyString(iTemp)+". Итак, исходя из цены в 12 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо! Скоро будет и все остальное.";
				link.l1.go = "clock_1";
			...
			{
				iSum = 12*amount;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" смол. Вы привезли "+FindRussianQtyString(iOil)+". Значит, вопрос со смолами мы с вами закрыли! Итак, исходя из цены в 12 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо!";
				link.l1.go = "clock_1";
		...
		case "shipsilk": // шелк
			...
			{
				iSum = 24*iSil;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" шелка. Вы привезли "+FindRussianQtyString(iSil)+". Значит, осталось довезти "+FindRussianQtyString(iTemp)+". Итак, исходя из цены в 24 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо! Скоро будет и все остальное.";
				link.l1.go = "clock_1";
			...
			{
				iSum = 24*amount;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" шелка. Вы привезли "+FindRussianQtyString(iSil)+". Значит, вопрос с шелком мы с вами закрыли! Итак, исходя из цены в 24 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо!";
				link.l1.go = "clock_1";
		...
		case "ropes": // канаты
			...
			{
				iSum = 13*iRop;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" канатов. Вы привезли "+FindRussianQtyString(iRop)+". Значит, осталось довезти "+FindRussianQtyString(iTemp)+". Итак, исходя из цены в 13 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо! Скоро будет и все остальное.";
				link.l1.go = "clock_1";
			...
			{
				iSum = 13*amount;
				dialog.text = "Так-так. Мне еще необходимо "+FindRussianQtyString(amount)+" канатов. Вы привезли "+FindRussianQtyString(iRop)+". Значит, вопрос с канатами мы с вами закрыли! Итак, исходя из цены в 13 золотых, ваше вознаграждение составляет "+FindRussianDublonString(iSum)+". Извольте получить.";
				link.l1 = "Спасибо!";
				link.l1.go = "clock_1";
