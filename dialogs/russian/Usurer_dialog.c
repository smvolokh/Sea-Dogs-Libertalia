...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		//<-- Бремя гасконца
			...
			{
				dialog.Text = "Добрый день. Меня зовут " + GetFullName(npchar) + ". Чем могу быть вам полезен?";
				link.l1 = "Я к вам по делу. У меня есть старая монета не для продажи, возможно вы сможете рассказать мне об этой монете, о её происхождении?";
			...
			if (CheckAttribute(pchar, "questTemp.piratesLine.Q5.city_1") && npchar.city == pchar.questTemp.piratesLine.Q5.city_1)
			{
				...
				{
				link.l12 = "Говорят, что вы Джона Эйвори нанимали.";
				link.l12.go = "StepPL5First_1";
				}
				if (pchar.questTemp.piratesLine == "PL5Hunter_toFirst")
				{
				link.l12 = "Послушайте, не нанимали ли вы Джона Эйвори?";
				link.l12.go = "StepPL5First_1";
				}
			}
			if (CheckAttribute(pchar, "questTemp.piratesLine.Q5.city_2") && npchar.city == pchar.questTemp.piratesLine.Q5.city_2)
			{
				...
				{
				link.l12 = "Говорят, у вас были проблемы с Джоном Эйвори...";
				link.l12.go = "StepPL5Second_0";
				}
				if (pchar.questTemp.piratesLine == "PL5Hunter_toSecond")
				{
				link.l12 = "Послушайте, не нанимали ли вы Джона Эйвори для выполнения поручений?";
			...
			{
			link.l12 = "Я ищу Джона Эйвори, к вам он не обращался?";
			...
			{
			link.l12 = "Я слышал"+ GetSexPhrase("","а") +", у вас случилось несчастье, возможно, вам понадобится помощь?";
		...
		//<--пиратка, квест №5, заглушка на возврат бабла
			...
			{
			 // if (rand(9) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 30 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 30)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 30); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 30 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 50); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 50) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 50)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 50)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
                 Link.l2 = "Чёрт, у меня нет с собой столько дублонов!";  // ЕСЛИ НЕТ ДУБЮЛОНОВ ТО ЗОВЕТ СТРАЖУ
			    link.l2.go = "oblom";  
				}
				break;
			  }
			...
			{
			 // if (rand(8) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 40 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 40)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 40); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 40 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 60); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 60) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 60)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 60)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			 // if (rand(7) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 50 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 50)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 50); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 50 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 70); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 70) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 70)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 70)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
                   // if (rand(6) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 60 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 60)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 60); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 60 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 80); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 80) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 80)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 80)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			 // if (rand(5) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 70 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 70)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 70); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 70 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 90); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 90) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 90)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 90)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			 // if (rand(4) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 80 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 80)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 80); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 80 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 100); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 100) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 100)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 100)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			  //if (rand(3) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 90 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 90)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";
				RemoveItems(pchar, "gold_dublon", 90); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 90 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 110); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 110) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 110)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 110)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			 // if (rand(2) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 100 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 110)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";  
				RemoveItems(pchar, "gold_dublon", 100); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 100 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 120); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 120) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 120)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 120)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
			...
			{
			 // if (rand(1) == 1)// ТОРГОВЕЦ УЗНАЛ ГГ ПО НЗГ
			  { 
				dialog.text = "Уважаемый, а Вы курсе что за Вас дают награду? Мы можем уладить этот вопрос за 130 дублонов.";
				if (GetCharacterItem(pchar, "gold_dublon") >= 130)
				{	
				Link.l1 = "Ах, ты, дьявол! Ладно, чёрт с тобой, вот твои дублоны! Показывай товары.";
				RemoveItems(pchar, "gold_dublon", 130); // ОТДАЛ ПРИНУДИТЕЛЬНО ДУБЛОНЫ И К ТОРГОВЛЕ 
			    Log_Info("Вы отдали 130 дублонов!");
PlaySound("VOICE\Russian\GOTHIC\GG_GELD\GELD_1.wav");
PlaySound("_CheatSurfMenu_\Gold.wav");
AddCharacterExpToSkill(pchar, SKILL_LEADERSHIP, 150); //+ харизма
AddCharacterExpToSkill(pchar, SKILL_SNEAK, 150) //+ скрытность 
AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 150)//+ везение
AddCharacterExpToSkill(pchar, SKILL_COMMERCE, 150)//+ торговля
              	Link.l1.go = "next"; 
              	}
				else
				{	
		...
		case "oblom":
		    dialog.text = "Тогда я вызываю стражу. Стража, сюда! Здесь преступник!!!";
            link.l1 = "Ну и каналья же ты!";
...
*/			
		...
		case "Wincorvette_1":
			dialog.text = "Да, конечно. За эту сложнейшую работу я рассчитаюсь соответствующим образом. Я презентую вам вот эту эксклюзивную подзорную трубу.";
		...
		case "Wincorvette_2":
			...
			Log_Info("Вы получили эксклюзивную подзорную трубу");
			PlaySound("interface\important_item.wav");
...
//<-- семейная реликвия
		...
		break;
		case "StepPL5Four_3":
			dialog.text = "...сколько денег пропало. Как о ком?! О грабителе, конечно! Ах, мои деньги! Ну, ничего, Джон Эйвори ему покажет! Эйвори - это сила! Мощь!.. Он имел наглость бахвалиться, что мол, руки у меня коротки, его на Исла-Тесоро достать...";
			link.l1 = "Кто? Эйвори?!";
		...
		case "UsurerTakeCoin":
			dialog.text = "Месье, я ростовщик а не историк.";
			link.l1 = "Я это понимаю, но всё же, вам приносят разные побрякушки, золотые украшения индейцев, поэтому я и спрашиваю...";
			link.l1.go = "UsurerTakeCoin_1";
		break;

		case "UsurerTakeCoin_1":
			dialog.text = "Месье, в этом вы правы, действительно приносят, но я не разбираюсь в истории и происхождении, меня интерисует - золото и только золото, а эта монета не из золота, я уже такую видел. Кстати заметить, обладатель такой же монетой - мертв.";
			link.l1 = "Мне это известно. Значит вы мне не поможете...";
			link.l1.go = "UsurerTakeCoin_2";
		break;

		case "UsurerTakeCoin_2":
			dialog.text = "Отнюдь, помогу, на такой вопрос сможет вам ответить один из моих вкладчиков.";
			link.l1 = "Ну вот, хоть что - то!";
			link.l1.go = "UsurerTakeCoin_3";
		break;

		case "UsurerTakeCoin_3":
			dialog.text = "Живёт он на Мартинике в Форт де Франсе, зовут его месье Mиксер, к сожалению фамилии не знаю, а так же не знаю где его дом. При случае я вам ничего не говорил.";
			link.l1 = "Значит Миксер, что ж - спасибо, я всё понял!";
			link.l1.go = "exit";
