...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "relation":
			...
			if (pchar.GenQuest.Piratekill > 20)
			{
				Link.l6 = "Я хочу помириться с пиратами.";
				Link.l6.go = "Piratekill";
		...
		case "Piratekill":
			if (IsCharacterPerkOn(pchar, "Trustworthy")) Pchar.questTemp.Relations.sum = makeint(0.3 * stf(Pchar.rank)/stf(Pchar.reputation)*DIPLOMAT_SUM) * 50;
			else Pchar.questTemp.Relations.sum = makeint(0.1 * stf(Pchar.rank)/stf(Pchar.reputation)*DIPLOMAT_SUM) * 100;
			...
			{
				Link.l1.go = "Piratekill_agreed";
			}
			Link.l2 = "Пожалуй, я передумал"+ GetSexPhrase("","а") +".";
			Link.l2.go = "exit";
		break;
		
		case "Piratekill_agreed":
