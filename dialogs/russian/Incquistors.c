...
PlaySound("VOICE\Russian\GOTHIC\GG_HALLO_M\GG_HALLO_M_" + rand(15) + ".wav");
			...
			if (findsubstr(npchar.id, "Prisoner", 0) != -1) //заключенные
			{
				if (LAi_group_IsActivePlayerAlarm())
				{
					...
					{
						link.l1 = "Послушай, ты не знаешь здесь такого человека по имени Горн?";
...
//**************************** сдаем зачарованный Дес-Мойнес ********************************
		...
		break;
		case "MCIncq_3":
			dialog.text = "А чтобы ты и в дальнейшем "+ GetSexPhrase("старался","старалась") +" как следует, прими в награду за труды 10 тысяч песо.";
