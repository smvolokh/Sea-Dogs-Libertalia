...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			{
				link.l1 = "Я к тебе по делу касательно Паскаля Вуазье.";
		...
		break;
		case "BlueBird_6":
			dialog.text = "Уверен. Только никто меня не слушает, все думают, что это говорит обида на него за потерянное дело на Исла-Тесоро. Оно, конечно, не без этого, но не до такой же степени!\nА потом я вообще перестал что-то говорить - намекнули добрые люди, чтобы я поутих...";
			link.l1 = "М-да... Ты знаешь, я сейчас занимаюсь поисками брига 'Черная Горгулья'. Именно этот корабль грабит личные суда торговцев.";
			link.l1.go = "BlueBird_7";
		break;
		case "BlueBird_7":
			dialog.text = "Думаю, что этот бриг действует как минимум по наводке мерзавца Паскаля. И очень даже может быть, что это и вовсе его корабль\nТы рассуди, каков хитрец - грабит торговцев, а потом ссужает им их же деньги!";
			link.l1 = "М-да, слов нет... Послушай, так как же мне отловить этот бриг?";
			link.l1.go = "BlueBird_8";
		break;
		case "BlueBird_8":
			dialog.text = "Не знаю... Но вот что посоветую. Подумай, как он доставляет товар к себе в магазин. В порт его бриг вряд ли причаливает.";
			link.l1 = "Не причаливает точно, это я "+ GetSexPhrase("узнавал","узнавала") +".";
			link.l1.go = "BlueBird_9";
		break;
		case "BlueBird_9":
			dialog.text = "Ну вот, значит ход моих рассуждений верен... Ты знаешь, на Исла-Тесоро есть подземный ход, ведет из верфи прямо в бухту. В общем, ищи, "+ GetSexPhrase("приятель","подруга") +", да прибудет с тобой сила!";
