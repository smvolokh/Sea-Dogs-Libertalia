//Пират Либерталии 
//Сей диалог предназначен для славных вольных капитанов, которые просиживают свои штанишки в тавернах и с которыми можно всяческими способами взамодействовать. За основу взят файл многоуважемого Jason. За что ему от меня огоромная благодарность. Спасибо)
...
//  Общавочка -->
		...
		case "dialog_pgg":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Приветствую Вас, коллега! Приятно, знаете ли, видеть преуспевающего капитана! Позвольте представиться - " + GetFullName(NPChar) + ". Чем могу быть полезен?";
link.l1 = "Приветствую, капитан! Мое имя - "+GetFullName(pchar)+". У меня к Вам деловое предложение!";
					link.l1.go = "march";
					link.l2 = "О, приветствую Вас! Мое имя - "+GetFullName(pchar)+". Я очень рад нашей встрече. Слыхал я, что Вы бывалый и опытный моряк, не желаете пойти офицером ко мне на корабль? В деньгах не обижу.";
if (FindFreeRandomOfficer() > 0)
			link.l2.go = "officer";
		link.l3 = "Приветствую, коллега! Мое имя - "+GetFullName(pchar)+". Я много слышал о Вас... Судя по слухам, Вы настоящая гроза всего архипелага! Думаю, что если бы мы объединили наши усилия, то вместе мы смогли бы гораздо больше... Что скажите, капитан?";
					link.l3.go = "companion";
link.l4 = "Здравствуйте, сударь! Мое имя - "+GetFullName(pchar)+". Не желаете что-нибудь купить или продать?";
link.l4.go = "Trade_1";
			link.l5 = "Рад встрече, уважаемый! Мое имя - "+GetFullName(pchar)+". Не найдется минутки поболтать?"
					link.l5.go = "question";
			link.l6 = "Приветствую, сударь! Мое имя - "+GetFullName(pchar)+". Не желаете ли скоротать время за игрой?";
			link.l6.go = "Play_Game";
			link.l7 = "Приветствую! Звать меня "+GetFullName(pchar)+". Если позволите, то я присяду. Выпьем по чарке?";
		link.l7.go = "buhach";
		link.l8 = "Опа! Какая встреча! Ну, здравствуй, дорогой. Давненько мы с тобой не виделись. Мое имя - "+GetFullName(pchar)+", помнишь меня, каналья? ";
					link.l8.go = "duel";
		link.l9 = "Привет, приятель! Мое имя - "+GetFullName(pchar)+". Я просто решил поздороваться. Бывай!";
					link.l9.go = "exit";
...
link.l4.go = "Trade_1";
			...
			link.l6.go = "Play_Game";
			link.l7 = "Не хотите немного выпить, коллега?";
		link.l7.go = "buhach";
		link.l8 = "Да, приятель, это снова я. Не желаешь ли пару лишних дырок в груди?";
					link.l8.go = "duel";
		link.l9 = "Я просто решил снова поприветствовать Вас!Удачи!";
					link.l9.go = "exit";
				}
				else
				{
...
// Общавочка <--
		...
		//квестовая часть
			case "march":
				dialog.text = "Деловое предложение говорите... Я Вас понял. Полагаю, что для обсуждения деловых вопросов таверна не самое лучшее место, а поэтому предлагаю продолжить разговор на моём корабле.";
				link.l1 = "Хорошо... Что у вас за корабль и как называется?";
				link.l1.go = "march_1";
			...
			DialogExit();
				...
				npchar.lifeday = 0;				pchar.quest.MarchCapOver.win_condition.l1.date.hour  = sti(GetTime() + 6);
...
case "companion":
		...
		{
			Dialog.Text = RandPhraseSimple("Я Вам уже говорил, что мне это неинтересно.", "Мне казалось, что мы все обсудили.");
...
case "duel_sea":
Dialog.Text = "Что-то не вижу Вашего корабля в порту...";
		link.l1 = "...Тогда я Вас покидаю.";
...
case "duel_otskok":
Dialog.Text = "Мерзавец! Думал соскочить, каналья? Доставай свой клинок, и мы посмотрим, какого цвета твоя кровь!";
		link.l1 = "Ну, если ты так хочешь умереть...";
		link.l1.go = "fight_right_now";
		break;

case "duel_smert":
Dialog.Text = "Искрнее надеюсь, что Вы сегодня умрёте!";
		link.l1 = "Взаимно...";
...
//  Выходы -->	
	...
	//		NPChar.quest.meeting = true;
		...
		case "exit_1":	
			...
			link.l1 = "Говори-говори, да не загаваривайся."; 
			link.l1.go = "exit_duel";
		break;	
