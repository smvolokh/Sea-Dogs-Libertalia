...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
    			dialog.text = "Враг на руднике! Тревога!!";
				link.l1 = "А-ать, дьявол!";
				link.l1.go = "fight"; 
				break;
			}	
			dialog.text = LinkRandPhrase(""+TimeGreeting()+", сударь. Я - "+GetFullName(npchar)+". Я начальник этого рудника и старший горный инженер. Что вам тут нужно?","Здравствуйте, сударь. Что вам угодно?","Хм. Вы что-то хотели от меня, сударь? Я вас слушаю.");
		...
		case "info":
			dialog.text = "Наш рудник добывает золото для "+NationKingsName(npchar)+ ". Все добытое на руднике принадлежит государству, и только ему. Кроме золота, здесь часто встречаются серебряные самородки и изредка - ценные самоцветы. Торговлю золотом мы не ведем - все добытое отправляется под усиленным конвоем в Старый Свет\nОднако насчет золотых и серебряных самородков действует исключение. Нам практически постоянно требуются рабочие руки для труда на руднике, поэтому мы покупаем рабов в обмен на самородки. Так что если у вас есть, что предложить - обращайтесь ко мне, будем договариваться\nКасательно поведения в шахтах, думаю, вам и так должно быть ясно: не пытайтесь что-либо украсть, ибо закончится это для вас плачевно, не отвлекайте каторжников от работы и не путайтесь под ногами у солдат охранения.";
			link.l1 = "Спасибо!";			
			link.l1.go = "exit";
		...
		case "slaves_0":
			...
			dialog.text = "Конечно, сударь! Сколько у вас голов 'живого товара'?";
		...
		case "slaves":
			...
			{
				dialog.text = "Сударь, к сожалению, на данный момент у нас нет нужды в рабах. Но ситуация меняется ежедневно, так что загляните ко мне попозже, через несколько недель, ну, или в другой раз.";
		...
		case "slaves_1":
			...
			link.l2.go = "slaves_s";
			link.l3 = "Извините, сударь, но что-то я передумал. В другой раз.";
		...
		case "slaves_trade":
			...
			if (iTotalTemp < 1)
			{
				dialog.text = "Сударь, я не расположен к глупым шуткам. Паясничать отправляйтесь в таверну!";
				link.l1 = "Гм...";
			...
			{
				dialog.text = "Сударь, мне кажется, вам нужно отдохнуть. Видимо, вы очень устали, или перегрелись на нашем солнце. Сходите в таверну, поспите, а потом продолжим нашу торговлю.";
				link.l1 = "Гм...";
				link.l1.go = "slaves_exit";
				break;
			}
			...
			{
				dialog.text = "К сожалению, сударь, нам в данный момент не нужно так много рабов. Потребность рудника на сегодняшний день составляет "+FindRussianQtyString(sti(location.quest.slaves.qty))+". Будете продавать?";
				link.l1 = "Да, конечно!";
			...
			link.l1 = "Не извольте беспокоиться, сударь. Ваши рабы будут доставлены как можно скорее. Я немедленно отдам нужные распоряжения.";
