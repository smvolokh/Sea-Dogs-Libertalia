	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (CheckAttribute(pchar, "otjig_v_taverne"))
				{	
					link.l22 = "Я хочу устроить для своих ребят отличную пирушку!";
				...
				link.l3.go = "room";
				link.l4 = pcharrepphrase("Есть ли у тебя что-нибудь, чтобы промочить горло?", "Я хотел бы выпить. Что у тебя есть?", "Выпить есть?", "Плесни-ка, приятель, чего-нибудь выпить!");
				link.l4.go = "vipivka_bulbul";				
				Link.l78 = "Увы, я уже ухожу, " + NPChar.name + ". До встречи.";
				Link.l78.go = "exit";
		break;
		
		case "vipivka_bulbul": // Тут собственно списание средств, выставлялка атрибута, что отжиг прошел и запуск видео с пирушки! Rigel
			dialog.text = "У меня есть эль по 2 песо за кружку, ром по 4 песо, вино по 5 песо. Есть еще очень хорошее вино по 6 песо за бокал. Что наливать будем, кэп?";			
			if(makeint(PChar.money)>=2)
			{
				link.l1 = "Давай эль за 2 песо.";
				link.l1.go = "vipivka_el";
			}
			if(makeint(PChar.money)>=4)
			{
				link.l2 = "Давай ром за 4 песо.";
				link.l2.go = "vipivka_rom";	
			}
			if(makeint(PChar.money)>=5)
			{
				link.l3 = "Давай вино за 5 песо.";
				link.l3.go = "vipivka_vino";
			}
			if(makeint(PChar.money)>=6)
			{
				link.l4 = "Давай хорошее вино за 6 песо.";
				link.l4.go = "vipivka_goodvino";
			}
			link.l5 = "А есть что-нибудь безалкогольное?";
			link.l5.go = "vipivka_bulbul2";
			link.l6 = "А целую бутылку ты мог бы мне продать?";
			link.l6.go = "vipivka_butilka";	
		break;		
		
		case "vipivka_bulbul2": // ответвление на безалкогольные напитки
			dialog.text = "Это да! Есть свежевыжатый фруктовый сок по 1 песо за бокал. И вода. Ее могу бесплатно налить.";			
			if(makeint(PChar.money)>=1)
			{
				link.l1 = "Да, сок это то что надо.";
				link.l1.go = "vipivka_sok";
			}
			link.l2 = "Плесни мне воды.";
			link.l2.go = "vipivka_voda";
			link.l3 = "Я передумал. Есть дела поважнее.";
			link.l3.go = "step_node";
		break;				
		
		case "vipivka_el": // ЭЛЬ
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Уххх... Крепкий какой!";
		...
		case "vipivka_rom": // РОМ
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Эхх! Все во рту горит!";
		...
		case "vipivka_vino": // ВИНО
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Кисловато, но сойдет.";
		...
		case "vipivka_goodvino": // ХОРОШЕЕ ВИНО
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Какой терпкий привкус! И послевкусие приятное.";
		...
		case "vipivka_sok": // СОК
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Ммм. Какой вкусный!";
			AddMoneyToCharacter(pchar, -1);
			link.l1.go = "step_node";
		break;			
		
		case "vipivka_voda": // ВОДА
			dialog.text = "Вот, пожалуйста!";			
			link.l1 = "Спасибо. А то жажда мучала.");
			link.l1.go = "step_node";
		break;
		
		case "vipivka_butilka": // Бутылка???
			dialog.text = "Дружище, у меня тут почти все открытое. Да и не продаю я так. Это тебе в лавку надо.";			
			link.l1 = "Хорошо.";
			link.l1.go = "step_node";
		break;

		case "Otjig_v_taverne_stoimost":
			dialog.text = LinkRandPhrase("Прекрасно! Давно мое заведение не гудело, что хороший улей.",
			"Вижу опытного гуляку! Что будете заказывать?",
			"Чудесно! Чем прикажете заставить столы?");
			link.l2 = "Сейчас не лучшее время для этого.";
			link.l2.go = "exit";
			if (sti(Pchar.money) >= GetCrewQuantity(pchar)*150 && GetCrewQuantity(pchar)>0)
			{
				link.l2 = "Давай сделаем скромный стол. Ставьте побольше рому, но не слишком дорогого.";
				link.l2.go = "go_otjig_1";
			}
			if (sti(Pchar.money) >= GetCrewQuantity(pchar)*200 && GetCrewQuantity(pchar)>0)
			{
				link.l3 = "Побольще рому и зажарьте мяса на ребрышках! Я сегодня угощаю!";
				link.l3.go = "go_otjig_2";
			}
			if (sti(Pchar.money) >= GetCrewQuantity(pchar)*250 && GetCrewQuantity(pchar)>0)
			{
				link.l4 = "Вина и рому на столы! Я угощаю сегодня. Я хочу увидеть на столе хорошего кабана!";
				link.l4.go = "go_otjig_3";
			}
	if (sti(Pchar.money) >= GetCrewQuantity(pchar)*300 && GetCrewQuantity(pchar)>0)
			{
				link.l5 = "Ставь свою лучшую выпивку! Пусть столы ломятся от лучших угощений!";
				link.l5.go = "go_otjig_4";
			}
	if (sti(Pchar.money) >= GetCrewQuantity(pchar)*350 && GetCrewQuantity(pchar)>0)
			{
				link.l6 = "Только лучшая выпивка и самая отменная еда! Я хочу, чтобы этому вечеру позавидовали в лучших трактирах Европы!";
				link.l6.go = "go_otjig_5";
			}

			{
				link.l7 = "Пожалуй, я передумал.";
				link.l7.go = "step_node";
			}
		break;

		case "go_otjig_1": // Тут собственно списание средств, выставлялка атрибута, что отжиг прошел и запуск видео с пирушки! Rigel
			dialog.text = LinkRandPhrase("Непременно, капитан! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 150 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*150) + ".",
			"Хорошо! У меня как раз остались старые запасы. Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 150 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*150) + ".",
			"Отлично! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 150 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*150) + ".");
		...
		case "go_otjig_2":
			dialog.text = LinkRandPhrase("Это будет отличный стол, кэп! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 200 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*200) + ".",
			"Как пожелаете! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 200 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*200) + ".",
			"Мне как раз привезли свежего мяса! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 200 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*200) + ".");
			link.l1 = "Хорошо, по рукам!";
		...
		case "go_otjig_3":
			dialog.text = LinkRandPhrase("Кабан будет что надо, капитан! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 250 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*250) + ".",
			"Отлично! Уже накрываем! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 250 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*250) + ".",
			"Вот повезло Вашим ребятам и местным пьяницам! Такого пира они еще не видели! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 250 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*250) + ".");
			link.l1 = "Хорошо, по рукам!";
		...
		case "go_otjig_4":
			dialog.text = LinkRandPhrase("Вот это щедрость! Я поставлю на столы все самое лучшее из своих запасов! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 300 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*300) + ".",
			"Вот это будет пир! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 300 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*300) + ".",
			"Только лучшие угощения! Ром и вино будут течь рекой! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 300 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*300) + ".");
			link.l1 = "Хорошо, по рукам!";
		...
		case "go_otjig_5":
			dialog.text = LinkRandPhrase("Вот это щедрость! Я поставлю на столы все самое лучшее из своих запасов! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 350 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*350) + ".",
			"Эй, вертихвостки! Накрывайте на стол все самое лучшее! Это будет отменный пир! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 350 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*350) + ".",
			"Вот это да! За такие деньги я устрою тут дествительно великий пир! Исходя из того, что на обслуживание каждого Вашего матроса придётся в среднем потратить около 350 песо, с Вас " + FindRussianMoneyString(GetCrewQuantity(pchar)*350) + ".");
		link.l1 = "Хорошо, по рукам!";
...
PlaySound("VOICE\Russian\GOTHIC\GG_CREW\CREW_1.wav");
	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////	
        case "int_quests":
			...
			{
				link.l13 = "Послушай, я ищу своего старого друга, "+pchar.GenQuest.FindFugitive.Name+" его зовут. Один шкипер сказал мне, что он собирался направиться именно в ваше поселение. Не слыхал ничего об этом человеке?";
			...
			if(CheckAttribute(pchar, "questTemp.pirateStartQuest") && pchar.questTemp.pirateStartQuest == "1" && GetArealByCityName(npchar.city) == pchar.questTemp.pirateStartQuest.Areal && npchar.city != pchar.questTemp.pirateStartQuest.City)
			{
				...
				{
					link.l14 = "Слушай, дружище, не подскажешь, у вас здесь никто рабами не приторговывает?";
				...
				else
				{
					...
					{
						link.l14 = "Принес я тебе 1000 пиастров. Говори что знаешь.";
		...
		case "pirateStartQuest_Tavern_1":
			...
			dialog.text = "Хм, ну что ж, может что-то и знаю... Но забесплатно говорить не буду, я благотворительностью не занимаюсь. Моя информация стоит 1000 песо. Плати или уходи.";
			if (sti(pchar.money) >= 1000)
			{
				link.l1 = "Хорошо, вот тебе 1000 песо... Говори что знаешь.";
		...
		dialog.text = "Ну, не знаю. Дело это подсудное, поэтому никто напрямую тебе рабов не предложит. Плантаторы сами скупают, где только найти смогут. Разве что у контрабандистов поспрашивай. Но пираты этот бизнес давно к рукам прибрали, вряд ли кто решит открыто позариться на их монополию...";
			link.l1 = "Ясно. А не знаешь, шхуна 'Грифон' недавно к вам в порт не заходила?";
			link.l1.go = "pirateStartQuest_Tavern_3";
		break;

		case "pirateStartQuest_Tavern_3":
		dialog.text = "'Грифон'? Нет, точно не заходила. Да и название это мне не знакомо.";
		link.l1 = "";
		...
		dialog.text = "";
		link.l1 = "Отлично, мне кажется я понял, спасибо. Ты мне здорово помог. До встречи.";
		link.l1.go = "exit";
