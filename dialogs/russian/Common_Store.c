		...
		case "node_1":
			...
			link.l3 = HeroStringReactionRepeat(NPCharRepPhrase(npchar,
											   ...
											   NPCharRepPhrase(npchar,
												    ...
												    pcharrepphrase("Серебро подождет. Я хочу узнать кое-что.",
...
//navy <--
            ...
            if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)
			{
				...
				// ----------------- квест получения Черной Гаргульи, сдаем квест -----------------
				if (pchar.questTemp.BlueBird == "weWon" && pchar.questTemp.BlueBird.traiderId == npchar.id)
				{
					link.l1 = "Хочу Вас уведомить, что мне удалось справиться с вашим поручением. Бриг ''Черная Горгулья'' не будет более досаждать торговцам. Я прибыл"+ GetSexPhrase("","а") +" за условленным гонораром.";
					link.l1.go = "RBlueBirdWon";
				}
				if (pchar.questTemp.BlueBird == "DieHard" && pchar.questTemp.BlueBird.traiderId == npchar.id)
				{
					link.l1 = "Долж"+ GetSexPhrase("ен","на") +" вам сообщить, что мне не удалось потопить бриг ''Черную Гаргулью''. Я сделал все, что мог...";
				...
				if (pchar.questTemp.BlueBird == "returnMoney" && pchar.questTemp.BlueBird.traiderId == npchar.id && sti(pchar.questTemp.BlueBird.count) > 0)
				{
					...
					{
						link.l1 = "Как ваш флейт поживает? Все нормально?";
				...
				// ----------------- квест получения Черной Гаргульи, продолжение -----------------	
...
// Jason --> -------------------------генератор Сомнительное предложение----------------------------------------
		...
		case "Exit":
			...
			{
				dialog.Text = "Семь футов под килем Вам, капитан!";
				Link.l1 = "Спасибо!";
				Link.l1.go = "exit";
			}
			else
