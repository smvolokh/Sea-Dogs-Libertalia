void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.text = "О, кто к нам пожаловал! Знаменитый корсар Барон "+pchar.name+" собственной персоной!";
			...
			if (CheckAttribute(pchar, "questTemp.Mtraxx.Retribution") && pchar.questTemp.Mtraxx.Retribution == "brothel_repeat" && npchar.location == "Tortuga_SecBrRoom")
			{
				...
				{
					link.l1 = "Конечно! Барон "+pchar.name+" слов на ветер не бросает, в отличие от двух-трех десятков тысяч песо, ха-ха-ха!";
        ...
        case "Horse_talk":
            ...
            //--> Гот. линейка, квест. базар с девокой о Библиии.
		...
		if (pchar.questTemp.GothicLee == "SeekBible_toGirl")
    			...
    			if (npchar.id == "HorseGen_"+FindLocation("FortFrance_Brothel")+"_1")
				{
					if (!CheckAttribute(npchar, "quest.HolLineTalk"))
					{
    					...
    					link.l4 = "Послушай, некоторое время назад у вас тут Торус с компанией развлекались...";
		...
		case "mtraxx_R1":
            dialog.text = "Ах, Барон "+pchar.name+", твой юмор так же остер, как и твоя шпага! Я уже было расстроилась... Девочки! У нас гость, да какой! Откупоривайте вина и несите закуску!";
			if (sti(pchar.money) >= 31000)
			{
...
//********************************* Диалоги девки из борделя ФФ **********************************
		...
		break;
  		case "Gothic_3_2":
			dialog.text = "Было такое, помню... Послушай, красавчик, давай поднимемся наверх и я отвечу на все твои вопросы. Тебе это будет стоить всего 1000 песо...";
			link.l1 = "Ты вообще хоть что-нибудь делаешь бесплатно?";
			link.l1.go = "Gothic_3_3";
		break;
  		case "Gothic_3_3":
			dialog.text = "Хи-хи-хи... Тебе нужна информация - мне нужны деньги. Хочешь, могу дать не только информацию, но и кое-что еще, если поднимемся наверх... Я тебя знаю. Только это будет дороже, хи-хи...";
			link.l1 = "У меня нет времени на глупости.";
		...
		break;
  		case "Gothic_3_4":
            ...
            {
    			link.l1 = "Ничего я не испугался, денег просто нет...";
    			link.l1.go = "exit";
