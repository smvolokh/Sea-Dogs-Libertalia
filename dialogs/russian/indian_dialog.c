...
if (sti(pchar.questTemp.Indian.relation) < 20)
		{
			Dialog.Text = RandPhraseSimple("Враг придти в деревня! К бою!", "Злой бледнолицый придти в деревня! К бою!");
link.l1 = RandPhraseSimple("Вот, чёрт!", "Сейчас я устрою вам геноцид, кранорожие!");
			link.l1.go = "fight";
			break;
			}
...
dialog.text = "Говори, зачем приш"+ GetSexPhrase("ел","ла") +".";
			link.l6 = "Да ничего особенного, просто послушать тебя хотел...";
			link.l6.go = "exit";
			if (CheckCharacterItem(pchar, "slave_01"))
			{
			link.l1 = "У меня есть кое-что для тебя. Хочешь посмотреть?";
			link.l1.go = "man_gift";
			}
...
PlaySound("VOICE\Russian\GOTHIC\GG_HALLO_W\GG_HALLO_W_" + rand(12) + ".wav");
		...
		case "cumvana_2":
			...
			{
				link.l1 = "Подарить бретерский пистоль."; // 5
		...
		case "cumvana_2_1":
			Log_Info("Вы отдали бретерский пистоль");
		...
		case "hayamee_7":
			dialog.text = "О-ей! Тагофа хотеть получить от сын моря два пистоль: один красивый, стрелять пули, второй стрелять мелкие камешки, и заряд, столько, сколько пальцы на две рука три раза, к каждый пистоль.";
		...
		case "hayamee_13":
			...
			LAi_CharacterDisableDialog(npchar);
			SetFunctionTimerCondition("Mtraxx_MeridaHayameeLate", 0, 0, 300, false); // таймер
...
//раз в день.
		...
		{
			Dialog.Text = RandPhraseSimple("Ты мне уже дарить подарок, друг. Ты забыть?", "Ты мне уже показывать подарок, белый. Хватит. Мне нечем отплатить тебе.");
			...
			if (CheckCharacterItem(pchar, "mineral10"))
			{
			link.l1 = "Подарить огниво.";
			link.l1.go = "man_gift_1";
			}
			if (CheckCharacterItem(pchar, "mineral15"))
			{
			link.l2 = "Подарить нож.";
			...
			if (CheckCharacterItem(pchar, "slave_01"))
			{
			link.l4 = "Подарить гарпун.";
			link.l4.go = "man_gift_4";
			}
			if (CheckCharacterItem(pchar, "spyglass2"))
			{
			link.l5 = "Подарить обычную подзорную трубу.";
			link.l5.go = "man_gift_5";
			}
			if (CheckCharacterItem(pchar, "pistol3"))
			{
			link.l6 = "Подарить картечницу.";
			link.l6.go = "man_gift_6";
			}
			if (CheckCharacterItem(pchar, "pistol_01"))
			{
			link.l7 = "Подарить пистоль.";
			link.l7.go = "man_gift_12";
			}
			if (CheckCharacterItem(pchar, "pistol_02"))
			{
			link.l8 = "Подарить колониальный пистолет.";
			link.l8.go = "man_gift_13";
			}
			if (CheckCharacterItem(pchar, "pistol1"))
			{
			link.l9 = "Подарить строевой пистолет.";
			link.l9.go = "man_gift_7";
			}
			if (CheckCharacterItem(pchar, "pistol_03"))
			{
			link.l10 = "Подарить тромбон.";
			link.l10.go = "man_gift_14";
			}
			if (CheckCharacterItem(pchar, "potionrum"))
			{
			link.l11 = "Подарить бутылку рома.";
			link.l11.go = "man_gift_8";
			}
			if (CheckCharacterItem(pchar, "potionwine"))
			{
			link.l12 = "Подарить бутылку вина.";
			link.l12.go = "man_gift_9";
			}
			if (CheckCharacterItem(pchar, "mushket_01"))
			{
			link.l13 = "Подарить карабин.";
			link.l13.go = "man_gift_15";
			}
			if (CheckCharacterItem(pchar, "mushket3"))
			{
			link.l14 = "Подарить аркебузу.";
			link.l14.go = "man_gift_10";
			}
			if (CheckCharacterItem(pchar, "mushket1"))
			{
			link.l15 = "Подарить мушкет.";
			link.l15.go = "man_gift_11";
			}
			link.l16 = "Пожалуй, я передумал.";
			link.l16.go = "exit";
		break;
			...
			PlaySound("interface\important_item.wav");
			GiveItem2Character(pchar, "obereg_2");
			dialog.text = "Ах! Теперь "+npchar.name+" сможет легче добывать огонь! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
		...
		break;
			...
			dialog.text = "Ах! Теперь "+npchar.name+" сможет быстрее освежевывать добычу и снимать скальпы врагам! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
		...
		case "man_gift_3":
			...
			PlaySound("interface\important_item.wav");
			GiveItem2Character(pchar, "obereg_1");
			dialog.text = "Ах! Теперь у "+npchar.name+" есть колдовской труба бледнолицых! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
		...
		break;
			...
			dialog.text = "Ах! Теперь "+npchar.name+" сможет охотиться на акул и китов! Спасибо, добрый чужеземец!Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
	...
	case "man_gift_6":
			...
			dialog.text = "Ах! Теперь у "+npchar.name+" есть малый	 гром палка белых! Теперь "+npchar.name+" сможет быть великий воин! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
	...
	case "man_gift_8":
			...
			dialog.text = "Ах! Теперь у "+npchar.name+" есть огненный вода! У "+npchar.name+" быть много веселье! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
...
case "man_gift_10":
			...
			dialog.text = "Ах! Теперь у "+npchar.name+" есть гром палка белых! Теперь "+npchar.name+" сможет быть великий воин! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
...
case "man_gift_11":
			...
			dialog.text = "Ах! Теперь у "+npchar.name+" есть большой гром палка белых! Теперь "+npchar.name+" сможет быть великий воин! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это помочь тебе в бою!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
...
case "man_gift_15":
			...
			PlaySound("interface\important_item.wav");
			GiveItem2Character(pchar, "obereg_3");
			dialog.text = "Ах! Теперь у "+npchar.name+" есть большой гром палка белых! Теперь "+npchar.name+" сможет быть великий воин! Спасибо, добрый чужеземец! Взять это, чужеземец! Это дар! Это уберечь тебя от злой духи!";
			link.l1 = "Хм, спасибо! Удачи тебе...";
