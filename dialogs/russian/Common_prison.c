...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First_officer":
			    ...
			    {
                dialog.text = "Так-так. Кто тут у нас? Ага, "+GetFullName(pchar)+"! Кажется Вы, уважаемый, находитесь в розыске. И за вашу голову объявлена награда. Какая удача! Взять его, ребята!";
                link.l1 = "Да уж... Это же надо было так опростоволоситься!";
                link.l1.go = "fight"; 
                break;
                }
		dialog.text = RandPhraseSimple("Я начальник тюрьмы. Что вам нужно здесь?", "Что вам нужно? Зачем вы явились в тюрьму?");
			...
			if(pchar.location == "Villemstad_prison" && CheckAttribute(pchar, "questTemp.PrisonEscort_quest") && pchar.questTemp.PrisonEscort_quest == "to_prisone")
			{
				...
				dialog.text = "Приношу свои извинения, " + GetSexPhrase("минхер","госпожа") + "! Мои люди спутали вас с одним человеком. Вот ослы! Ведь вы ни капли не похожи на Сантоса Де Пайва!!!";
				link.l14 = "Ну, знаете ли! У меня дел хватает, а я вынужд" + GetSexPhrase("ен","на") + " тратить свое время на разбирательства обстоятельств, к которым, как оказалось, не имею ни малейшего отношения!!! Надеюсь, инцидент исчерпан и я могу быть свобод" + GetSexPhrase("ен","на") + "?";
			...
			if(pchar.location == "Villemstad_prison" && CheckAttribute(pchar, "questTemp.PrisonEscort_quest"))
			{
				if(pchar.questTemp.PrisonEscort_quest == "sink_ship" || pchar.questTemp.PrisonEscort_quest == "capture_ship")
				{
					...
					dialog.text = "Надеюсь, у вас есть чем меня порадовать...";
					link.l1 = "Да, кое что у меня для вас есть.";
