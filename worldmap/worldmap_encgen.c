...
void Map_TraderSucces_quest(string sChar)
{
	...
	//поиски фрегата Черная Жемчужина
	if (sChar == "Avrora" && characters[GetCharacterIndex(sChar)].quest == "InMap")
	{
		SetAvroraFromMapToSea();
		Log_TestInfo("Энканутер кэпа фрегата Queen дошел до места назначения.");
	}
