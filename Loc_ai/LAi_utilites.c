	...
	if (checkAttribute(loc, "citizens") || loc.type == "church")
	{
		if(iNation != PIRATE || sti(Colonies[iColony].HeroOwn) == true)//колония
		{
			...
			for(i=0; i<iCapQty; i++)//капитаны кораблей
			{
				...
				if (rand(10) < 1 && !CheckAttribute(pchar, "GenQuest.MarchCap") && !CheckAttribute(pchar, "questTemp.Sharlie.Lock") && chr.City != "Panama") //пойдем на дельце patch-5
				{
			...
			for(i=0; i<iCitizQty; i++)//мещане-бюргеры + посетители церкви
			{
				...
				if(chr.sex == "man")
				{
										...
										float Mft = MOD_SKILL_ENEMY_RATE/20;											chr.MultiFighter = 2.5; // мультифайтер
			chr.MultiShooter = 2.5;
									}
...
void CreateIslatesoroEng(aref loc)//Jason, Исла Тесоро при провале Саги
{
	if (loc.id == "Pirates_town" && CheckAttribute(loc, "EngBase"))
	{
		...
		for (i=1; i<=4; i++)
		{
			sld = GetCharacter(NPC_GenerateCharacter("Eng_IT_Prot_"+i, "shturm_eng_"+(rand(5)+1), "man", "man", iRank, ENGLAND, 1, false, "soldier"));
		...
		for (i=1; i<=10; i++)
		{
			if (i < 8)
			{
				sld = GetCharacter(NPC_GenerateCharacter("Eng_IT_Sold_"+i, "shturm_eng_"+(rand(5)+1), "man", "man", iRank, ENGLAND, 1, false, "soldier"));
				SetFantomParamFromRank(sld, iRank, true);
			}
			else
			{
				sld = GetCharacter(NPC_GenerateCharacter("Eng_IT_Sold_"+i, "shturm_mush_eng_"+(rand(4)+1), "man", "mushketer", iRank, ENGLAND, 1, false, "soldier"));
...
void CreateHabitues(aref loc)
{
	...
	if (!isLocationHasCitizens(loc.id))  // boal  если есть еще с того раза, но не нужно
	{
		if (CheckAttribute(loc, "fastreload"))
		{
			...
			if (CheckAttribute(pchar, "CSM.RelationAgent") && iNation == PIRATE && sti(Colonies[iColony].HeroOwn) != true && colonies[iColony] != "SanAndres")
			{
				iChar = NPC_GeneratePhantomCharacter("citizen", iNation, MAN, 1);
				chr = &characters[iChar];
				characters[iChar].model = "Albermal";
				characters[iChar].model.animation = "man";
				chr.City = Colonies[iColony].id;
				chr.FaceId = 958;
				GiveItem2Character(chr, "pistol5");
				chr.equip.gun = "pistol5";
				GiveItem2Character(chr, "newblade12");
				chr.equip.blade = "newblade12";
				chr.CityType = "citizen";
				sTemp = PlaceCharacter(chr, "sit", "random_free"); // может не быть вовсе, если все места заняты
				ReSitCharacterOnFree(chr, loc.id, sTemp);
				LAi_SetSitType(chr);
				...
				chr.dialog.currentnode = "first time";
				chr.greeting = "Voice\Russian\Rel_m_b_00" + (1 + rand(2)) + ".wav";
				LAi_group_MoveCharacter(chr, slai_group);
			}
			// CSM <--
			// офицеры -->
			...
			if (rand(15) == 3 || CheckAttribute(pchar, "CSM.TreasureMapTrader")) // --> CheatSurfMenu by Cheatsurfer
			{ // карты кладов
...
void CreateMaltains(aref loc)//Jason, заполнение базы мальтийцев в Форт де Франсе
{	
	...
	if (CheckAttribute(loc, "Maltains"))
	{
		...
		if (!CheckAttribute(loc, "Maltains_date") || GetNpcQuestPastDayParam(loc, "Maltains_date") >= 2)
		{
			...
			//офицер
			sld = GetCharacter(NPC_GenerateCharacter("Maltie_officer", "off_Malt_1", "man", "man", 35, FRANCE, 2, false, "officer"));
			FantomMakeCoolFighter(sld, sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 100, 100, "blade_36", "pistol5", "bullet", 150);//спецназ
			sld.greeting = "patrol";
			...
			for (i=1; i<=4; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter("MaltainsPro_"+i, "Malta_"+(rand(7)+1), "man", "man", 25, FRANCE, 2, false, "soldier"));
				FantomMakeCoolFighter(sld, sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 90, 90, "newblade17", "pistol_08", "grapeshot", 100);//спецназ
				sld.greeting = "protector";
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
			...
			for (i=1; i<=4; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter("MaltainsSit_"+i, "Malta_"+(rand(7)+1), "man", "man", 20, FRANCE, 2, false, "soldier"));
				FantomMakeCoolFighter(sld, sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 70, 70, "newblade17", "pistol_08", "grapeshot", 100);//спецназ
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
				sld.dialog.currentnode = "Maltie_soldier";	
				sld.greeting = "soldier";
			...
			for (i=1; i<=5; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter("MaltainsStay_"+i, "Malta_"+(rand(7)+1), "man", "man", 20, FRANCE, 2, false, "soldier"));
				FantomMakeCoolFighter(sld, sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 70, 70, "newblade12", "pistol_08", "grapeshot", 100);//спецназ
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
				sld.dialog.currentnode = "Maltie_soldier";	
				sld.greeting = "soldier";
				LAi_LoginInCaptureTown(sld, true);
			...
			for (i=1; i<=5; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter("MaltainsGo_"+i, "Malta_"+(rand(7)+1), "man", "man", 20, FRANCE, 2, false, "soldier"));
				FantomMakeCoolFighter(sld, sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 70, 70, "newblade12", "pistol_08", "grapeshot", 100);//спецназ
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
				sld.dialog.currentnode = "Maltie_soldier";	
				sld.greeting = "soldier";
...
{	
	...
	if (CheckAttribute(loc, "HWIC"))
	{
		...
		if (!CheckAttribute(loc, "HWIC_date") || GetNpcQuestPastDayParam(loc, "HWIC_date") >= 30)
		{
			...
			for (i=1; i<=6; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter("HWIC_sold_"+i, "guard_hol_"+(rand(7)+1), "man", "man", 25, HOLLAND, 30, false, "officer"));
...
{	
	...
	if (CheckAttribute(loc, "Incquisitio"))
	{
		...
		if (CheckNPCQuestDate(loc, "Guard_date"))
		{
			...
			for (i=1; i<=4; i++)
			{
			sld = GetCharacter(NPC_GenerateCharacter("IncqGuard_"+i, "Paladin_"+(rand(10)+1), "man", "man2", 35, SPAIN, 1, true, "officer"));
		...
		if (!CheckAttribute(loc, "Mayak_date") || GetNpcQuestPastDayParam(loc, "Mayak_date") > 2)
		{
			...
			if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.soldiers"))
			{
				...
				for (i=0; i<iCitizensQuantity; i++)
				{
					...
					{		
						iChar = NPC_GeneratePhantomCharacter("fort_soldier", iNation, MAN, 2);
						chr = &characters[iChar];
					}
					else
					{
						chr = GetCharacter(NPC_GenerateCharacter("GenChar_", "shturm_mush_eng_1", "man", "mushketer", sti(pchar.rank), iNation, 2, false, "soldier"));
						chr.id = "GenChar_" + chr.index;
			...
			while (qty < qtyAll)
			{
				...
				if (horse[num] != "")
				{
					...
					switch (location.fastreload)
					{
						...
						case "VillaAlegria": 	sld.quest.price = 400 + 40*(rand(5) + rand(5)); 	break;
						case "Panama":   		sld.quest.price = 500 + 40*(rand(5) + rand(5)); 	break;
						case "FortOrange":   		sld.quest.price = 550 + 40*(rand(5) + rand(5)); 	break;
						case "Roseau":   		sld.quest.price = 600 + 40*(rand(5) + rand(5)); 	break;
						case "LeKe":   		sld.quest.price = 600 + 40*(rand(5) + rand(5)); 	break;
						case "PuertoPrincipe":   		sld.quest.price = 550 + 40*(rand(5) + rand(5)); 	break;
						case "LeFransua":   		sld.quest.price = 550 + 40*(rand(5) + rand(5)); 	break;
						case "PortAntonio":  	sld.quest.price = 50 + 40*(rand(5) + rand(5)); 		break;		
					}
...
void CreateMinentownMine(aref loc) // Jason: шахта на руднике
{
	if (loc.id == "Minentown_mine")
	{
		if (!CheckAttribute(loc, "mine_date") || GetNpcQuestPastDayParam(loc, "mine_date") > 30)
		{
			...
			{
				chr = GetCharacter(NPC_GenerateCharacter("MineSoldier_"+i, "shturm_spa_"+(rand(7)+1), "man", "man", iRank+5, SPAIN, 30, true, "soldier"));
			...
			{
				chr = GetCharacter(NPC_GenerateCharacter("MineProtector_"+i, "shturm_spa_"+(rand(7)+1), "man", "man", iRank+10, SPAIN, 30, true, "soldier"));
...
void CreateMineBandits(aref loc) // Jason: бандиты у рудника у Блуфилдса
{
	ref chr;
	int i;
	int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
...
void CreateResidenceNpc(aref loc)
{
	...
	case 2:
		...
		sModel = "guard_"+NationShortName(iNation)+"_"+(rand(7)+1);
		sSex = "man";
		sAni = "man"
		bWeapon = true;
		sNode = "ResGuard";
	...
	chr = GetCharacter(NPC_GenerateCharacter(sName, sModel, sSex, sAni, Rank, iNation, 1, bWeapon, "officer"));
...
void CreateVillaAlegriaRudnik(aref loc) // наполнение рудника Вилья Аллегрия
{
	if (loc.id == "VillaAlegria_Rudnik")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
	...
	if (CheckNPCQuestDate(loc, "VillaAlegria_Rudnik_date"))
		{
			SetNPCQuestDate(loc, "VillaAlegria_Rudnik_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
			slai_group = GetNationNameByType(iNation)  + "_citizens";
			nShortName = NationShortName(iNation);
			...
			if (iRank < 10) iRank = 10;
			if (iRank > 20) iRank = 20;
			for (i=1; i<=12; i++) // бродячие солдаты
			{
			chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikSold_"+i, "shturm_"+nShortName+"_"+(rand(7)+1), "man", "man", iRank, iNation, 2, true, "soldier"));
				chr.City = loc.parent_colony;
				chr.CityType = "soldier";
				chr.rank = iRank;
		chr.dialog.filename = "Plantation_dialog.c";
		chr.dialog.currentnode = "rudnik_soldier";
				LAi_SetLoginTime(chr, 0.0, 24.0);
				chr.greeting = "soldier";
	LAi_CharacterReincarnation(chr, true, true);
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
ChangeCharacterAddressGroup(chr, loc.id, "goto", "goto"+i);
				LAi_SetCitizenType(chr);
			LAi_group_MoveCharacter(chr, slai_group);
			}
			for (i=1; i<=7; i++) //бродячие мушкетёры
			{
			chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikMush_"+i, "shturm_mush_"+nShortName+"_"+(rand(2)+1), "man", "mushketer", iRank, iNation, 2, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+5, true);
				chr.City = loc.parent_colony;
				chr.CityType = "soldier";
				chr.rank = iRank;
		...
		chr.dialog.currentnode = "rudnik_soldier";
				...
				GiveItem2Character(chr, "blade_12");
				EquipCharacterbyItem(chr, "blade_12");
...
ChangeCharacterAddressGroup(chr, loc.id, "goto", "goto"+i);
			...
			for (i=1; i<=1; i++) // стоячие солдаты
			{
			chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikDef_"+i, "shturm_"+nShortName+"_"+(rand(7)+1), "man", "man", iRank, iNation, 2, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "Rudnik_protector"; 
			chr.protector = true;
			chr.greeting = "protector";
	LAi_CharacterReincarnation(chr, true, true);
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.City = loc.parent_colony;
				chr.CityType = "soldier";
ChangeCharacterAddressGroup(chr, loc.id, "merchant", "goto22");
			LAi_SetGuardianType(chr);
			LAi_group_MoveCharacter(chr, slai_group);
			}
			for (i=1; i<=1; i++) // стоячие солдаты
			{
			chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikProt_"+i, "shturm_"+nShortName+"_"+(rand(7)+1), "man", "man", iRank, iNation, 2, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+5, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "Rudnik_protector"; 
			chr.protector = true;
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
			chr.City = loc.parent_colony;
				chr.CityType = "soldier";
ChangeCharacterAddressGroup(chr, loc.id, "merchant", "goto23");
			LAi_SetGuardianType(chr);
			LAi_group_MoveCharacter(chr, slai_group);
			}
			for (i=1; i<=7; i++) // зеки
			{
				chr = GetCharacter(NPC_GenerateCharacter("RudnikZek_"+i, "Prison_"+(rand(4)+1), "man", "man2", iRank+10, PIRATE, 30, true, "slave"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.dialog.filename = "Plantation_dialog.c";
	...
	LAi_group_MoveCharacter(chr, slai_group);
			...
			for (i=1; i<=7; i++) // рабы
			{
			chr = GetCharacter(NPC_GenerateCharacter("RudnikSlave_"+i, "prizon_"+(rand(7)+1), "man", "man2", iRank+10, PIRATE, 30, true, "slave"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "plantation_slave";
			chr.greeting = "convict";
...
void CreateVillaAlegriaRudnikResidence(aref loc) // наполнение рудника Вилья Аллегрия
{
	if (loc.id == "VillaAlegria_Rudnik_Residence")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
...
if (CheckNPCQuestDate(loc, "VillaAlegria_Rudnik_Residence_date"))
		{
SetNPCQuestDate(loc, "VillaAlegria_Rudnik_Residence_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
			slai_group = GetNationNameByType(iNation)  + "_citizens";
			nShortName = NationShortName(iNation);
			...
			if (iRank < 10) iRank = 10;
			if (iRank > 20) iRank = 20;
			for (i=1; i<=1; i++) // начальник
			{
			chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikHead_"+i, "off_"+nShortName+"_"+(rand(2)+1), "man", "man", iRank, iNation, 2, true, "officer"));
				chr.City = loc.parent_colony;
				chr.CityType = "soldier";
				chr.rank = iRank;
		chr.dialog.Filename = "rudnikHead_dialog.c";
		chr.greeting = "comandante";
	LAi_CharacterReincarnation(chr, true, true);
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
ChangeCharacterAddressGroup(chr, loc.id, "sit", "sit1");
	chr.standUp = true; //вставать и нападать на врага
	LAi_SetLoginTime(chr, 0.0, 24.0);
...
void CreateVillaAlegriamines(aref loc) // наполнение рудника Вилья Аллегрия
{
	if (loc.id == "VillaAlegria_mines")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
...
if (CheckNPCQuestDate(loc, "VillaAlegria_mines_date"))
		{
SetNPCQuestDate(loc, "VillaAlegria_mines_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
	slai_group = GetNationNameByType(iNation)  + "_citizens";
nShortName = NationShortName(iNation);
			...
			if (iRank < 10) iRank = 10;
			if (iRank > 20) iRank = 20;
			for (i=1; i<=15; i++) // каторжники
			{
				chr = GetCharacter(NPC_GenerateCharacter("RudnikMineConvict_"+i, "prizon_"+(rand(7)+1), "man", "man_B", iRank, PIRATE, 30, true, "slave"));
				SetFantomParamFromRank(chr, iRank, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "plantation_slave";
				chr.greeting = "convict";
			...
			for (i=1; i<=15; i++) // каторжники
			{
				chr = GetCharacter(NPC_GenerateCharacter("RudnikMineZek_"+i, "prison_"+(rand(4)+1), "man", "man_B", iRank, PIRATE, 30, true, "slave"));
				SetFantomParamFromRank(chr, iRank, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "plantation_slave";
				chr.greeting = "convict";
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
		chr.City = loc.parent_colony;
			...
			for (i=1; i<=4; i++) // протекторы
			{
chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikProtector_"+i, "shturm_"+nShortName+"_"+(rand(7)+1), "man", "man", iRank, iNation, 2, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.dialog.filename = "Plantation_dialog.c";
			chr.dialog.currentnode = "Rudnik_protector"; 
			chr.protector = true;
...
ChangeCharacterAddressGroup(chr, loc.id, "soldiers", "soldier"+i);
		...
		chr = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"RudnikMineHead_"+i, "off_"+nShortName+"_"+(rand(2)+1), "man", "man", iRank, iNation, 2, true, "officer"));
		chr.City = loc.parent_colony;
		chr.CityType = "soldier";
		chr.rank = iRank;
		chr.dialog.filename = "Plantation_dialog.c";
...
void CreatePortAntonioRudnik(aref loc) // наполнение рудника Форта Оранж
{
	if (loc.id == "PortAntonio_Rudnik")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
	...
	if (CheckNPCQuestDate(loc, "PortAntonio_Rudnik_date"))
		{
			SetNPCQuestDate(loc, "PortAntonio_Rudnik_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
			slai_group = GetNationNameByType(iNation)  + "_citizens";
			nShortName = NationShortName(iNation);
...
void CreatePortAntonioRudnikResidence(aref loc) // наполнение рудника Форта Оранж
{
	if (loc.id == "PortAntonio_Rudnik_Residence")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
...
if (CheckNPCQuestDate(loc, "PortAntonio_Rudnik_Residence_date"))
		{
SetNPCQuestDate(loc, "PortAntonio_Rudnik_Residence_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
			slai_group = GetNationNameByType(iNation)  + "_citizens";
			nShortName = NationShortName(iNation);
...
void CreatePortAntoniomines(aref loc) // наполнение рудника Форта Оранж
{
	if (loc.id == "PortAntonio_mines")
	{
	ref chr;
		string LocatorGroup, LocatorName, slai_group, nShortName;
		int i, nSit, iNation, iRank;			
...
if (CheckNPCQuestDate(loc, "PortAntonio_mines_date"))
		{
SetNPCQuestDate(loc, "PortAntonio_mines_date");
			LAi_group_Delete("Prisoner_Group"); 
			iNation = GetCityNation(loc.parent_colony);
	slai_group = GetNationNameByType(iNation)  + "_citizens";
nShortName = NationShortName(iNation);
...
void CreateSanAndres(aref loc) // наполнение Сан Андреас
{
	if (loc.id == "SanAndres_town")
	{
		if (!CheckAttribute(loc, "SanAndres_town_date") || GetNpcQuestPastDayParam(loc, "SanAndres_town_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "SanAndres_town_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;

			for (i=1; i<=2; i++) // бродячие мергены
			{	                                   
				chr = GetCharacter(NPC_GenerateCharacter("SantLousSoldier_"+i, "Apostates_"+(rand(16)+1), "man", "man2", iRank+5, PIRATE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
				chr.dialog.filename    = "Gothic_Quest\npc\Captain.c";
				chr.greeting = "SantLous_man";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "SanAndres_town";
				chr.CityType = "citizen";
				LAi_SetLoginTime(chr, 6.0, 21.99);
				PlaceCharacter(chr, "goto", "random");
				LAi_SetPatrolType(chr);
				LAi_group_MoveCharacter(chr, "FRANCE_CITIZENS");
			}
			for (i=1; i<=5; i++) // телки
			{
				chr = GetCharacter(NPC_GenerateCharacter("SantLousgirl_"+i, "Womens_"+(rand(18)+1), "woman", "towngirl", iRank+10, PIRATE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+10, true);
		chr.dialog.Filename = "Gothic_Quest\npc\Noblegirl.c";
				chr.greeting = "SantLous_woman";
				chr.CityType = "citizen";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "SanAndres_town";
				LAi_SetLoginTime(chr, 10.0, 21.99);
				ChangeCharacterAddressGroup(chr, loc.id, "goto", "goto"+i);
				LAi_SetCitizenType(chr);
				LAi_group_MoveCharacter(chr, "FRANCE_CITIZENS");
			}	
...
void CreateSanAndres_2(aref loc) // наполнение Сан Андреас
{
	if (loc.id == "SanAndres_town_2")
	{
		if (!CheckAttribute(loc, "SanAndres_town_2_date") || GetNpcQuestPastDayParam(loc, "SanAndres_town_2_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "SanAndres_town_2_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;
...
void CreateSanAndres_3(aref loc) // наполнение Сан Андреас
{
	if (loc.id == "SanAndres_town_3")
	{
		if (!CheckAttribute(loc, "SanAndres_town_3_date") || GetNpcQuestPastDayParam(loc, "SanAndres_town_3_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "SanAndres_town_3_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;
...
void CreateSanAndres_4(aref loc) // наполнение Сан Андреас
{
	if (loc.id == "SanAndres_town_4")
	{
		if (!CheckAttribute(loc, "SanAndres_town_4_date") || GetNpcQuestPastDayParam(loc, "SanAndres_town_4_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "SanAndres_town_4_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;
...
void CreateAmmo(aref loc) // Jason, оружейная
{
	if (loc.type == "ammo")
	{
		...
		if (!CheckAttribute(loc, "Ammo_date") || GetNpcQuestPastDayParam(loc, "Ammo_date") > 2)
		{
			...
			for (i=1; i<=3; i++)
			{
				sld = GetCharacter(NPC_GenerateCharacter(loc.parent_colony+"AmmoSold_"+i, "shturm_"+NationShortName(iNation)+"_"+(rand(7)+1), "man", "man", iRank, iNation, 2, true, "soldier"));
...
void CreateVilla(aref loc) // Имение Губернатора
{
	if (loc.id == "Villa")
	{
		if (!CheckAttribute(loc, "Villa_date") || GetNpcQuestPastDayParam(loc, "Villa_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "Villa_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;

			for (i=1; i<=7; i++) // бродячие офицеры
			{
				chr = GetCharacter(NPC_GenerateCharacter("VillaSoldier_"+i, "off_fra_"+(rand(5)+1), "man", "man", iRank+5, FRANCE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
				chr.dialog.Filename = "Gothic_Quest\VillaHead_dialog.c";
				chr.greeting = "patrol";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "Villa";
			...
			for (i=1; i<=5; i++) // леди
			{
				chr = GetCharacter(NPC_GenerateCharacter("VillaNoblegirl_"+i, "Women_"+(rand(5)+1), "woman", "woman", iRank+10, FRANCE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+10, true);
				chr.dialog.Filename = "Population\Noblegirl.c";
				chr.greeting = "noble_female";
				chr.CityType = "citizen";
		...
		// главный клерк, статик
		sld = GetCharacter(NPC_GenerateCharacter("Villa_headclerk", "Diplomate", "man", "man2", 20, FRANCE, -1, false, "citizen"));
		SetFantomParamFromRank(sld, 20, true);
		sld.name = "Камиль";
		sld.lastname = "Пузетье";
		sld.Dialog.Filename = "Gothic_Quest\Office_Rosea_1.c";
		...
		ChangeCharacterAddressGroup(sld, "EstatetOffice1", "sit", "sit1");
		LAi_group_MoveCharacter(sld, "FRANCE_CITIZENS");
		// 230912 торгует
		sld.Merchant = true;
		sld.Merchant.type = "company";
...
void CreateLeKe(aref loc) // наполнение Ле-Ке
{
	if (loc.id == "LeKe_town")
	{
		if (!CheckAttribute(loc, "LeKe_town_date") || GetNpcQuestPastDayParam(loc, "LeKe_town_date") >= 2)
		{
			SaveCurrentNpcQuestDateParam(loc, "LeKe_town_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;

			for (i=1; i<=10; i++) // бродячие киллеры
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKeSoldier_"+i, "Bandit_" + (rand(6) + 1), "man", "man", iRank+5, PIRATE, 30, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+15, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";	
			chr.greeting = "town_pirate";
	LAi_CharacterReincarnation(chr, true, true);
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
		...
		LAi_group_MoveCharacter(chr, "PIRATE_CITIZENS");
			...
			for (i=1; i<=10; i++) // бродячие пираты
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKePirate_"+i, "sold_pir_" + (rand(7) + 1), "man", "man", iRank+5, PIRATE, 30, true, "marginal"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";	
			chr.greeting = "town_pirate";
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
		...
		LAi_group_MoveCharacter(chr, "PIRATE_CITIZENS");
			...
			for (i=1; i<=5; i++) //бродячие киллеры-мушкетёры
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKemush_"+i, "Bandit_mush_" + (rand(6) + 1), "man", "mushketer", iRank+5, PIRATE, 30, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+15, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";	
			chr.greeting = "town_pirate";
			chr.CanTakeMushket = true;
			chr.SuperShooter  = true;								LAi_CharacterReincarnation(chr, true, true);
			chr.equip.gun = "mushket";
				SetCharacterPerk(chr, "Energaiser"); 
				SetCharacterPerk(chr, "BasicDefense"); 
				SetCharacterPerk(chr, "AdvancedDefense"); 
			SetCharacterPerk(chr, "ProfessionalDefense");
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
			...
			for (i=1; i<=5; i++) // бродячие пираты-мушкетёры
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKemushpir_"+i, "mush_ctz_"+(rand(2)+7), "man", "mushketer", iRank+5, PIRATE, 30, true, "soldier"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";		
			chr.greeting = "town_pirate";
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
		...
		LAi_group_MoveCharacter(chr, "PIRATE_CITIZENS");
			...
			for (i=1; i<=6; i++) // стоячие пираты
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKeProtector_"+i, "sold_pir_" + (rand(7) + 1), "man", "man", iRank+5, PIRATE, 30, true, "marginal"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";	
			chr.greeting = "town_pirate";
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
			...
			for (i=1; i<=4; i++) // стоячие пираты
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKeDefender_"+i, "sold_pir_" + (rand(7) + 1), "man", "man", iRank+5, PIRATE, 30, true, "marginal"));
			SetFantomParamFromRank(chr, iRank+10, true);
			chr.Dialog.Filename = "Population\Townman.c";
			chr.dialog.currentnode = "First time";	
			chr.greeting = "town_pirate";
	...
	LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
			...
			for (i=1; i<=5; i++) // шлюхи
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKeWomen_"+i, "Horse0"+(rand(7)+1), "woman", "woman", iRank+10, PIRATE, 30, true, "citizen"));
			SetFantomParamFromRank(chr, iRank, true);
			chr.dialog.Filename = "Population\Noblegirl.c";
			chr.greeting = "SantLous_woman";
			chr.CityType = "citizen";
...
ChangeCharacterAddressGroup(chr, loc.id, "goto", "goto"+i);
		...
		LAi_group_MoveCharacter(chr, "PIRATE_CITIZENS");
			...
			for (i=1; i<=5; i++) // леди
			{
				chr = GetCharacter(NPC_GenerateCharacter("LeKegirl_"+i, "Women_"+(rand(5)+1), "woman", "woman", iRank+10, PIRATE, 30, true, "citizen"));
			SetFantomParamFromRank(chr, iRank, true);
			chr.dialog.Filename = "Population\Noblegirl.c";
			chr.greeting = "SantLous_woman";
			chr.CityType = "citizen";
...
void CreateSantLous(aref loc) // деревушка Сент-Луи
{
	if (loc.id == "SantLous_town")
	{
		if (!CheckAttribute(loc, "SantLous_town_date") || GetNpcQuestPastDayParam(loc, "SantLous_town_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "SantLous_town_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;

			for (i=1; i<=5; i++) // бродячие мергены
			{	                                   
				chr = GetCharacter(NPC_GenerateCharacter("SantLousSoldier_"+i, "mercen_"+(rand(19)+1), "man", "man", iRank+5, FRANCE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
				chr.dialog.filename    = "Gothic_Quest\npc\Captain.c";
				chr.greeting = "SantLous_man";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "SantLous_town";
				chr.CityType = "citizen";
				LAi_SetLoginTime(chr, 6.0, 23.99);
				PlaceCharacter(chr, "goto", "random");
				LAi_SetPatrolType(chr);
				LAi_group_MoveCharacter(chr, "FRANCE_CITIZENS");
			}
			for (i=1; i<=5; i++) // телки
			{
				chr = GetCharacter(NPC_GenerateCharacter("SantLousgirl_"+i, "Womens_"+(rand(18)+1), "woman", "towngirl", iRank+10, FRANCE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+10, true);
		chr.dialog.Filename = "Gothic_Quest\npc\Noblegirl.c";
				chr.greeting = "SantLous_woman";
				chr.CityType = "citizen";
...
void CreateFishingSettlement(aref loc) // деревушка рыбаков
{
	if (loc.id == "Fishing_Settlement_town")
	{
		if (!CheckAttribute(loc, "Fishing_Settlement_town_date") || GetNpcQuestPastDayParam(loc, "Fishing_Settlement_town_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "Fishing_Settlement_town_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;
			...
			for (i=1; i<=5; i++) // бродячие мергены
			{
				chr = GetCharacter(NPC_GenerateCharacter("FishingSettlementSoldier_"+i, "mercen_"+(rand(19)+1), "man", "man", iRank+5, HOLLAND, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
				chr.dialog.filename    = "Gothic_Quest\npc\Captain.c";
				chr.greeting = "SantLous_man";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "Fishing_Settlement_town";
				chr.CityType = "citizen";
				LAi_SetLoginTime(chr, 6.0, 23.99);
				PlaceCharacter(chr, "goto", "random");
				LAi_SetPatrolType(chr);
				LAi_group_MoveCharacter(chr, "HOLLAND_CITIZENS");
			}
			for (i=1; i<=5; i++) // телки
			{
				chr = GetCharacter(NPC_GenerateCharacter("FishingSettlementgirl_"+i, "Womens_"+(rand(18)+1), "woman", "towngirl", iRank+10, HOLLAND, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+10, true);
		chr.dialog.Filename = "Gothic_Quest\npc\Noblegirl.c";
				chr.greeting = "SantLous_woman";
				chr.CityType = "citizen";
...
void CreateRoseau(aref loc) // Розо
{
	if (loc.id == "Roseau_town")
	{
		if (!CheckAttribute(loc, "Roseau_town_date") || GetNpcQuestPastDayParam(loc, "Roseau_town_date") > 30)
		{
			SaveCurrentNpcQuestDateParam(loc, "Roseau_town_date");
			int i;
			int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
			ref chr;
			...
			for (i=1; i<=5; i++) // бродячие мергены
			{
				chr = GetCharacter(NPC_GenerateCharacter("RoseauSoldier_"+i, "mercen_"+(rand(19)+1), "man", "man", iRank+5, PIRATE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+5, true);
				chr.dialog.filename    = "Gothic_Quest\npc\Captain.c";
				chr.greeting = "Roseau_man";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "Roseau_town";
				chr.CityType = "citizen";
				LAi_SetLoginTime(chr, 6.0, 23.99);
				PlaceCharacter(chr, "goto", "random");
				LAi_SetPatrolType(chr);
			...
			for (i=1; i<=5; i++) // телки
			{
				chr = GetCharacter(NPC_GenerateCharacter("Roseaugirl_"+i, "Womens_"+(rand(19)+1), "woman", "towngirl", iRank+10, PIRATE, 30, true, "soldier"));
				SetFantomParamFromRank(chr, iRank+10, true);
		                                          chr.dialog.Filename = "Gothic_Quest\npc\Noblegirl.c";
				chr.greeting = "Roseau_woman";
				chr.CityType = "citizen";
				LAi_CharacterReincarnation(chr, true, true);
				LAi_SetReincarnationRankStep(chr, MOD_SKILL_ENEMY_RATE+2); 
				chr.city = "Roseau_town";
...
//наполнение жемчужных поселений
void CreateDestinyGiftPearlVillage(aref loc)
{
	if (CheckAttribute(loc, "DestinyGiftpearlVillage") && sti(loc.DestinyGiftpearlVillage) == true)
	{	
		...
		int iTemp = GetCharacterIndex("DestinyGiftPearlGuardMan_"+iPrefix);
		if (iTemp != -1 && !CheckAttribute(&characters[iTemp], "quest.TalkOk")) 
		{//если не завалил часового
			if (characters[iTemp].location == "none")
			{
				loc.DestinyGiftpearlVillage = false;
				DeleteAttribute(&locations[FindLocation("DestinyGiftPearlTown"+iPrefix+"_Townhall")], "box1");
				DeleteAttribute(&locations[FindLocation("DestinyGiftPearlTown"+iPrefix+"_Townhall")], "box2");
		...
		DoQuestCheckDelay("DestinyGiftPearl_GuardManIsLife", 3.5);
				                                         pchar.quest.DestinyGiftPearl_GuardManIsLife.idx = iTemp;//индекс часового передаем
				chrDisableReloadToLocation = true;			
			}
		}
		else 
		{//часовой убит
			//LAi_group_SetRelation("DestinyGiftPearlGroup_"+iPrefix, LAI_GROUP_PLAYER, LAI_GROUP_FRIEND);
		}
		ref chr;
		int num, i, iMassive, iRank;
		string sAnime;
		string model[10];
		loc.DestinyGiftPearlVillage = false; //флаг не генерить пиплов
		// ==> глава администрации
		iRank = 30+rand(15);
		chr = GetCharacter(NPC_GenerateCharacter("HeadMan_"+iPrefix, "HeadManPearl", "man", "man", iRank, iPearNation, 30, true, "officer"));
		SetCharacterPerk(chr, "Energaiser"); // скрытый перк дает 1.5 к приросту энергии, дается ГГ и боссам уровней  
		chr.DontClearDead = true;
		chr.SaveItemsForDead = true;
		chr.FaceId = 403;
		chr.SuperShooter  = true;
		AddItems(chr, "gunpowder", Rand(20) + 5);
				SetCharacterPerk(chr, "Energaiser"); 
				SetCharacterPerk(chr, "BasicDefense"); 
				SetCharacterPerk(chr, "AdvancedDefense"); 
			SetCharacterPerk(chr, "ProfessionalDefense");
			...
			SetCharacterPerk(chr, "SwordplayProfessional");
				...
				SetCharacterPerk(chr, "Grus"); 
				SetCharacterPerk(chr, "CriticalHit");
		chr.MultiFighter = 2.0; // мультифайтер
		chr.MultiShooter = 2.0;

		chr.dialog.Filename = "Gothic_Quest\Pearl_dialog.c";
	                     chr.dialog.currentnode = "HeadMan";		
		chr.city = "SantaCatalina"; //НЗГ Санта-Каталины
		SetFantomParamFromRank(chr, iRank, true);
		LAi_SetWarriorType(chr);
		LAi_group_MoveCharacter(chr, "DestinyGiftPearlGroup_"+iPrefix);
        sTemp = "DestinyGiftPearlTown"+iPrefix+"_Townhall"; //ID резиденции
        ...
        pchar.GenQuestBox.(sTemp).box1.items.jewelry52 = rand(1000) + rand(1000);
        pchar.GenQuestBox.(sTemp).box1.items.jewelry53 = rand(2500) + rand(2500);
		pchar.GenQuestBox.(sTemp).box2.items.DestinyGiftPearl = rand(333) + rand(333);
		pchar.GenQuestBox.(sTemp).box2.items.gold_dublon = rand(1000) + rand(1000);
		pchar.GenQuestBox.(sTemp).box2.money = 120000 + rand(100000);
		...
		model[9] = "citiz_54";
		i = 0;
		num = rand(4) + 4; //кол-во защитников
		while(i < num)
		{
			iMassive = rand(9);
			if (model[iMassive] != "")
			{
				chr = GetCharacter(NPC_GenerateCharacter("FightMan"+iPrefix+"_"+i, model[iMassive], "man", "man", 15, iPearNation, 30, true, "soldier"));
			SetFantomParamFromRank(chr, 20+rand(15), true);
		chr.dialog.Filename = "Gothic_Quest\Pearl_dialog.c";
		...
		while(i < num)
		{
			...
			if (model[iMassive] != "")
			{
				...
				chr = GetCharacter(NPC_GenerateCharacter("WorkMan"+iPrefix+"_"+i, model[iMassive], "man", sAnime, 7, iPearNation, 30, false, "quest"));
				chr.dialog.Filename = "Gothic_Quest\Pearl_dialog.c";
				chr.dialog.currentnode = "PearlMan";
		...
		//=========================== квесты в поселениях ================================
		//--> перс Алекса
		if (loc.id == "DestinyGiftPearl_town_1" && !CheckAttribute(pchar, "GenQuest.GhostShip.LastBattle"))
		{
			chr = GetCharacter(NPC_GenerateCharacter("JohnDavy", "QuestMan1", "man", "man", 10, iPearNation, -1, false, "quest")); // постоянный перец, чтоб помнил реплики, второй раз он не создается, а просто ищется
			chr.dialog.Filename = "Gothic_Quest\GhostShip_dialog.c";
			chr.name = "Белый";
		...
		//<-- перс Алекса
		//--> главарь грабителей
		if (loc.id == "DestinyGiftPearl_town_1" && sti(pchar.questTemp.tugs.berglarState) > 6)
		{
			pchar.quest.Berglars_Ostin.win_condition.l1 = "location";
			pchar.quest.Berglars_Ostin.win_condition.l1.location = "DestinyGiftPearlTown1_House6";
		...
		//<-- главарь грабителей
		//=========================== квесты в поселениях ================================
		LAi_group_SetLookRadius("DestinyGiftPearlGroup_"+iPrefix, 16);
		LAi_group_SetHearRadius("DestinyGiftPearlGroup_"+iPrefix, 10);
	}
}

// Зомби - скелеты
void CreateRuinsZombie(aref loc)
{	
	if (CheckAttribute(loc, "RuinsZombie"))
	{
		ref sld;
		int i;
		int n = sti(loc.RuinsZombie);
		string sZmb = loc.id;
		string sModel = "Zombie_"+(rand(4)+1);
		string sBlade = LinkRandPhrase("blade_10","blade_06","blade_04");
		string sPistol = LinkRandPhrase("pistol6","pistol1","pistol5");
		int iRank = sti(pchar.rank)+MOD_SKILL_ENEMY_RATE+8;
		if (iRank > 56) iRank = 56;
		...
		if (!CheckAttribute(loc, "_Ruins_date") || GetNpcQuestPastDayParam(loc, "_Ruins_date") >= 2)
		{
			SaveCurrentNpcQuestDateParam(loc, "_Ruins_date");
			for (i=1; i<=n; i++)
			{
			sld = GetCharacter(NPC_GenerateCharacter(sZmb+"_Zombie_"+i, sModel, "Zombie", "Zombie", iRank, PIRATE, 2, false, "soldier"));

				FantomMakeCoolFighter(sld, iRank, iScl, iScl, sBlade, sPistol, "bullet", iScl*2+100);
			sld.MultiFighter = 1.5; // мультифайтер
			sld.MultiShooter = 1.5;
				ChangeCharacterAddressGroup(sld, sZmb, "goto", "goto"+i);
				LAi_SetWarriorType(sld);
				LAi_warrior_SetStay(sld, true);
				LAi_CharacterDisableDialog(sld);
				LAi_group_MoveCharacter(sld, LAI_GROUP_MONSTERS);
...
// Обезьяны - гоблины
void CreateGrottoMonkey(aref loc)
{	
	if (CheckAttribute(loc, "GrottoMonkey"))
	{
		ref sld;
		int i;
		int n = sti(loc.GrottoMonkey);
		string sMnk = loc.id;
		string sModel = "Monkey_"+(rand(2)+1);
	                     int iRank = 16+MOD_SKILL_ENEMY_RATE*2;
		PlaySound("People Fight\GOB_HAPPY.wav");
		if (!CheckAttribute(loc, "_Grotto_date") || GetNpcQuestPastDayParam(loc, "_Grotto_date") >= 2)
		{
			SaveCurrentNpcQuestDateParam(loc, "_Grotto_date");
			for (i=1; i<=n; i++)
			{
			sld = GetCharacter(NPC_GenerateCharacter(sMnk+"_Monkey_"+i, sModel, "Monkey", "Monkey", iRank, PIRATE, 0, true, "quest"));

	                                                               sld.NotRemoved = true;
				ChangeCharacterAddressGroup(sld, sMnk, "goto", "goto"+i);
				LAi_SetWarriorType(sld);
				LAi_CharacterDisableDialog(sld);
				LAi_group_MoveCharacter(sld, LAI_GROUP_MONSTERS);
			}
