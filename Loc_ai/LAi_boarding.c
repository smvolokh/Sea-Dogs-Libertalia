...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
	...
	if (CheckOfficersPerk(echr, "MusketsShoot") && IsFort == false) // для противника
	{
		...
		if (CheckAttribute(pchar, "CSM.BoardingCrewGodMode"))
		{
			...
			Log_SetStringToLog("Мушкетным залпом противника, чудесным образом, не убито ни одного человека нашей команды!");
		}
		else
		{
...
{        
    ...
    else
    {
		...
		int deadCrew    = makeint((boarding_player_base_crew - crew) / fDefenceSkill + 0.3 + AIShip_isPerksUse(CheckOfficersPerk(mchar, "Doctor3"), 0.0, 0.7)); // бонус выжившим
...
void LAi_SetBoardingActors(string locID)
{
	...
	{ // не грузим матросов в каюту
		for(i = LAi_numloginedcharacters; i < limit; i++)
		{
			...
			// --> CheatSurfMenu by Cheatsurfer
			// if (LAi_IsImmortal(mchr)) // --> лучше сделаем независимо от ГГ
			if (CheckAttribute(pchar, "CSM.BoardingCrewGodMode"))
			{
...
if (CheckAttribute(rCharacter, "GenQuest.CrewGuardMode"))
 ...
 if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) <= 1) //матросы с патентом 
        {
...
if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) > 4) //гвардейская форма форма только со звания адмирал
        {
            atr = "guardModel.enemy";
            iNation = sti(Items[sti(rCharacter.EquipedPatentId)].Nation);
		}
if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) <= 4) //форма до звания командор
        {
...
string LAi_GetBoardingMushketerModel(ref rCharacter)
{
	...
	if(sti(rCharacter.index) == GetMainCharacterIndex()) // для ГГ
	{
		...
		// by Sticksъ <--
		//<-- форма солдат на квестовиках
        if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) <= 4) //если есть патент, оставлю, хотя рудимент, форма только со звания капитан
        {
        iNation = sti(Items[sti(rCharacter.EquipedPatentId)].Nation);
		model = "shturm_mush_"+NationShortName(iNation)+"_"+(rand(2)+1); // три модели
        }
		...
		case "guard": 		
				...
				case "catalan": 	model = "mush_Catalan_"+(rand(2)+1); break; // каталонцы
				case "Malta": 	model = "mush_Malta_"+(rand(2)+1); break; // мальтийцы
				case "VaterSkel": 	model = "VaterSkel_" + (rand(3) + 1) + "_mush"; break; // рыбьи хари
				case "skel":        model = "skel" + rand(4) + "_mush"; break; // скелетоны
		...
		else // если не прописан - выставляем согласно нации
        {
			...
			else model = "shturm_mush_"+NationShortName(iNation)+"_"+(rand(2)+1); // 3 модели
        }
	}
	return model;
}
