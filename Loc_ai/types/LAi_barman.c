...
void LAi_type_barman_SetAfterGoto(aref chr)
{
	...
	case "bar1":
		...
		if(rand(100) < 30)
		{
			if(chr.sex!="woman") LAi_CharacterPlaySound(chr, "barman_do");
			else LAi_CharacterPlaySound(chr, "barman_f_do");
		}
		isSet = true;
		break;
	case "bar2":
		...
		if(rand(100) < 30)
		{
			if(chr.sex!="woman") LAi_CharacterPlaySound(chr, "barman_wtrs");
			else LAi_CharacterPlaySound(chr, "barman_f_wtrs");
		}
		isSet = true;
		break;
	}
