...
void LAi_tmpl_ani_PlayAnimation(aref chr, string actionName, float timeout)
{
	...
	if(sti(chr.index) == GetMainCharacterIndex()) // Для горного дела
	{
		if(CheckAttribute(pchar.questTemp, "mining_ft") && pchar.questTemp.mining_ft == "true")
		{
			pchar.questTemp.mining_ft = "false";
			chr.chr_ai.tmpl.animation = "frosting";
		}
		else
		{
	...
	if(actionName == "ore_go") SendMessage(chr, "lslss", MSG_CHARACTER_EX_MSG, "TieItem", 10, "HandsItems\topor_015", "Saber_hand");
