...
CreateJail(location);
CreatePortAntonioRudnik(location);
CreatePortAntonioRudnikResidence(location);
CreatePortAntoniomines(location);
CreateVillaAlegriaRudnik(location);
CreateVillaAlegriaRudnikResidence(location);
CreateVillaAlegriamines(location);
...
CreateItzaLand(location);
		...
		//дополнение
	 CreateDestinyGiftPearlVillage(location); // Ловцы Жемчуга
	CreateLostShipsCityy(location);//по ГПК2
	CreatUnderwaterr(location); //по ГПК2
	CreatTenochtitlann(location);//теночтитлан
	CreatTenochtitlannInside(location);//теночтитлан
	CreatDesMoines(location);//зачарованный город
	CreateVaterSkelLand(location);// Рыбьи Хари	
	CreateIrokuLand(location);// ирокезы	
	CreateZombieLand(location);// Зомби	
	CreateMonkeyLand(location);// Мартышки	
	CreateOrcLand(location);// Орки, ненавижу орков!
	CreateAztecLand(location);// Ацтеки, несчастный народ
	CreateVilla(location);// Вилла Пуанси
	CreateSantLous(location);// Сент-Луи 
	CreateLeKe(location);// Ле-Ке
	CreateFishingSettlement(location);// Рыбаки
	CreateSanAndres(location);//Сан Андреас
	CreateSanAndres_2(location);//Сан Андреас
	CreateSanAndres_3(location);//Сан Андреас
	CreateSanAndres_4(location);//Сан Андреас
	CreateRoseau(location);//Розо
LAi_GreateCrabsToShore(location); //R // Lugger: крабы в бухтах
LAi_CreateFlowerscrypt(location); // Jason: выращиваем травку
