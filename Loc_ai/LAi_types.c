...
// Шахтёр
#include "Loc_ai\types\LAi_mining.c"
//Работающий
#include "Loc_ai\types\LAi_staywork.c"
...
//Установить персонажу тип добычи руды
void LAi_SetMiningType(aref chr)
{
	chr.chr_ai.type = LAI_DEFAULT_TYPE;
	LAi_type_mining_Init(chr);
	LAi_group_MoveCharacter(chr, LAI_DEFAULT_GROUP);
}

//Установить персонажу тип добычи руды, без перемещения в группу (only man)
void LAi_SetMiningTypeNoGroup(aref chr)
{
	chr.chr_ai.type = LAI_DEFAULT_TYPE;
	LAi_type_mining_Init(chr);
}
