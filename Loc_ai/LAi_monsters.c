...
//////////////////////////////////////////////////////////////////////////////// 
//R © Lugger: крабы в бухтах //// void SetCrabsToShore(ref loc) 
///////////////////////////////////////////////////////////////////////////////// 
...
//				sld.Crab = true; 
				sld.animal = true;
				sld.name = "Краб"; 
				sld.lastname = "Пляжный Обыкновенный"; 
...
{// выращивание травки: раз в 10 дней при заходе в локацию вырастает новый экземпляр - садовник Jason
	...
	{
		int n = 1;
		if(location.id.label == "Graveyard" && !CheckAttribute(location, "fastreload")) n = 2;// на кладбище выращиваем травку
		for(int i = 0; i < n; i++)
		{
			rItm = ItemsFromID("cannabis"+(rand(5)+1));
			rItm.shown = true;
...
bool LAi_CreateQuestFlowers(ref location) // Jason рассада квестовой травки
{
	...
	if (CheckAttribute(location, "fennisgrass"))
	{
		...
		//Log_TestInfo("В этой локации произрастает болотная трава");// patch
...
bool LAi_CreateEncounters(ref location)
{
	...
	ref squadLeader; // --> [SticksMod] -  будем ставить толпу вокруг главаря, если таковой сгенерится
	...
	switch (iRand)
	{
		...
		case 0:
			...
			if (CheckAttribute(pchar, "CSM.MoreBands")) {
				if (rand(10) > 8) return false; // меньше вероятность скипов
			} else {
				if (rand(10) > 6) return false; // оригинал
			}
			// <-- [SticksMod]
			...
			// --> [SticksMod] - если нет костра, делаем больше бандюг в толпе
			...
			while(i < num)
			{
				...
				if (model[iMassive] != "")
				{
					...
					else
					{
						...
						{
							if (i == 0)
								locator = GetAttributeName(GetAttributeN(grp, i)); // локаторов может быть меньше, получим крашь
						}
						else
						{
			...
			// --> [SticksMod] // для мода этот триггер лишний.
		...
		case 4:
			...
			for(i = 0; i < num; i++)
			{
				...
				if (CheckAttribute(pchar, "CSM.MoreBands"))
				{
					...
					if (i == 0) {
						squadLeader = chr;
						// Патруль сам бежит проверять документы гг, не отвяжется
						DoQuestFunctionDelay("SM_HideWeapon", SM_GetRunTime(pchar, chr)); 
				...
				if(model[iMassive] != "")
				{
					...
					if(i == 0)
					{
						...
						if (CheckAttribute(pchar, "CSM.MoreBands")) { // --> [SticksMod]
							squadLeader = chr; // главюк
						} // <-- [SticksMod]
					}
					else
					{
						if (CheckAttribute(pchar, "CSM.MoreBands")) { // --> [SticksMod]
							SM_SeparateNPC(squadLeader, chr, i, num); // распихиваем толпу вокруг него
						} // <-- [SticksMod]
						LAi_ActorFollow(chr, pchar, "", -1);
					}
		...
		case 7:
			...
			if(rand(4) == 1 && CheckAttribute(location, "onUninhabitedIsland") && !CheckAttribute(location, "deadlocked") && !CheckAttribute(PChar, "GenQuest.PiratesOnUninhabited") && !CheckAttribute(pchar, "GenQuest.ShipWreck") && reload_cur_island_index > -1)
			{
				...
				// --> [SticksMod] - в диалоге очепятка может быть - ароду спавится 8, а подвезти просят 5..
				if (CheckAttribute(pchar, "CSM.MoreBands")) {
					num = 5 + rand(4);
				}
				// <-- [SticksMod]
...
bool LAi_CreateCrabsToShore(aref loc) // Злобные крабы в бухтах
{
	if(loc.type != "seashore" && loc.type != "mayak") return;
	{
			...
			for (int i=1; i<=5; i++)
			{
				...
				sld = GetCharacter(NPC_GenerateCharacter("FantomShoreCrab_"+i, "crabSmall", "crab", "crabSmall", 5+(MOD_SKILL_ENEMY_RATE), PIRATE, -1, false, "quest"));
                                LAi_CharacterDisableDialog(sld);
                                LAi_SetCitizenTypeNoGroup(sld);                                                                               sld.SaveItemsForDead = true; // сохранять на трупе вещи
                                sld.Crab = true;
                                sld.name = "Краб";
                                sld.lastname = "Пляжный Обыкновенный";
                                AddItems(sld, "crab_jaw", 1);
...
void CreateVaterSkelLand(aref loc) // Рыбьи Хари
{
...
void CreateZombieLand(aref loc) // Зомби
{
if(CheckAttribute(loc, "ZombieLand") && !bBettaTestMode)
	{
		...
		//определим ранг воинов. обработаем от сложности, но не менее 20 уровня
		warriorRank = GetCoffDiff(sti(pchar.rank), 1000) + MOD_SKILL_ENEMY_RATE;
		if (warriorRank < 20) warriorRank = 20;
...
void CreateOrcLand(aref loc) //Орки
{
