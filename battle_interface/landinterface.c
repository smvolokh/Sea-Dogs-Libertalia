...
void BLI_SetObjectData()
{
	...
	if(nLoc >= 0) {
		...
		if(nFile >= 0) {
			objLandInterface.textinfo.islandname.font = "MLG_228";
			objLandInterface.textinfo.islandname.scale = 0.99;
			objLandInterface.textinfo.islandname.pos.x = sti(showWindow.right) - RecalculateHIcon(130);
			objLandInterface.textinfo.islandname.pos.y = RecalculateVIcon(13);
			...
			//
			objLandInterface.textinfo.villagename.font = "MLG_228";
			objLandInterface.textinfo.villagename.scale = 0.98;
			objLandInterface.textinfo.villagename.pos.x = sti(showWindow.right) - RecalculateHIcon(140);
			objLandInterface.textinfo.villagename.pos.y = RecalculateVIcon(40);

			objLandInterface.textinfo.locationname.font = "MLG_228";
			objLandInterface.textinfo.locationname.scale = 0.98;
			objLandInterface.textinfo.locationname.pos.x = sti(showWindow.right) - RecalculateHIcon(140);
			objLandInterface.textinfo.locationname.pos.y = RecalculateVIcon(67);
	...
	objLandInterface.textinfo.datatext.font = "MLG_228";
	objLandInterface.textinfo.datatext.scale = 0.99;
	objLandInterface.textinfo.datatext.pos.x = sti(showWindow.right) - RecalculateHIcon(138);
	objLandInterface.textinfo.datatext.pos.y = RecalculateVIcon(95);
	RefreshLandTime();
	...
	//
	/*времени в игре нет
	objLandInterface.textinfo.timetext.font = "MLG_228";
	objLandInterface.textinfo.timetext.scale = 0.9;
...
void BLI_SetPossibleCommands()
{
	...
	if(chrMode==0)
	{
		...
		if (!CheckAttribute(pchar, "CSM.FastTravelEnemy")) // --> CheatSurfMenu
		{
