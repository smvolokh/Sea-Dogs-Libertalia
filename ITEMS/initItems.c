...
//==================PATENTS
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Кулаки
	n = InitStdBlade(n,  "unarmed", "ITEMS_40",  12,   TGT_QUEST, 		B_POOR, "FencingL",17, 1.00, 1.00, 0,  0, 0, 		  1); // кулаки Jason: нельзя кулаки в группу янки - индеи и каторжники получаются без оружия
...
//int InitStdBlade(индекс, модель, текстура, картинка, целевая группа, качество, тип клинка, номер в группе, длина, изгиб, генерируемый, кол-во генерируемых, генерабельная цена, цена если не генерируемый);
	n = InitStdBlade(n, "knife_01", "ITEMS_16",  7,    TGT_QUEST, 		B_POOR, "FencingL", 9, 0.50, 1.00, 0,  0, 0, 	   1000); // дага
		n = InitStdBlade(n, "blade48", "ITEMS_43", 14,    TGT_QUEST,  B_ORDINARY, "FencingL", 14, 0.95, 1.00, 0, 0, 0, 		  28000); // Укротитель нежити
	n = InitStdBlade(n, "slave_01", "ITEMS_51",  4,   TGT_YANKEE, 		B_POOR, "FencingL", 0, 0.70, 1.00, 0,  0, 0, 		200); // гарпун 
	n = InitStdBlade(n, "blade_03", "ITEMS_51",  6, TGT_MARGINAL,   B_POOR, "FencingL", 0, 1.05, 1.00, 1, 10, 1, 		  0); // Стокката
	n = InitStdBlade(n, "blade_27", "ITEMS_51",  12,    TGT_QUEST,       B_EXCELLENT, "FencingL", 2, 1.05, 1.00, 1, 15, 1, 		  0); // Саксенфедер
	n = InitStdBlade(n, "blade_01", "ITEMS_51",  5,   TGT_NATIVE,   B_POOR, "FencingL", 0, 0.90, 1.05, 1, 10, 1, 		  0); // Нааб-Те
	n = InitStdBlade(n, "blade_25", "ITEMS_51", 16,    TGT_QUEST,  B_EXCELLENT, "FencingL", 0, 1.05, 1.10, 1, 15, 1, 		  0); // Моргана
	n = InitStdBlade(n, "blade_26", "ITEMS_51", 15,    TGT_QUEST,  B_EXCELLENT, "FencingL", 1, 1.15, 1.00, 1, 15, 1, 		  0); // Асоледа
	n = InitStdBlade(n, "blade_08", "ITEMS_51",  8,  TGT_SOLDIER,   B_ORDINARY, "FencingL", 0, 1.10, 1.10, 1, 10, 1, 		  0); // Бильбо
	n = InitStdBlade(n, "blade_09", "ITEMS_51",  7,  TGT_SOLDIER,   B_ORDINARY, "FencingL", 1, 0.95, 1.00, 1, 10, 1, 		  0); // Карцолета
	n = InitStdBlade(n, "blade_15", "ITEMS_51",  10,  TGT_OFFICER,       B_GOOD, "FencingL", 0, 0.90, 1.05, 1, 15, 1, 		  0); // Маринера
	n = InitStdBlade(n, "blade_16", "ITEMS_51",  9,  TGT_OFFICER,       B_GOOD, "FencingL", 1, 0.95, 1.00, 1, 15, 1, 		  0); // Смоллсворд
	n = InitStdBlade(n, "blade_18", "ITEMS_51", 13,  TGT_OFFICER,  B_EXCELLENT, "FencingL", 3, 1.05, 1.05, 1, 15, 1, 		  0); // Паппенхаймер	
	n = InitStdBlade(n, "blade_22", "ITEMS_51",  11,  TGT_PERSIAN,       B_GOOD, "FencingL", 0, 1.00, 1.10, 1, 10, 1, 		  0); // Гнев Пророка
	n = InitStdBlade(n, "blade42", "ITEMS_43", 15,    TGT_QUEST,  B_ORDINARY, "FencingL", 14, 0.95, 1.10, 0, 0, 0, 		  40000); // Ярость Иннаса
	n = InitStdBlade(n, "blade46", "ITEMS_43", 10,  TGT_QUEST,  B_POOR, "FencingL", 13, 1.05, 1.10, 0, 0, 0, 		  30000); // Власть Рун
	n = InitStdBlade(n, "blade37", "ITEMS_54", 13,    TGT_QUEST,  B_ORDINARY, "FencingL", 1, 0.85, 1.15, 0, 0, 0, 		  41000); // Катана самурая
	n = InitStdBlade(n, "blade39", "ITEMS_54", 15,    TGT_QUEST,  B_POOR, "FencingL", 1, 1.00, 1.05, 0, 0, 0, 		  32000); // Военная катана
	n = InitStdBlade(n, "newblade11", "ITEMS_54",  7,    TGT_MARGINAL, 	B_POOR, "FencingL", 18, 0.45, 1.00, 0,  0, 0, 500); // Кинжал
	n = InitStdBlade(n, "newblade13", "ITEMS_11",  1,    TGT_OFFICER,       B_GOOD, "FencingL", 2, 1.05, 1.00, 1, 15, 1, 		  0); // Нобиль
	n = InitStdBlade(n, "newblade18", "ITEMS_15",  1,    TGT_QUEST,  B_EXCELLENT, "FencingL", 18, 0.60, 1.00, 1,  15, 1, 	   0); // Катар
	n = InitStdBlade(n, "newblade20", "ITEMS_15", 14,  TGT_OFFICER,  B_EXCELLENT, "FencingL", 2, 1.00, 1.00, 1, 15, 1, 		  0); // Акокий
	n = InitStdBlade(n, "blade_17", "ITEMS_51", 14,  TGT_OFFICER,  B_EXCELLENT, "FencingL", 2, 1.00, 1.00, 1, 15, 1, 		  0); // Бретта
	n = InitStdBlade(n, "newblade8", "ITEMS_15",  4,  TGT_SOLDIER,   B_ORDINARY, "FencingL", 0, 1.15, 1.00, 1, 10, 1, 		  0); // Сарагосса
	n = InitStdBlade(n, "newblade2", "ITEMS_16",  8, TGT_MARGINAL,   B_POOR, "FencingL", 0, 1.10, 1.10, 1, 10, 1, 		  0); // Венецианская шпага
	n = InitStdBlade(n, "q_blade_18", "ITEMS_16", 14,  TGT_QUEST,  B_EXCELLENT, "FencingL", 13, 1.20, 1.00, 0, 0, 0, 	  70000); // Шпага
	n = InitStdBlade(n, "q_blade_16", "ITEMS_15",  13,  TGT_QUEST,       B_EXCELLENT, "FencingL", 12, 1.00, 1.00, 0, 0, 0, 		  65000); // Рапира

// сабли и тесаки
//int InitStdBlade(индекс, модель, текстура, картинка, целевая группа, качество, тип клинка, номер в группе, длина, изгиб, генерируемый, кол-во генерируемых, генерабельная цена, цена если не генерируемый);
	n = InitStdBlade(n, "slave_02", "ITEMS_52", 1,   TGT_YANKEE,       B_POOR, "FencingS", 1, 0.60, 1.20, 1, 10, 1, 		  0); // мачете 
	n = InitStdBlade(n, "blade_02", "ITEMS_52", 2,   TGT_NATIVE,   B_ORDINARY, "FencingS", 1, 1.05, 0.95, 1, 10, 1, 		  0); // Тлакомакан
	n = InitStdBlade(n, "blade_04", "ITEMS_52",  6, TGT_MARGINAL,       B_ORDINARY, "FencingS", 1, 1.00, 1.00, 1, 15, 1, 		  0); // Госсемесер
	n = InitStdBlade(n, "blade_05", "ITEMS_52", 3, TGT_MARGINAL,   B_ORDINARY, "FencingS", 2, 1.00, 1.05, 1, 10, 1, 		  0); // Катлас
	n = InitStdBlade(n, "blade_10", "ITEMS_52",  7,  TGT_SOLDIER,      B_ORDINARY, "FencingS", 2, 0.90, 1.10, 1, 15, 1, 		  0); // Корд
	n = InitStdBlade(n, "blade_11", "ITEMS_52",  5,  TGT_SOLDIER,   B_ORDINARY, "FencingS", 3, 0.85, 1.10, 1, 10, 1, 		  0); // Абордажный тесак
	n = InitStdBlade(n, "blade_12", "ITEMS_52",  4,  TGT_SOLDIER,   B_ORDINARY, "FencingS", 4, 0.90, 1.10, 1, 10, 1, 		  0); // Полусабля
	n = InitStdBlade(n, "blade_19", "ITEMS_52",  11,  TGT_OFFICER,  B_EXCELLENT, "FencingS", 5, 1.00, 1.15, 1, 15, 1,  	      0); // Морская сабля
	n = InitStdBlade(n, "blade_20", "ITEMS_52",  10,  TGT_OFFICER,  B_EXCELLENT, "FencingS", 4, 0.90, 1.15, 1, 15, 1, 		  0); // Мадонна
	n = InitStdBlade(n, "blade_23", "ITEMS_52",  9,  TGT_PERSIAN,       B_EXCELLENT, "FencingS", 1, 1.05, 1.10, 1, 10, 1, 		  0); // Килидж
	n = InitStdBlade(n, "blade_28", "ITEMS_52", 13,    TGT_QUEST,  B_EXCELLENT, "FencingS", 3, 1.00, 1.10, 1, 15, 1, 		  0); // Катана
	n = InitStdBlade(n, "blade_29", "ITEMS_52",  12,    TGT_QUEST,  B_EXCELLENT, "FencingS", 4, 1.05, 1.10, 1, 15, 1, 		  0); // Сторта
	n = InitStdBlade(n, "blade_30", "ITEMS_52",  8,    TGT_QUEST,      B_EXCELLENT, "FencingS", 5, 0.95, 1.15, 1, 15, 1, 		  0); // Офицерский клеванг
	n = InitStdBlade(n, "blade38", "ITEMS_54", 14,    TGT_QUEST,  B_GOOD, "FencingS", 2, 1.05, 1.10, 0, 0, 0, 		  50000); // Катана японского наемника
	n = InitStdBlade(n, "blade41", "ITEMS_43", 13,    TGT_QUEST,  B_GOOD, "FencingS", 14, 0.90, 1.05, 0, 0, 0, 		  48000); // Рудный Боевой Клинок
	n = InitStdBlade(n, "blade49", "ITEMS_43", 16,    TGT_QUEST,  B_GOOD, "FencingS", 14, 0.75, 1.10, 0, 0, 0, 		  52000); // Слово Берсеркера 
	n = InitStdBlade(n, "blade50", "ITEMS_44", 1,    TGT_MARGINAL,  B_ORDINARY, "FencingS", 14, 0.90, 1.20, 1, 10, 1, 		  0); // Скимитар 
	n = InitStdBlade(n, "blade51", "ITEMS_44", 2,    TGT_QUEST,  B_GOOD, "FencingS", 14, 1.05, 1.10, 0, 0, 0, 		  51000); // Черное Сердце 
	n = InitStdBlade(n, "blade52", "ITEMS_44", 3,    TGT_QUEST,  B_GOOD, "FencingS", 14, 1.05, 1.00, 0, 0, 0, 		  49000); // Святой Исполнитель 
	n = InitStdBlade(n, "newblade1", "ITEMS_16",  9,  TGT_MARGINAL,   B_POOR, "FencingS", 3, 0.90, 1.05, 1, 10, 1, 		  0); // Канонирский тесак
	n = InitStdBlade(n, "newblade3", "ITEMS_16", 6, TGT_MARGINAL,   B_POOR, "FencingS", 2, 1.00, 1.05, 1, 20, 1, 		  0); // Кайман
	n = InitStdBlade(n, "newblade4", "ITEMS_16",  5,  TGT_SOLDIER,   B_POOR, "FencingS", 4, 0.95, 1.10, 1, 10, 1, 		  0); // Пехотная сабля
	n = InitStdBlade(n, "newblade6", "ITEMS_15", 11,  TGT_MARGINAL,  B_ORDINARY, "FencingS", 2, 0.90, 1.10, 1, 15, 1, 		  0); // Висельник
		n = InitStdBlade(n, "newblade10", "ITEMS_44",  13,  TGT_SOLDIER,   B_ORDINARY, "FencingS", 4, 0.95, 1.05, 1, 10, 1, 		  0); // Кавалерийская сабля
	n = InitStdBlade(n, "newblade16", "ITEMS_15",  8,    TGT_SOLDIER,      B_EXCELLENT, "FencingS", 5, 0.85, 1.15, 1, 15, 1, 		  0); // Кракемарт
	n = InitStdBlade(n, "newblade17", "ITEMS_16",  13,  TGT_OFFICER,  B_EXCELLENT, "FencingS", 4, 0.90, 1.15, 1, 15, 1, 		  0); // Карабела
		n = InitStdBlade(n, "newblade23", "ITEMS_54",  6,  TGT_MARGINAL,  B_POOR, "FencingS", 4, 0.85, 1.05, 1, 15, 1, 		  0); // Дюсак
		n = InitStdBlade(n, "newblade24", "ITEMS_44",  15,  TGT_OFFICER,  B_GOOD, "FencingS", 4, 1.00, 1.10, 1, 15, 1, 		  0); // Голландская сабля
		n = InitStdBlade(n, "newblade25", "ITEMS_54",  16,  TGT_OFFICER,  B_GOOD, "FencingS", 4, 1.00, 1.15, 1, 15, 1, 		  0); // Испанская сабля
		n = InitStdBlade(n, "newblade26", "ITEMS_8",  1,  TGT_MARGINAL,  B_GOOD, "FencingS", 4, 0.85, 1.20, 1, 15, 1, 		  0); // Ятаган
		n = InitStdBlade(n, "newblade27", "ITEMS_54",  9,  TGT_SOLDIER,  B_ORDINARY, "FencingS", 4, 0.90, 1.10, 1, 15, 1, 		  0); // Малкус
	n = InitStdBlade(n, "newblade28", "ITEMS_6",  1,  TGT_MARGINAL,  B_GOOD, "FencingS", 5, 1.00, 1.20, 1, 15, 1,  	      0); // Самзир
	n = InitStdBlade(n, "q_blade_10", "ITEMS_11",  2,  TGT_QUEST,       B_EXCELLENT, "FencingS", 10, 1.00, 1.20, 0, 0, 0, 		  65000); // Шамшир
	n = InitStdBlade(n, "q_blade_19", "ITEMS_11",  16,  TGT_QUEST,  B_EXCELLENT, "FencingS", 14, 1.00, 1.20, 0, 0, 0, 	  70000); // Шашка	

// топоры и палицы+сельхоз орудия
//int InitStdBlade(индекс, модель, текстура, картинка, целевая группа, качество, тип клинка, номер в группе, длина, изгиб, генерируемый, кол-во генерируемых, генерабельная цена, цена если не генерируемый);
	n = InitStdBlade(n, "topor_01", "ITEMS_40", 3,   TGT_NATIVE,       B_GOOD, "FencingH", 3, 1.00, 1.10, 1, 15, 1, 		  0); // Макуауитль
	n = InitStdBlade(n, "newblade29", "ITEMS_6", 3,   TGT_QUEST,       B_EXCELLENT, "FencingH", 3, 1.00, 1.10, 0, 0, 0, 		  15000); // Макуахутл
	n = InitStdBlade(n, "topor_02", "ITEMS_52", 16, TGT_MARGINAL,   B_ORDINARY, "FencingH", 5, 0.80, 1.15, 1, 10, 1, 		  0); // Бартакс
	n = InitStdBlade(n, "topor_03", "ITEMS_52", 15, TGT_MARGINAL,   B_ORDINARY, "FencingH", 6, 0.85, 1.15, 1, 10, 1, 		  0); // Годендаг
	n = InitStdBlade(n, "topor_04", "ITEMS_40",  6,    TGT_QUEST,       B_GOOD, "FencingH", 8, 0.90, 1.15, 1, 15, 1, 		  0); // Секира
	n = InitStdBlade(n, "topor_05", "ITEMS_52", 14,   TGT_YANKEE,       B_POOR, "FencingH", 2, 0.70, 1.10, 1, 10, 1, 		  0); // Томагавк
	n = InitStdBlade(n, "topor_06", "ITEMS_40", 15,    TGT_QUEST,      B_EXCELLENT, "FencingH", 18, 0.80, 1.15, 0, 0, 0, 		6000); // Стервятник
	n = InitStdBlade(n, "topor_07", "ITEMS_44", 7, TGT_MARGINAL,   B_POOR, "FencingH", 6, 0.75, 1.15, 1, 10, 1, 0); // Костолом
	n = InitStdBlade(n, "topor_08", "ITEMS_44",  8,    TGT_QUEST,       B_GOOD, "FencingH", 15, 0.80, 1.15, 0, 0, 0, 		  45000); // Утренняя звезда
	n = InitStdBlade(n, "topor_09", "ITEMS_44",  9,    TGT_QUEST,       B_GOOD, "FencingH", 15, 0.65, 1.15, 0, 0, 0, 		  53000); // Инквизитор
	n = InitStdBlade(n, "topor_010", "ITEMS_44",  10,    TGT_QUEST,       B_EXCELLENT, "FencingH", 15, 0.90, 1.15, 0, 0, 0, 		  63000); // Секира Древних 
	n = InitStdBlade(n, "topor_011", "ITEMS_44",  11,    TGT_QUEST,  B_EXCELLENT, "FencingH", 15, 0.95, 0.90, 0, 0, 0, 	  59000); //  Кулак Варвара
	n = InitStdBlade(n, "topor_012", "ITEMS_44", 6, TGT_MARGINAL,   B_ORDINARY, "FencingH", 5, 0.70, 1.15, 1, 10, 1, 0); // Морской топор
	n = InitStdBlade(n, "topor_013", "ITEMS_11", 4, TGT_MARGINAL,   B_ORDINARY, "FencingH", 5, 0.70, 1.15, 1, 10, 1, 0); // Клевец
	n = InitStdBlade(n, "topor_014", "ITEMS_44", 16, TGT_MARGINAL,   B_ORDINARY, "FencingH", 6, 0.75, 1.15, 1, 10, 1, 0); // Абордажный топор
	n = InitStdBlade(n, "topor_015", "ITEMS_49",  4, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.75, 1.00, 1,  10, 1, 		   0); // Кирка для добычи руды
	n = InitStdBlade(n, "topor_016", "ITEMS_49",  2, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.85, 1.10, 1,  10, 1, 		   0); // Колун
	n = InitStdBlade(n, "topor_017", "ITEMS_49",  1, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.85, 1.05, 1,  10, 1, 	   0); // Тяпка
	n = InitStdBlade(n, "topor_018", "ITEMS_49",  4, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.85, 1.10, 1,  10, 1, 		   0); // Кирка
	n = InitStdBlade(n, "topor_019", "ITEMS_49",  3, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.85, 1.05, 1,  10, 1, 	   0); // Лопата
	n = InitStdBlade(n, "topor_020", "ITEMS_11",  7, TGT_YANKEE,         B_POOR, "FencingH", 0, 0.90, 1.00, 1,  10, 1, 	   0); // Лом
	n = InitStdBlade(n, "topor_021", "ITEMS_54",  10,    TGT_QUEST,    B_EXCELLENT, "FencingH", 8, 0.70, 1.15, 1, 15, 0, 		  70000); // Рында
	
// палаши и мечи
//int InitStdBlade(индекс, модель, текстура, картинка, целевая группа, качество, тип клинка, номер в группе, длина, изгиб, генерируемый, кол-во генерируемых, генерабельная цена, цена если не генерируемый);
	n = InitStdBlade(n, "blade_06", "ITEMS_40",  4, TGT_MARGINAL,       B_EXCELLENT, "FencingH", 3, 1.00, 1.00, 1, 15, 1, 		  0); // Скьявона
	n = InitStdBlade(n, "blade_07", "ITEMS_40", 1, TGT_MARGINAL,   B_POOR, "FencingH", 4, 1.00, 1.00, 1, 10, 1, 		  0); // Риттершверт
	n = InitStdBlade(n, "blade_13", "ITEMS_40",  5,  TGT_SOLDIER,       B_GOOD, "FencingH", 5, 0.95, 1.05, 1, 15, 1, 		  0); // Офицерский палаш
	n = InitStdBlade(n, "blade_14", "ITEMS_40", 2,  TGT_SOLDIER,   B_ORDINARY, "FencingH", 6, 1.05, 1.00, 1, 10, 1, 		  0); // Валлонская шпага
	n = InitStdBlade(n, "blade_21", "ITEMS_40",  7,  TGT_OFFICER,  B_EXCELLENT, "FencingH", 6, 0.95, 1.00, 1, 15, 1, 		  0); // Клейбэг
	n = InitStdBlade(n, "blade_24", "ITEMS_40",  8,  TGT_PERSIAN,  B_EXCELLENT, "FencingH", 2, 0.90, 1.15, 1, 10, 1, 		  0); // Кханда
	n = InitStdBlade(n, "blade_31", "ITEMS_40",  10,    TGT_QUEST,  B_EXCELLENT, "FencingH", 6, 1.05, 1.05, 1, 15, 1, 		  0); // Нарвал
	n = InitStdBlade(n, "blade40", "ITEMS_43", 12,    TGT_QUEST,  B_GOOD, "FencingH", 14, 0.90, 1.10, 0, 0, 0, 		  10500); // Зазубрина
	n = InitStdBlade(n, "blade43", "ITEMS_43", 6,    TGT_QUEST,  B_POOR, "FencingH", 13, 1.05, 1.05, 0, 0, 0, 		  31000); // Судьбоносная Сталь
	n = InitStdBlade(n, "blade44", "ITEMS_43", 7,    TGT_QUEST,  B_ORDINARY, "FencingH", 14, 0.90, 1.00, 0, 0, 0, 		  44000); // Рубиновый Клинок
	n = InitStdBlade(n, "newblade19", "ITEMS_15", 6, TGT_SOLDIER,   B_ORDINARY, "FencingH", 4, 1.15, 1.00, 1, 10, 1, 	0); // Эсток
	n = InitStdBlade(n, "blade45", "ITEMS_43", 9,  TGT_QUEST,  B_POOR, "FencingH", 13, 0.90, 1.00, 0, 0, 0, 		  25000); // Волчий Зуб
	n = InitStdBlade(n, "blade47", "ITEMS_43", 11,  TGT_QUEST,  B_ORDINARY, "FencingH", 13, 0.90, 1.05, 0, 0, 0, 		  39000); // Рудный Клинок Чудовища
	n = InitStdBlade(n, "blade53", "ITEMS_44",  4,    TGT_QUEST,  B_EXCELLENT, "FencingH", 15, 0.90, 1.20, 0, 0, 0, 		  60000); //  Грах Каррок 
	n = InitStdBlade(n, "blade300", "ITEMS_54",  5,    TGT_QUEST,  B_EXCELLENT, "FencingH", 15, 0.95, 1.15, 0, 0, 0, 	  72000); //  Меч прошлого
	n = InitStdBlade(n, "blade_32", "ITEMS_40",  9,    TGT_QUEST,  B_EXCELLENT, "FencingH", 7, 1.00, 1.10, 0, 0, 0, 		  60000); // Танат
	n = InitStdBlade(n, "blade_33", "ITEMS_11",  8,    TGT_QUEST,  B_EXCELLENT, "FencingH",16, 1.00, 1.10, 0, 0, 0, 		  70000); // Меч Стража
	n = InitStdBlade(n, "blade_36", "ITEMS_40",  16,    TGT_QUEST, B_EXCELLENT, "FencingH", 7, 1.00, 1.00, 0, 0, 0, 		  50000); // Мальтийский Меч
	n = InitStdBlade(n, "blade301", "ITEMS_44",  5,    TGT_QUEST,  B_EXCELLENT, "FencingH", 15, 0.90, 1.10, 0, 0, 0, 	  80000); //  Коготь Белиара
	n = InitStdBlade(n, "newblade5", "ITEMS_11",  6,    TGT_MARGINAL, B_ORDINARY, "FencingH", 0, 0.95, 1.15, 1, 10, 1, 		  0); // Кортелач
	n = InitStdBlade(n, "newblade9", "ITEMS_15",  9,  TGT_SOLDIER,    B_ORDINARY, "FencingH", 5, 0.95, 1.05, 1, 15, 1, 		  0); // Рейтарский палаш
	n = InitStdBlade(n, "newblade12", "ITEMS_11",  3, TGT_OFFICER,       B_GOOD, "FencingH", 3, 0.95, 1.00, 1, 15, 1, 		  0); // Скаллоп
	n = InitStdBlade(n, "newblade14", "ITEMS_54", 12,  TGT_OFFICER,   B_GOOD, "FencingH", 6, 0.95, 1.00, 1, 10, 1, 		  0); // Хаудеген
	n = InitStdBlade(n, "newblade15", "ITEMS_15", 12,  TGT_OFFICER,   B_GOOD, "FencingH", 6, 0.90, 1.00, 1, 10, 1, 		  0); // Бастард
	n = InitStdBlade(n, "newblade21", "ITEMS_11", 5,  TGT_OFFICER,   B_EXCELLENT, "FencingH", 6, 1.05, 1.00, 1, 10, 1, 		  0); // Валлонский меч
	n = InitStdBlade(n, "newblade22", "ITEMS_54", 8, TGT_QUEST,    B_EXCELLENT, "FencingH", 2, 0.85, 1.00, 1, 15, 1, 0); // Цербер
	n = InitStdBlade(n, "newblade7", "ITEMS_15", 7,  TGT_MARGINAL,  B_ORDINARY, "FencingH", 2, 0.75, 1.00, 1, 15, 1, 		  0); // Катцбальтер
	n = InitStdBlade(n, "q_blade_13", "ITEMS_15",  5,  TGT_QUEST,  B_EXCELLENT, "FencingH", 11, 0.90, 1.05, 0, 0, 0, 		  65000); // Фальшион
	n = InitStdBlade(n, "q_blade_21", "ITEMS_15",  10,  TGT_QUEST,  B_EXCELLENT, "FencingH", 15, 1.00, 1.00, 0, 0, 0, 	  70000); // Клэймор	
	...
	n = InitStdGun(n, "pistol_01", 		    "pistol_01", 	"ITEMS_15", 	15,    0.1, 	1,  	1000, 	20.0, 	100.0, 	3.0, 	1, 	     B_POOR); // пистоль
	n = InitStdGun(n, "pistol1", 		    "pistol1", 	"ITEMS_37", 	12,    0.1, 	1,  	2000, 	40.0, 	200.0, 	4.0, 	1, 	     B_ORDINARY); // строевой пистолет
	n = InitStdGun(n, "pistol_02", 		    "pistol_02", 	"ITEMS_15", 	16,    0.1, 	1,  	1500, 	30.0, 	150.0, 	5.0, 	1, 	     B_ORDINARY); // колониальный пистолет
	n = InitStdGun(n, "pistol11",   	  "pistol11",  "ITEMS_44", 12, 0.001,  1,     3000,    60.0,    300.0, 3.0,    1,  B_GOOD); // Дуэльный пистолет
	n = InitStdGun(n, "pistol6",    	    "pistol6",  "ITEMS_37",  3, 0.001,  1,     3500,    45.0,   220.0,  3.5,    2,  B_GOOD); // Бок-Пистолет
	n = InitStdGun(n, "pistol_06",    	    "pistol_06",  "ITEMS_54",  11, 0.001,  1,     4000,    55.0,   270.0,  5.0,    2,  B_GOOD); // Дос Мортос
	n = InitStdGun(n, "pistol9",	      "pistol9", "ITEMS_40", 13, 0.0001,  1,	   4500,	50.0, 	250.0,	3.5,	2,	   B_UNIQUE); // Двухзарядный пистолет
	n = InitStdGun(n, "pistol5",    	    "pistol5",  "ITEMS_37",  8, 0.0001,  1,     5000,    90.0,   450.0,  2.5,    1,  B_EXCELLENT); // Бретерский пистолет
	n = InitStdGun(n, "pistol_05",    	    "pistol_05",  "ITEMS_37",  2, 0.0001,  1,     6000,    100.0,   460.0,  2.0,    1,  B_EXCELLENT); // Пистолет семьи де Монпе
	n = InitStdGun(n, "pistol4",            "pistol4",  "ITEMS_37",  7, 0.0001,  1,     5500,    80.0,   400.0,  6.0,    4,  B_EXCELLENT); // 4-х ствольный пистолет

	// картечные пистолеты
	//n = InitStdGun(индекс, идентификатор, модель, текстура, картинка, редкость, мин.уровень, цена, мин. повреждение, макс.повреждение, вес, кол-во зарядов, качество);
	n = InitStdGun(n, "pistol3",       	    "pistol3",  "ITEMS_37",  11, 0.01,  1,     1000,    15.0,   85.0,  4.0,    1,   B_POOR); // Картечница
	n = InitStdGun(n, "pistol_03",       	    "pistol_03",  "ITEMS_16",  1, 0.01,  1,     2000,    20.0,   110.0,  6.0,    1,   B_ORDINARY); // Тромбон
	n = InitStdGun(n, "pistol_08",       	    "pistol_08",  "ITEMS_43",  2, 0.0001,  1,     4000,    50.0,   260.0,  9.0,    1,   B_GOOD); // Абордажный Мушкетон
	n = InitStdGun(n, "pistol2", 	        "pistol2",  "ITEMS_37", 	9, 0.0001, 	1,     3000, 	30.0, 	160.0, 	4.5, 	3, 	 B_GOOD); // 3-х ствольный дробовик
	n = InitStdGun(n, "pistol02", 	        "pistol02",  "ITEMS_6", 	2, 0.0001, 	1,     3500, 	40.0, 	210.0, 	5.0, 	2, 	 B_GOOD); // Гауда
	n = InitStdGun(n, "pistol_04",       	    "pistol_04",  "ITEMS_16",  3, 0.0001,  1,     6000,    80.0,   380.0,  6.0,    1,   B_EXCELLENT); // Бретерский мушкетон
	n = InitStdGun(n, "pistol8",            "pistol8",  "ITEMS_37",  6, 0.0001,  1,    7000,    85.0,   390.0,  5.5,    1,     B_UNIQUE); // Бландербуз

	// модерн пистолеты
	//n = InitStdGun(индекс, идентификатор, модель, текстура, картинка, редкость, мин.уровень, цена, мин. повреждение, макс.повреждение, вес, кол-во зарядов, качество);
	n = InitStdGun(n, "pistol7",    	    "pistol7",  "ITEMS_37",  5, 0.0001,  1,    70000,    150.0,   560.0,  2.5,    5,     B_UNIQUE); // револьвер Кольта
	n = InitStdGun(n, "pistol10",            "pistol10",  "ITEMS_11",  14, 0.0001,  14,     100000,    200.0,   660.0,  3.0,    5,  B_UNIQUE); //Супершот

	// Мушкеты
	//n = InitStdGun(индекс, идентификатор, модель, текстура, картинка, редкость, мин.уровень, цена, мин. повреждение, макс.повреждение, вес, кол-во зарядов, качество);
	n = InitStdGun(n, "mushket_01",   	    "mushket_01",  "ITEMS_8", 2, 0.0001,  1,     3500,   80.0,   420.0, 9.5,    1,       B_POOR); // Карабин
	n = InitStdGun(n, "mushket1",   	    "mushket",  "ITEMS_51", 2, 0.0001,  1,     7000,   130.0,   520.0, 17.5,    1,       B_ORDINARY); // Мушкет
	n = InitStdGun(n, "mushket2",   	 "SeaCarbine",  "ITEMS_51", 1, 0.0001,  1,     12000,   180.0,   620.0, 14.0,    1,       B_GOOD); // Фузея
	n = InitStdGun(n, "mushket5",   	    "shtuzer",  "ITEMS_37",  13, 0.0001,  1,    20000,   230.0,   720.0, 12.0,    1,     B_UNIQUE); // винтовальный штуцер
	n = InitStdGun(n, "mushket2x2",        "DBmusket",  "ITEMS_37", 14, 0.0001,  1,    22000,   200.0,   670.0, 15.0,    2,     B_UNIQUE); // охотничий штуцер
	n = InitStdGun(n, "mushket_10",        "mushket_10",  "ITEMS_11", 11, 0.0001,  1,    150000,   330.0,   820.0, 6.0,    5,     B_EXCELLENT); // Шотган

	// Мушкетоны
	//n = InitStdGun(индекс, идентификатор, модель, текстура, картинка, редкость, мин.уровень, цена, мин. повреждение, макс.повреждение, вес, кол-во зарядов, качество);
	n = InitStdGun(n, "mushket3",   	  "Arguebuse",  "ITEMS_51", 3, 0.0001,  1,     6000,    120.0,    450.0, 10.5,    1,  B_ORDINARY); // Аркебуза
	n = InitStdGun(n, "Mushket_8",   	  "Mushket_8",  "ITEMS_43", 5, 0.0001,  1,    11000,    170.0,    550.0, 13.0,    1,     B_GOOD); // Дондербасс
	n = InitStdGun(n, "Mushket_9",   	  "Mushket_9",  "ITEMS_37", 4, 0.0001,  1,    18000,    220.0,    650.0, 14.0,    1,     B_EXCELLENT); // Трабукко
	n = InitStdGun(n, "mushket6",   	  "portugize",  "ITEMS_37", 16, 0.0001,  1,    23000,    170.0,    550.0, 12.5,    5,     B_UNIQUE); // башенный мушкетон
	n = InitStdGun(n, "mushkett2x2",        "mushkett2x2",  "ITEMS_6", 4, 0.0001,  1,    25000,   190.0,   580.0, 11.0,    2,     B_UNIQUE); //двуствольный мушкетон
	n = InitStdGun(n, "grape_mushket",       "mortar",  "ITEMS_37", 15, 0.0001,  1,     20000,   280.0,   820.0,  9.0,  1,     B_UNIQUE); // склопетта
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// тяжелые доспехи
	n = InitStdSuit(n, "cirass1",     	"1", "ITEMS_47",  8,  15000, 40,   0.01,  1, false, 0.35, 0.30, 0.9 ); // траншейный доспех
	n = InitStdSuit(n, "cirass2",     	"2", "ITEMS_47",  9,  19000, 30,   0.01,  5, false, 0.25, 0.25, 0.8 ); // рейтарская кираса
	n = InitStdSuit(n, "cirass3",     	"3", "ITEMS_47",  10,  22000, 15,   0.01, 11, false, 0.20, 0.20, 0.7 ); // парадная кираса
	n = InitStdSuit(n, "cirass4",     	"4", "ITEMS_47",  11,  30000, 18,   0.01, 20, false, 0.30, 0.30, 0.9 ); // миланский панцирь
	n = InitStdSuit(n, "underwater",  	"5", "ITEMS_54",  3, 300000, 70, 0.0001, 20,  true, 0.35, 0.35, 1.0 ); // водолазное снаряжение
	// костюмы
	n = InitStdSuit(n,   "suit1", 		"6", "ITEMS_47",  16, 150000,  7, 0.0001, 20,  true, 0.10, 0.00, 0.2 ); // французский офицерский мундир
	n = InitStdSuit(n,   "suit2", 		"6", "ITEMS_54", 1, 150000,  7, 0.0001, 20,  true, 0.10, 0.00, 0.2 ); // английский офицерский мундир
	n = InitStdSuit(n,   "suit3", 		"6", "ITEMS_54", 2, 150000,  7, 0.0001, 20,  true, 0.10, 0.00, 0.2 ); // испанский офицерский мундир
	n = InitStdSuit(n,   "suit4", 		"7", "ITEMS_47",  15,   8000,  6, 0.0001,  1,  false, 0.15, 0.10, 0.6 ); // костюм Лампорта
	// легкие доспехи
	n = InitStdSuit(n, "cirass5",  		"0", "ITEMS_47",  12,   7000,  3,   0.01,  1, false, 0.05, 0.00, 0.0 ); // кожаный корсет
    n = InitStdSuit(n, "cirass6",  		"0", "ITEMS_47",  13,   8000,  6,   0.01, 10, false, 0.10, 0.05, 0.3 ); // жилет бретера
	n = InitStdSuit(n, "cirass7",  		"0", "ITEMS_47",  14,   9000, 12,   0.01, 10, false, 0.20, 0.10, 0.5 ); // бригант
	n = InitStdSuit(n, "cirass8",  		"0", "ITEMS_40", 14,  10000,  8,   0.01, 20, false, 0.15, 0.10, 0.4 ); // плетеный доспех
	
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//										МУЛЬТИКОМПОНЕНТНЫЕ АРТЕФАКТЫ - ТОТЕМЫ
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	n = InitStdAmulet(n, "totem_01",  "Totem_1", "ITEMS_21",  3,  rand(5000) + 10000, 0.5, 0.001, 1,  30, ITEM_TOTEM, 1, 1, 		 "", 1); // Лужёная глотка
	...
	makeref(itm,Items[n]); // черная метка
	itm.id = "BlackLabel";
	...
	makeref(itm,Items[n]);            //Черная Жемчужина
...
//	itm.shown = false;
	...
	makeref(itm,Items[n]); //карта Акила
	itm.id = "mapAkil";
	...
	makeref(itm,Items[n]); 
	itm.id = "Totem_001";//Тотем Шочикецаль
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_002";//Тотем Миктлантекутли
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_003";//Тотем Кецалькоатля
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_004";//Тотем Мишкоатля
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_005";//Тотем Тескатлипока
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_006";//Тотем Чалчиуитликуэ
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_007";//Тотем Уицилопочтли
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_008";//Тотем Тлалока
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_0010";//Тотем Тонакатекутли
	...
	makeref(itm,Items[n]);
	itm.id = "Totem_0011";//Тотем Камаштли
	...
	makeref(itm,Items[n]);//Тотем Синтеотля
	itm.id = "Totem_0012";//открывалка на каменный сундук с кинжалом
	...
	makeref(itm,Items[n]);    //один из двух тотемов на снятие огня
	itm.id = "Totem_0013"; //Тотем Тласолтеотль
	...
	makeref(itm,Items[n]);	//один из двух тотемов на снятие огня
	itm.id = "Totem_0014";//Тотем Тонатиу
	...
	makeref(itm,Items[n]);	//открывалка на второй каменный сундук
	itm.id = "Totem_0015";//Тотем Шипе-Тотеку
	...
	makeref(itm,Items[n]);//Древняя табличка Зодчих
	...
	makeref(itm,Items[n]);//Каменная табличка Куарходрона
	...
	makeref(itm,Items[n]);//Пюпитры Замка учёных (Библиотека)
	...
	makeref(itm,Items[n]);//Серая табличка
	...
	makeref(itm,Items[n]);//Красная табличка
	...
	makeref(itm,Items[n]); //Пират Либерталии - ремонтный комплект
	...
	makeref(itm,Items[n]);	//Вскрытое письмо
	itm.id = "letter_LSCC";
	itm.groupID		= SPECIAL_ITEM_TYPE;
	...
	makeref(itm,Items[n]);	//Письмо Оливера Траста начальству
	...
	makeref(itm,Items[n]);	//Послание Тизера Дэна
	...
	makeref(itm,Items[n]); // болотная трава
	itm.id = "fennis_grass";
	...
	n = InitStdMap(n, 		  "map_bad", "ITEMS_46",  1,  (rand(4) + 6) * 1000, 0.1, 0.005,        "map_2", "cabin_map",  0,   "Special"); // плохая карта
	n = InitStdMap(n,      "map_normal", "ITEMS_45",  14,  (rand(8) + 9) * 1020, 0.1, 0.001,        "map_1", "cabin_map",  0,   "Special"); // обычная карта
    n = InitStdMap(n,        "Map_Best", "ITEMS_27",  3, (rand(8) + 37) * 1000, 0.1, 0.001,     "map_good", "cabin_map",  0,   "Special"); // отличная карта
    n = InitStdMap(n,    "map_bermudas", "ITEMS_41",  4,  (rand(8) + 6) * 1050, 0.2,   0.0,     "cap_bermudas",          "",  1,	 	  ""); // Исла-Тесоро	
	n = InitStdMap(n,         "map_jam", "ITEMS_41",  11,  (rand(8) + 6) * 1100, 0.2,   0.0,      "cap_Jamaica",          "",  1,	  	  ""); // Ямайка	
	n = InitStdMap(n,      "map_cayman", "ITEMS_41",  12,  (rand(8) + 8) * 1015, 0.2,   0.0,       "cap_kayman",          "",  1,		  ""); // Кайман		
	n = InitStdMap(n,    "map_barbados", "ITEMS_41",  2,  (rand(8) + 6) * 1200, 0.2,   0.0,     "cap_barbados",          "",  1,		  ""); // Барбадос			
	n = InitStdMap(n,     "map_tortuga", "ITEMS_42",  8,  (rand(8) + 6) * 1070, 0.2,   0.0,      "cap_tortuga",          "",  1,	 	  ""); // Тортуга				
	n = InitStdMap(n,     "map_curacao", "ITEMS_41",  7,  (rand(8) + 6) * 1300, 0.2,   0.0,      "cap_curacao",          "",  1,		  ""); // Кюрасао				
	n = InitStdMap(n,  "map_martiniqua", "ITEMS_41",  16,  (rand(8) + 6) * 1250, 0.2,   0.0,    "cap_martinica",          "",  1,	  	  ""); // Мартиника				
	n = InitStdMap(n,    "map_dominica", "ITEMS_41",  8,  (rand(8) + 7) * 1030, 0.2,   0.0,     "cap_dominica",          "",  1,	 	  ""); // Доминика						
	n = InitStdMap(n,    "map_trinidad", "ITEMS_42",  9,  (rand(8) + 6) * 1080, 0.2,   0.0,     "cap_trinidad",          "",  1,		  ""); // Тринидад и Тобаго							
	n = InitStdMap(n,      "map_puerto", "ITEMS_42",  2,  (rand(8) + 6) * 1500, 0.2,   0.0,       "cap_p_rico",          "",  1,		  ""); // Пуэрто-Рико								
	n = InitStdMap(n,        "map_cuba", "ITEMS_41",  5, (rand(8) + 12) * 1090, 0.2,   0.0,         "cap_cuba",          "",  1,		  ""); // Куба								
	n = InitStdMap(n,        "map_hisp", "ITEMS_41",  10, (rand(8) + 12) * 1220, 0.2,   0.0,   "cap_hispaniola",          "",  1,		  ""); // Эспаньола									
	n = InitStdMap(n,       "map_nevis", "ITEMS_42",  1,  (rand(8) + 6) * 1010, 0.2,   0.0,        "cap_nevis",          "",  1,	 	  ""); // Сент-Кристофер									
	n = InitStdMap(n,       "map_beliz", "ITEMS_41",  3,  (rand(8) + 9) * 1300, 0.2,   0.0,        "cap_beliz",          "",  1,		  ""); // Белиз									
	n = InitStdMap(n,        "map_guad", "ITEMS_41",  9,  (rand(8) + 6) * 1015, 0.2,   0.0,    "cap_guadelupa",          "",  1,		  ""); // Гваделупа										
	n = InitStdMap(n,       "map_santa", "ITEMS_42",  6,  (rand(8) + 8) * 1280, 0.2,   0.0,        "cap_santa",          "",  1,		  ""); // Блуфилдс										
	n = InitStdMap(n,     "map_antigua", "ITEMS_41",  1,  (rand(8) + 6) * 1400, 0.2,   0.0,      "cap_antigua",          "",  1,		  ""); // Антигуа 											
	n = InitStdMap(n,       "map_terks", "ITEMS_42",  7,  (rand(8) + 9) * 1350, 0.2,   0.0,        "cap_terks",          "",  1,		  ""); // Теркс											
	n = InitStdMap(n,          "map_sm", "ITEMS_42",  5,  (rand(8) + 6) * 1500, 0.2,   0.0,     "cap_s_martin",          "",  1,		  ""); // Синт-Маартен											
	n = InitStdMap(n,     "map_maine_1", "ITEMS_41",  13, (rand(8) + 17) * 1100, 0.2,   0.0,      "cap_maine_1",          "",  1,		  ""); // Новая Гранада, Центр (Картахена + Маракайбо)										
	n = InitStdMap(n,     "map_maine_2", "ITEMS_41",  14, (rand(8) + 18) * 1150, 0.2,   0.0,      "cap_maine_2",          "",  1,		  ""); // Каракас											
	n = InitStdMap(n,      "map_panama", "ITEMS_42",  3,  (rand(8) + 7) * 1200, 0.2,   0.0,       "cap_panama", 		 "",  1,		  ""); // Новая Гранада, Запад (Панама)												
	n = InitStdMap(n,      "map_cumana", "ITEMS_41",  6,  (rand(8) + 7) * 1400, 0.2,   0.0,       "cap_cumana",          "",  1,		  ""); // Кумана									
	...
	n = InitStdMap(n,       "A_map_santa", "ITEMS_27",  8,  (rand(8) + 8) * 2000, 0.2,   0.0,        "santa",          "",  0,		  "Special"); // Блуфилдс										
	...
	makeref(itm,Items[n]);
	itm.id = "spyglass5"; // Корабельный телескоп
	itm.groupID = SPYGLASS_ITEM_TYPE;
	...
	makeref(itm,Items[n]);
	itm.id = "spyglass6"; // Эксклюзивная подзорная труба
	itm.groupID = SPYGLASS_ITEM_TYPE;
	...
	makeref(itm,Items[n]);
	itm.id = "spyglass7"; // Эксклюзивный телескоп
	itm.groupID = SPYGLASS_ITEM_TYPE;
...
//-----------ADDED BY VERRUCKT BROTHEL MOD------------------
	...
	makeref(itm,Items[n]); // косяк из болотной травы
	...
	n = InitStdTalisman(n, "DOjeronRing", "RingJew", "ITEMS_24",  4, rand(5000), 0.5,       "fighter"); // для бравых орлов - телохранителей и разгон крыс
	n = InitStdTalisman(n, "talisman9", "", "ITEMS_5",  11, rand(5000), 0.5,       "treasurer"); // крысобог
	n = InitStdTalisman(n, "DeSouzaCross", "sapphire", "ITEMS_12",  5, rand(5000), 0.3,       "fighter"); //крест Антониу де Соуза
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	n = InitStdItem( n,  "jewelry14",       "pursel", "ITEMS_28",  12, 0.0001,   1,   180,   2,  0.1,   ITEM_TRADE_JEWELRY); // огненный опал
	n = InitStdItem( n,  "jewelry15",       "pursel", "ITEMS_28",  13, 0.0001,   1,   170,   2,  0.1,   ITEM_TRADE_JEWELRY); // аквамарин
	...
	n = InitStdItem( n,  "jewelry21",       "pursel", "ITEMS_29",   5,  0.001,   1,   300,   3,  0.1,   ITEM_TRADE_JEWELRY); // лазурит
	...
	n = InitStdItem( n,  "jewelry100",       "pursel", "ITEMS_53",  4, 0.05,   1,   40,   2,  7.1,   ITEM_TRADE_JEWELRY); // слиток железа
	n = InitStdItem( n,   "jewelry101",     "sapphire", "ITEMS_53",   7,   0.05,   1,   130,   2,  6.4,   ITEM_TRADE_JEWELRY); // слиток стали	
	n = InitStdItem( n,  "jewelry102",       "pursel", "ITEMS_53",   8,  0.05,   1,   2,   2,  3.6,   ITEM_TRADE_JEWELRY); // уголь
	n = InitStdItem( n,  "jewelry103",       "pursel", "ITEMS_53",  1, 0.05,   1,   20,   2,  5.5,   ITEM_TRADE_JEWELRY); // железная руда
	n = InitStdItem( n,   "jewelry104",     "sapphire", "ITEMS_53",   2,   0.05,   1,   450,   2,  4.4,   ITEM_TRADE_JEWELRY); // серебряный руда	
	n = InitStdItem( n,   "jewelry105",     "sapphire", "ITEMS_53",   3,   0.05,   1,   1050,   3,  2.5,   ITEM_TRADE_JEWELRY); // золотой руда
	n = InitStdItem( n,   "jewelry106",     "sapphire", "ITEMS_53",   6,   0.05,   1,   5250,   3,  1.0,   ITEM_TRADE_JEWELRY); // золотой слиток
	n = InitStdItem( n,   "jewelry107",     "sapphire", "ITEMS_53",   5,   0.05,   1,   750,   3,  1.0,   ITEM_TRADE_JEWELRY); // серебряный слиток
	n = InitStdItem( n,   "jewelry108",     "sapphire", "ITEMS_53",   2,   0.05,   1,   750,   3,  1.0,   ITEM_TRADE_JEWELRY); // платиновая руда
	n = InitStdItem( n,   "jewelry109",     "sapphire", "ITEMS_53",   5,   0.05,   1,   750,   3,  1.0,   ITEM_TRADE_JEWELRY); // платиновая слиток
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	n = InitStdItem( n,   "mineral9", 	  "pursel", "ITEMS_30", 10,   0.10,   2,    80, 0,   1.0,   ITEM_TRADE_NORMAL);	// кружка
                ...
                n = InitStdItem( n,  "mineral010", 	  "pursel", "ITEMS_38", 16,   0.10,   0,     4, 0,   1.0,   ITEM_TRADE_NORMAL);	// мешочек соли
...
// Косяк из болотной травки
	InitMultiObject(  	  	"fennisgrasstotal",	"t1",  	     "Mineral7",    "AlchemyTool", 	  1); // Фарфоровая трубка
	InitMultiObject(  	  	"fennisgrasstotal",	"t2",     "fennis_grass",    "Component", 	  1); // Болотная травка
	InitMultiObject(  	  	"fennisgrasstotal",	"t3",  	     "Mineral10",    "AlchemyTool", 	  1); // Огниво
	...
	InitMultiObject(  		 "totem_05",	"t1",     "mineral24",    "Component",    1); // ремень
	...
	InitMultiObject(  		 "totem_11",	"t2",     "mineral31",    "Component", 	  2); // заговоренная кость	
...
void InitMushkets()
{
	...
	InitMushketExt(		"mushket_01",  10.0,  25.0,  10.0,  20.0, "FencingH", "FencingL" ); // Карабин
	InitMushketExt(		"mushket1",  40.0,  70.0,  15.0,  40.0, "FencingH", "FencingL" ); // Мушкет
	InitMushketExt(		"mushket2",  15.0,  50.0,  40.0,  90.0, "FencingH", "FencingL" ); // Фузея
	InitMushketExt(		"mushket5",  15.0,  50.0,  15.0,  35.0, "FencingH", "FencingL" ); // Винтовальный штуцер
	InitMushketExt(	  "mushket2x2",  15.0,  55.0,  15.0,  40.0, "FencingH", "FencingL" );	// Охотничий Штуцер
	InitMushketExt(		"mushket3",  20.0,  45.0,  20.0,  35.0, "FencingH", "FencingL" ); // Аркебуза
	InitMushketExt(		"mushket_8",  25.0,  50.0,  25.0,  40.0, "FencingH", "FencingL" ); // Дондербасс
	InitMushketExt(		"mushket_9",  30.0,  55.0,  30.0,  45.0, "FencingH", "FencingL" ); // Трабукко
	InitMushketExt(		"mushket6",  15.0,  40.0,  15.0,  30.0, "FencingH", "FencingL" ); // Башенный Мушкетон
	InitMushketExt(	  "mushkett2x2",  15.0,  55.0,  15.0,  40.0, "FencingH", "FencingL" ); // Двухствольный мушкетон
	InitMushketExt("grape_mushket",  15.0,  40.0,   10.0,  25.0, "FencingH", "FencingL" ); // Склопетта
	InitMushketExt("mushket_10",  10.0,  35.0,   5.0,  20.0, "FencingH", "FencingL" ); // Шотган		
}
