...
void Box_EnterToLocator(aref loc, string locName)
{
			...
			if(CheckAttribute(loc, locName+".key")) 
			{
				...
				else 
				{
					...
					PlaySound("interface\box_locked.wav");
	                                                                             PlaySound("VOICE\Russian\GOTHIC\GG_KEY\NEEDKEY_1.wav");
					return;
			}
			}
			else
	...
	if(CheckAttribute(loc, locName + ".QuestClosed"))
	{
		Log_SetStringToLog(XI_ConvertString("Box is closed"));
		PlaySound("interface\door_locked.wav");
	                   PlaySound("VOICE\Russian\GOTHIC\GG_KEY\NEEDKEY_" + rand(6) + ".wav");
		return;
	}
...
void OpenBoxProcedure()
{
	...
	if (sti(chr.GenQuest.God_hit_us) == 1 && rand(100) >= (85 + GetCharacterSkillToOld(chr, SKILL_FORTUNE)) && chr.location != "SentJons_HouseF3_Room2") // 280313
	{
		...
		PlaySound("people\clothes1.wav");
	                   PlaySound("VOICE\Russian\GOTHIC\GG_AARGH\AARGH_" + rand(3) + ".wav");
	...
	else
	{
		...
		if(chr.boxname != "mining1" && chr.boxname != "mining2" && chr.boxname != "mining3" && chr.boxname != "mining4" && chr.boxname != "mining5") // Проверка на рудную/угольную жилу
		{
		PlaySound("interface\box_open.wav"); // Звук открытия сундука 
		LaunchItemsBox(&ar);
		LAi_tmpl_ani_PlayAnimation(chr, "open_chest", 1.0);
		}
		else
		{		
		...
		if(pchar.model.animation != "mushketer") // Нельзя добывать руду с мушкетом
		{
		
		if (!LAi_IsFightMode(pchar) && !LAi_grp_alarmactive ) // Запрещаем добывать руду в режиме боя
		{

                                       if((CheckCharacterItem2(pchar,"topor_015")) || (CheckCharacterItem(pchar,"topor_015"))) //проверяем наличие кирки
		{
			if(Locations[locidx].(atrName).closed != 1) // Проверка на блокировку
			{
			
			if(chr.boxname == "mining1") // Железная руда
			{
				//if(makeint(GetCharacterSkill(pchar,SKILL_MINEPICKING)) >= 20) // Проверяем скилл
				{
					if(chr.boxname == "mining1") // Железная руда
					{
						if(pchar.questTemp.mining_type != "mining1")
						{
							if(sti(pchar.questTemp.Mining.Fatigue) < 200) // Проверим устал ли персонаж
							{
								locCameraSleep(true);
								pchar.questTemp.mining_type = "mining1";
						
								LAi_SetActorTypeNoGroup(pchar);
								pchar.questTemp.mining_ft = "true";
								LAi_ActorAnimation(pchar, "ore_go", "OreTimer", 0.1);
							
								Log_Info("Вы вошли в режим добычи руды");
							}
							else
							{
				...
				{
					if(chr.boxname.noskill != 1)
					{
						Log_Info("Необходим навык 'Горного дела' не менее 20");
						chr.boxname.noskill = 1
					}
				}
			}
		
			if(chr.boxname == "mining2") // Серебряная руда
			{
				//if(makeint(GetCharacterSkill(pchar,SKILL_MINEPICKING)) >= 40)
				{
					if(chr.boxname == "mining2") // Серебряная руда
					{
						if(pchar.questTemp.mining_type != "mining2")
						{
							if(sti(pchar.questTemp.Mining.Fatigue) < 200) // Проверим устал ли персонаж
							{
								locCameraSleep(true);
								pchar.questTemp.mining_type = "mining2";
							...
							{
								Log_Info("Персонаж чувствует усталость. Необходим отдых");
	                                                                                                                                       PlaySound("VOICE\Russian\GOTHIC\GG_AARGH\NOTHINGTOGET_1.wav");
							}
						}
						else
						{
				...
				else
				{
					if(chr.boxname.noskill != 1)
					{
						Log_Info("Необходим навык 'Горного дела' не менее 40");
						chr.boxname.noskill = 1
					}
				}
			}
				
			if(chr.boxname == "mining3") // Золотая руда
			{
				//if(makeint(GetCharacterSkill(pchar,SKILL_MINEPICKING)) >= 60)
				{
					if(chr.boxname == "mining3") // Золотая руда
					{
						if(pchar.questTemp.mining_type != "mining3")
						{
							if(sti(pchar.questTemp.Mining.Fatigue) < 200) // Проверим устал ли персонаж
							{
								locCameraSleep(true);
								pchar.questTemp.mining_type = "mining3";
				...
				else
				{
					if(chr.boxname.noskill != 1)
					{
						Log_Info("Необходим навык 'Горного дела' не менее 60");
						chr.boxname.noskill = 1
					}
				}
			}	
				
				if(chr.boxname == "mining4") // Уголь
				{
					if(pchar.questTemp.mining_type != "mining4")
					{
						if(sti(pchar.questTemp.Mining.Fatigue) < 200) // Проверим устал ли персонаж
						{
							locCameraSleep(true);
							pchar.questTemp.mining_type = "mining4";
			...
			if(chr.boxname == "mining5") // Платиновая руда
			{
				//if(makeint(GetCharacterSkill(pchar,SKILL_MINEPICKING)) >= 60)
				{
					if(chr.boxname == "mining5") // Платиновая руда
					{
						if(pchar.questTemp.mining_type != "mining5")
						{
							if(sti(pchar.questTemp.Mining.Fatigue) < 200) // Проверим устал ли персонаж
							{
								locCameraSleep(true);
								pchar.questTemp.mining_type = "mining5";
				...
				if(chr.boxname == "mining4") // Уголь
	                                                      	{
					Log_Info("Угольная жила истощилась");
	                                                                             PlaySound("VOICE\Russian\GOTHIC\GG_AARGH\NOTHINGTOGET_1.wav");
				}
				else
				{
					Log_Info("Рудная жила истощилась");
	                                                                             PlaySound("VOICE\Russian\GOTHIC\GG_AARGH\NOTHINGTOGET_1.wav");
				}
			         }
			}
		...
		{
		                  DoQuestCheckDelay("NOTNEAR", 0.1);
		}
		}
		else
		{
			Log_Info("Добыча руды невозможна в режиме боя");
		}		
		}
		else
		{
			Log_Info("Добыча руды невозможна с мушкетом в руках");
		}		
		}
		else
		{
			Log_Info("Недостаточно места в инвентаре для добычи руды");
		}
		                   // Горное дело <---
