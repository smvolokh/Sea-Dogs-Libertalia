...
void ProcCommand()
{
	...
	switch(nodName)
	{
		...
		case "CBB_SHIP":		if (comName == "click")ProcessFilter(nodName); break;
		case "CBB_ALL":		if (comName == "click")ProcessFilter(nodName); break;
		case "CBB_STORE":	if (comName == "click")ProcessFilter(nodName); break;
		//// {*} BUHO END ADDITION
	}
}
...
void ShowHelpHint()
{
	...
	switch (sNode)
	{
		case "CBB_SHIP":		bCheckboxes = True; break;
		case "CBB_ALL":		bCheckboxes = True; break;
		case "CBB_STORE":	bCheckboxes = True; break;
	}
	if (bCheckboxes)
	{
		sHeader = "Фильтры";
...
void ProcessFilter(string sButton)
{
	...
	case "CBB_SHIP":
		...
		- If the button was not selected, deselect all others, set the state variable and
		  reload the table.
		*/
		SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_SHIP", 2, 1, 1);
		if (FIS_FilterState != FIS_SHIP)
		{
			// Button being selected.
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_ALL", 2, 1, 0);
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_STORE", 2, 1, 0);
			FIS_FilterState = FIS_SHIP;
			AddToTable();
		}
		break;
	case "CBB_ALL":
	    SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_ALL", 2, 1, 1);
		if (FIS_FilterState != FIS_ALL)
		{
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_SHIP", 2, 1, 0);
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_STORE", 2, 1, 0);
		...
		break;
	case "CBB_STORE":
	    SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CBB_STORE", 2, 1, 1);
