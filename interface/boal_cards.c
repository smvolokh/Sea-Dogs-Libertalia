...
void BetaInfo()
{
    ...
    {
        GameInterface.strings.Beta_P       = "Ваша сумма очков: " + CountCardsP();
        GameInterface.strings.Beta_N       = "Сумма очков соперника: " + CountCardsN();
        GameInterface.strings.Beta_Next    = "Номинал следующей карты: " + NextCardPack();
        GameInterface.strings.Beta_MoneyN  = "Денег у соперника: " + MakeMoneyShow(iMoneyN, MONEY_SIGN,MONEY_DELIVER);
        GameInterface.strings.Beta_WinLose = "Число побед: " + (Statistic_AddValue(Pchar, "GameCards_Win", 0)+iHeroWin) + " Число поражений: " + (Statistic_AddValue(Pchar, "GameCards_Lose", 0)+iHeroLose);
    }
}
int NextCardPack()
{
