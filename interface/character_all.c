...
void InitInterface(string iniName)
{
	...
	SetEventHandler("ExitChangeNameWindow","ExitChangeNameWindow",0);
	SetEventHandler("ShowChangeNameWindow","ShowChangeNameWindow",0);
...
void IDoExit(int exitCode)
{
	...
	DelEventHandler("ExitChangeNameWindow","ExitChangeNameWindow");
	DelEventHandler("ShowChangeNameWindow","ShowChangeNameWindow");
...
void ProcessCommandExecute()
{
    ...
    switch(nodName)
	{
		...
		case "B_CSM_CONFIRM": if (comName == "activate" || comName == "click") { AcceptChangeName(); } break;
		case "B_CSM_CANCEL": if (comName == "activate" || comName == "click") { ExitChangeNameWindow(); } break;
	}
...
// CheatSurfMenu -->
void ExitChangeNameWindow()
{
	XI_WindowShow("CHANGENAME_WINDOW", false);
	XI_WindowDisable("CHANGENAME_WINDOW", true);
...
void ShowChangeNameWindow()
{
	if (!CheckAttribute(xi_refCharacter, "name")) xi_refCharacter.name = "";
	if (!CheckAttribute(xi_refCharacter, "nameGen")) xi_refCharacter.nameGen = "";
	if (!CheckAttribute(xi_refCharacter, "nameDat")) xi_refCharacter.nameDat = "";
	if (!CheckAttribute(xi_refCharacter, "lastname")) xi_refCharacter.lastname = "";
	if (!CheckAttribute(xi_refCharacter, "lastnameGen")) xi_refCharacter.lastnameGen = "";
	if (!CheckAttribute(xi_refCharacter, "lastnameDat")) xi_refCharacter.lastnameDat = "";

	GameInterface.char_name.str = xi_refCharacter.name;
	GameInterface.char_name_g.str = xi_refCharacter.nameGen;
	GameInterface.char_name_d.str = xi_refCharacter.nameDat;
	GameInterface.char_lastname.str = xi_refCharacter.lastname;
	GameInterface.char_lastname_g.str = xi_refCharacter.lastnameGen;
	GameInterface.char_lastname_d.str = xi_refCharacter.lastnameDat;

	SetFormatedText("INFO_TEXT_1", "Кто?");
	SetFormatedText("INFO_TEXT_2", "Кого?");
	SetFormatedText("INFO_TEXT_3", "Кому?");

	XI_WindowShow("CHANGENAME_WINDOW", true);
	XI_WindowDisable("CHANGENAME_WINDOW", false);
	XI_WindowDisable("MAIN_WINDOW", true);

	SetCurrentNode("B_CSM_CANCEL");
}

void AcceptChangeName()
{
	ClearAllLogStrings();
	Log_info("Старое имя перса: " + GetFullName(xi_refCharacter));
	...
	xi_refCharacter.name = myname;
	xi_refCharacter.nameGen = mynameG;
	xi_refCharacter.nameDat = mynameD;
	xi_refCharacter.lastname = mylastname;
	xi_refCharacter.lastnameGen = mylastnameG;
	xi_refCharacter.lastnameDat = mylastnameD;
	Log_info("Новое имя перса: " + GetFullName(xi_refCharacter));
	ExitChangeNameWindow();
}
// <-- CheatSurfMenu