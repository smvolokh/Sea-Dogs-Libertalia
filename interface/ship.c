...
// Dolphin -> (загрязнение дна)
		if(!CheckAttribute(xi_refCharacter, "ship.dirt_hull")) xi_refCharacter.ship.dirt_hull = 0;
		
		GameInterface.StatusLine.DIRT_HULL.Max   = 100;
		GameInterface.StatusLine.DIRT_HULL.Min   = 0;
		GameInterface.StatusLine.DIRT_HULL.Value = sti(xi_refCharacter.ship.dirt_hull);
		
		SetFormatedText("DIRT_HULL_STR", "Обрастание днища %");
		// <-
	...
	else
	{
		...
		// Dolphin -> (загрязненность дна)
		GameInterface.StatusLine.DIRT_HULL.Max   = 0;
		GameInterface.StatusLine.DIRT_HULL.Min   = 0;
		GameInterface.StatusLine.DIRT_HULL.Value = 1;
		
		SetFormatedText("DIRT_HULL_STR", "");
		// <-
...
void ShowInfoWindow()
{
	...
	switch (sCurrentNode)
	{
		...
		case "DIRT_HULL_STR":
			sHeader = GetConvertStr("DirtHull", "ShipsDescribe.txt");
			if (shipIndex != -1)
			{
				sText1 = GetConvertStr("DirtHull_hint", "ShipsDescribe.txt");
				sText2 = GetConvertStr("DirtHull_hint2", "ShipsDescribe.txt") + " " +xi_refcharacter.ship.dirt_hull + "%";
			}
			else
			{
				sText1 = GetConvertStr("DirtHull_hint3", "ShipsDescribe.txt");
			}
		break;
