...
void ProcessExitCancel()
{
	...
	if(xi_refCharacter.Id == "Jackman")//Jason, Сага
	{
	 ...
	 if(xi_refCharacter.Id == "TerraxCap")//Месть корсара
	{
		sMessageMode = "ShipGoFreeAsk";
		GoToShipChange(); // Выход там	
		return;
...
void OnShipScrollChange()
{
	...
	{  // не наш, значит убит или сдался
		...
		if(xi_refCharacter.id == "MaryCelesteCapitan" || xi_refCharacter.Id == "PiratesOnUninhabited_BadPirate" || CheckAttribute(pchar,"GenQuest.ShipSituation.Explosion") || xi_refCharacter.Id == "ShipWreck_0" || xi_refCharacter.id == "CureerCap" || xi_refCharacter.id == "Cap_Utreht" || xi_refCharacter.id == "TerraxCap" || CheckAttribute(pchar,"GenQuest.CannotTakeShip"))
		{
			SetSelectable("CAPTAN_BUTTON", false);
			SetSelectable("SWAP_BUTTON", false);
			SetSelectable("CREW_BUTTON", false);
...
void GoToShipChange() // нажатие ОК на табличке ок-отмена
{
	...
	switch (sMessageMode)
	{
		...
		case "ShipGoFreeAsk":
			...
			// Месть корсара
			if (CheckAttribute(pchar, "questTemp.RevengeCorsair.Self") && pchar.questTemp.RevengeCorsair.Self == "GothicRevengeCorsair")
			{
				sld.DontDeskTalk = true;
				sld.ShipEnemyDisable  = true;
				LAi_SetImmortal(sld, true);
