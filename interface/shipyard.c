...
void InitInterface_R(string iniName, ref _shipyarder)
{
	...
	SetEventHandler("ExitDirtMenu", "ExitDirtMenu", 0);
	...
	// Dolphin -> Кренгование
	SetNewGroupPicture("DIRT_Hull_PIC", "SHIP_STATE_ICONS", "Hull"); 
	SetNewGroupPicture("DIRT_Money_PIC", "ICONS_CHAR", "Money");
	// <-
...
void IDoExit(int exitCode)
{
	...
	DelEventHandler("ExitDirtMenu", "ExitDirtMenu");
...
void SetButtionsAccess()
{
    ...
    SetSelectable("BUTTON_DIRT", false);	// Dolphin -> кренгование
    ...
    else
    {
		...
		// Dolphin -> кренгование
		if(!CheckAttribute(xi_refCharacter, "ship.dirt_hull")) xi_refCharacter.ship.dirt_hull = 0;
		if(sti(xi_refCharacter.ship.dirt_hull) > 0) SetSelectable("BUTTON_DIRT", true); 
		// <-
        ...
        if (shipIndex == -1)
        {
            SetSelectable("BUTTON_SELL", false);
            SetSelectable("BUTTON_REPAIR", false);
            SetSelectable("BUTTON_DIRT", false);
        }
        else
        {
            if (!GetRemovable(xi_refCharacter) || !GetShipRemovable(xi_refCharacter))
...
void DoBuyShip()
{
	...
	xi_refCharacter.Ship.dirt_hull = 0; // Dolphin -> (обрастание дна)
}
...
void ExitDirtMenu()
{
	XI_WindowShow("DIRT_WINDOW", false);
	XI_WindowDisable("DIRT_WINDOW", true);
	XI_WindowDisable("MAIN_WINDOW", false);
...
void ShowDirtMenu()
{
	XI_WindowShow("DIRT_WINDOW", true);
	XI_WindowDisable("DIRT_WINDOW", false);
	XI_WindowDisable("MAIN_WINDOW", true);

    SetDirtData();
	SetCurrentNode("DIRT_CANCEL");
}

void SetDirtData()
{
	BaseDirt = sti(xi_refCharacter.ship.dirt_hull);
	DirtHull = 0;
 	DirtStatShow();
}
...
void DirtStatShow()
{
	int dh = 100-sti(xi_refCharacter.ship.dirt_hull);
	
	SetFormatedText("DIRT_QTY_H", (dh+DirtHull) + "%");
	
	DirtMoneyShow();
}

void ClickDirtArror(int add)
{
    int i;
    	
	if(add > 0)
	{
		if(sti(pchar.money) - GetDirtPrice(DirtHull) >= GetDirtPrice(DirtHull+add))
			DirtHull = DirtHull + add;
		else
		{
			i = makeint((sti(pchar.money)-GetDirtPrice(DirtHull))/GetDirtOnePercPrice()));
			DirtHull = DirtHull + i;
		}
				
		if(100-sti(xi_refCharacter.ship.dirt_hull)+DirtHull > 100) DirtHull = 100-(100 - sti(xi_refCharacter.ship.dirt_hull));
	}
	else
	{
		DirtHull = DirtHull + add;
		if (DirtHull < 0)  DirtHull = 0;
	}
...
void DirtMoneyShow()
{
	SetFormatedText("DIRT_WINDOW_TEXT", its(GetDirtPrice(DirtHull)));
}

int GetDirtPrice(int i)
{
	int iCost;
	iCost = GetDirtOnePercPrice() * i;
	return iCost;
}

int GetDirtOnePercPrice()
{
	ref rShip = GetRealShip(sti(xi_refCharacter.ship.type));
	int iDirtCost = makeint(sti(rShip.Weight)/100);

	if(CheckAttribute(refNPCShipyard, "discount")) return makeint(iDirtCost - iDirtCost * stf(refNPCShipyard.discount)); // Dolphin скидка
...
void DirtOk()
{
	if (DirtHull > 0)
	{
		timeHull = timeHull + DirtHull * (8-GetCharacterShipClass(xi_refCharacter));
	    AddMoneyToCharacter(pchar, -GetDirtPrice(DirtHull));
		xi_refCharacter.ship.dirt_hull = sti(xi_refCharacter.ship.dirt_hull) - DirtHull;
		if(sti(xi_refCharacter.ship.dirt_hull)<=0) xi_refCharacter.ship.dirt_hull = 0;
	}
