...
void AddToTable()
{
	...
	for (i = ITEMS_QUANTITY; i < TOTAL_ITEMS; i++)
	{
		...
		if (CheckAttribute(&Items[i], "ItemType") && Items[i].ItemType == "QUESTITEMS") continue;
...
void ShowHelpHint()
{
	...
	{
		sHeader = "Фильтры";
		sText1 = "Предметы персонажа: отображаются только Ваши предметы.";
		sText2 = "Отображать всё: отображаются все предметы сразу.";
		sText3 = "Предметы торговца: отображаются только предметы торговца.";
...
void ProcessFilter(string sButton)
{
	...
	case "CB_BACKPACK":
		...
		*/
		...
		{
			// Button being deselected, reselect it.
