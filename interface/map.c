...
void InitInterface(string iniName)
{
	...
	PlaySound("interface\_EvShip"+rand(21)+".wav");
}
	...
	for(int i = 0; i < numEncounters; i++)
	{
		if(wdmSetCurrentShipData(i))
		{
			...
			else
			{
				...
				if(iRealEncounterType == ENCOUNTER_TYPE_WAR_KAPER)
				{
					totalInfo = totalInfo + XI_ConvertString("Kaper") + GetTextOnShipsQuantity(iNumWarShips);
				}
...
void CalculateInfoData()
{
    ...
    {
		if (!CheckAttribute(pchar, "CSM.SkipShips")) // --> CheatSurfMenu by Cheatsurfer
		{
	...
	{
		if (!CheckAttribute(pchar, "CSM.SkipShips")) SetSelectable("B_CANCEL",false); // --> CheatSurfMenu by Cheatsurfer
	}
	pchar.space_press = 0;
}