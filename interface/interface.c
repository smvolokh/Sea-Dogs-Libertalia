...
void CSMLaunchMenuChars()
{
	if(procInterfacePrepare(INTERFACE_CSMCHARS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMCHARS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuGoods()
{
	if(procInterfacePrepare(INTERFACE_CSMGOODS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMGOODS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuItems()
{
	if(procInterfacePrepare(INTERFACE_CSMITEMS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMITEMS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuMisc()
{
	if(procInterfacePrepare(INTERFACE_CSMMISC))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMMISC;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuShips()
{
	if(procInterfacePrepare(INTERFACE_CSMSHIPS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMSHIPS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuSkills()
{
	if(procInterfacePrepare(INTERFACE_CSMSKILLS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMSKILLS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuTowns()
{
	if(procInterfacePrepare(INTERFACE_CSMTOWNS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMTOWNS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuOfficers()
{
	if(procInterfacePrepare(INTERFACE_CSMOFFICERS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMOFFICERS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuShipsTuning()
{
	if(procInterfacePrepare(INTERFACE_CSMSHIPSTUNING))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMSHIPSTUNING;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void CSMLaunchMenuName()
{
	if(procInterfacePrepare(INTERFACE_CSMNAME))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CSMNAME;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
// <-- CheatSurfMenu by Cheatsurfer
...
void _Procedure_EndVideoPlay()
{
	...
	DoQuestFunctionDelay("csmSoundFix", 0.1); // --> CheatSurfMenu
}
...
void procInfoShow()
{
	...
	if( prevQ>0 )
	{
		if( !IsEntity(&objInfoList[nInfoIdx]) )
		{
			...
			case "game prepare":
						objInfoList[nInfoIdx].picfilename = "loading\Start_Loading_" + rand(1) + ".tga";
					break;
...
// espkk # quick save/load # 28/Jan/2017 -->
	...
	//string sSaveDescriber = GetSaveDataString(GetCurLocationName(true));
	GameInterface.SavePath = "SAVE\" + PlayerProfile.name;
	string curLocName = GetCurLocationName(true);
...
string GetCurLocationName(bool bSea)
{
...
string GetCurAkvatoriaName(bool bSea)
{
	if( nMainCharacterIndex < 0 ) return "";
	string locLabel = "";
	string locLabelMein = "";
	...
	{
		// boal -->
		int nLablesFileID = LanguageOpenFile("LocLables_1.txt");
        locLabel = "";
		if (CheckAttribute(&locations[locidx], "islandId"))
		{
			locLabel = LanguageConvertString(nLablesFileID, Locations[locidx].islandId) + " ";
	...
	else
	{
		if (worldMap.island != "")
		{
			...
			if (locLabel == "Cuba2") locLabel = "Cuba";
            if (locLabel == "Hispaniola2") locLabel = "Hispaniola";
			locLabel = GetConvertStr(locLabel, "LocLables_1.txt");
			if (locLabel == " ")
      		{
      		    locLabel = GetConvertStr("Mein", "LocLables_1.txt") + ", " + GetConvertStr(locLabelMein + "_town", "LocLables_1.txt");
      		}
			if (bSea)
				locLabel += " - " + XI_ConvertString("Sea");
		}
