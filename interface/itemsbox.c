...
void InterfaceInitButtons(ref _refCharacter)
{
	...
	switch(sInterfaceType)
	{
		...
		SendMessage(&GameInterface, "lslllll", MSG_INTERFACE_MSG_TO_NODE, "SKULL_BUTTON", 1, 545, 552, 645, 577); 

								
			// Проверка на возможность использования черепа
			if(CheckCharacterItem(_refCharacter, "SkullAztecs") && LAi_IsDead(characters[sti(arDeadChar.index)]) && CheckNPCQuestDate(_refCharacter, "AztecSkull"))
			{
...
void IDoExit(int exitCode)
{
	...
	{
		PlaySound("interface\box_close.wav"); // Звук закрытия сундука
		bMainCharacterInBox = false; // снимаем флаг 'в боксе'
		LAi_tmpl_ani_PlayAnimation(pchar, "close_chest", 1.0);
	}
...
int GetMaxItemsToTake(ref _char, String _item)
{
	...
	if (_char != refToChar)
		...
		maxItems = MakeInt((maxWeight - GetItemsWeight(_char)) / itemWeight);
...
void OfficerReincarnation(ref rPassanger);
{
	...
	rOff = GetCharacter(NPC_GenerateCharacter("Clon", "none", rPassanger.sex, rPassanger.model.animation, 1, sti(rPassanger.nation), -1, false, "officer"));
	...
	rOff.Dialog.Filename = "Enc_Officer_dialog.c";
	rOff.Dialog.currentnode = "hired";
	rOff.OfficerWantToGo.DontGo = true;//не пытаться уйти
	rOff.loyality = MAX_LOYALITY;
	LAi_SetCurHPMax(rOff);
	rOff.id = "Location fantom character <" + GetFullName(rOff) + ">";
			}else{
	Log_Info("Абордажник " + GetFullName(rOff) + " реанимирована.");
	rOff.Dialog.Filename = "Enc_OfficerGirl.c";
	rOff.Dialog.currentnode = "hired";
	rOff.OfficerWantToGo.DontGo = true;//не пытаться уйти
	rOff.loyality = MAX_LOYALITY;
	LAi_SetCurHPMax(rOff);
	...
	DeleteAttribute(pchar, "items.mushkett2x2");
	AddPassenger(pchar, rOff, false);

        DoQuestFunctionDelay("Gothic_OfficerReincarnation", 2.5);
