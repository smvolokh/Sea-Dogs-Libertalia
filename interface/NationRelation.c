...
void InitInterface(string iniName)
{
    ...
    /////////////
    CreateString(true,"titul", XI_ConvertString("Hunter_type"),"DIALOG228",COLOR_NORMAL,609,100,SCRIPT_ALIGN_LEFT,1.0);
    ...
    CreateString(true,"Flag1","Флаг на корабле","DIALOG228",COLOR_NORMAL,630,432,SCRIPT_ALIGN_LEFT,1.0);
...
void ShowInfoWindow()
{
	...
	switch(sNode)
	{
		...
		case "INFO_CLICK":
			...
			sText2 = "Англия ("+ChangeCharacterNationReputation(pchar, England, 0)+") Франция ("+ChangeCharacterNationReputation(pchar, France, 0)+") Испания ("+ChangeCharacterNationReputation(pchar, Spain, 0)+") Голландия ("+ChangeCharacterNationReputation(pchar, Holland, 0)+") \nКонтрабандисты ("+sti(pchar.questTemp.Contraband.relation)+") Индейцы ("+sti(pchar.questTemp.Indian.relation)+")";  //krizis
		break;
	}
...
void SetNewNation(int add)
{
    ...
    SetNewGroupPicture("FlagPic", "NATIONS_FLAG", GetNationNameByType(curNationIdx));
