...
void SM_PusherSwitch()
{
	...
	{
		pchar.CSM.CrowdPusher = sti(pchar.CSM.CrowdPusher) + 1;
		if (sti(pchar.CSM.CrowdPusher) > 3)
			SM_PusherDisable();
	}
}
...
void SM_PS_OnEnterFrame()
{
	...
	SM_PusherSkipFrame++;
	if (SM_PusherSkipFrame < 10)
		return;
	SM_PusherSkipFrame = 0;
	
	SM_PS_PushRevert(false);
}
...
void SM_PS_PushAction()
{
	...
	if(SendMessage(pchar, "ls", MSG_CHARACTER_EX_MSG, "IsActive") == 0)
		return;
	...
	for (i = 0; i < n; i++)
	{
		...
		if (LAi_group_IsEnemy(pchar, npc) ||
		npc.chr_ai.type == LAI_TYPE_CARRY ||
		npc.chr_ai.type == LAI_TYPE_POOR ||
		npc.chr_ai.type == LAI_TYPE_SIT ||
		npc.chr_ai.type == LAI_TYPE_STUNNED ||
		npc.chr_ai.type == LAI_TYPE_LAY ||
		CheckAttribute(npc, "chr_ai.hpchecker"))
			continue;
		
		if (CheckAttribute(npc, "chr_ai.type.quest") && npc.chr_ai.type.quest != "")
			continue;
			
		if (!LAi_IsFightMode(npc) && SM_GetCharactersDistance(pchar, npc) > 1.2)
			continue;
