...
#include "interface\CheatSurfMenu\CheatSurfMenu.c" // --> CheatSurfMenu by Cheatsurfer
#include "SticksMod\SticksMod.c" // --> [SticksMod]
...
extern void InitAztecNames(); // Пират Либерталии имена индейцев
...
case 4:
		...
		StartPostVideo("ch_logo",1);
	break;

case 5:
		InterfaceStates.videoIdx = 6;
		StartPostVideo("WaltDisneyLogo",1);
	break;

case 6:
		InterfaceStates.videoIdx = 7;
		StartPostVideo("Corsair",1);
	break;
...
void LoadGame()
{
    ...
    string loadScr="";
	loadScr = "loading\Start_Loading_" + rand(1) + ".tga";
...
void InterfaceDoExit()
{
	...
	switch(interfaceResultCommand)
	{
		...
		*/
		...
		case RC_INTERFACE_RUN_CSM_CHARS:
		    CSMLaunchMenuChars();
        break;
		case RC_INTERFACE_RUN_CSM_SHIPS:
		    CSMLaunchMenuShips();
        break;
		case RC_INTERFACE_RUN_CSM_TOWNS:
		    CSMLaunchMenuTowns();
        break;
		case RC_INTERFACE_RUN_CSM_ITEMS:
		    CSMLaunchMenuItems();
        break;
		case RC_INTERFACE_RUN_CSM_SKILLS:
		    CSMLaunchMenuSkills();
        break;
		case RC_INTERFACE_RUN_CSM_GOODS:
		    CSMLaunchMenuGoods();
        break;
		case RC_INTERFACE_RUN_CSM_MISC:
		    CSMLaunchMenuMisc();
        break;
		case RC_INTERFACE_RUN_CSM_TOWNS:
		    CSMLaunchMenuTowns();
        break;
		case RC_INTERFACE_RUN_CSM_OFFICERS:
		    CSMLaunchMenuOfficers();
        break;
		case RC_INTERFACE_RUN_CSM_SHIPSTUNING:
		    CSMLaunchMenuShipsTuning();
        break;
		case RC_INTERFACE_RUN_CSM_NAME:
		    CSMLaunchMenuName();
        break;
		// <-- CheatSurfMenu by Cheatsurfer
	}
...
void OnLoad()
{
	...
	if( CharacterIsDead(pchar) ) {
		if (CheckCharacterPerk(characterFromId("Mary"), "Doctor3")){
		...
		if(ldLoc >= 0)
		{
			...
			if (ngMusicID <= 0)
			{
				if (CheckAttribute(pchar, "CSM.JungleContinuousMusic") && pchar.CSM.JungleContinuousMusic == true)
				{
					...
					{
						if (loadedLocation.type == locations[reload_cur_location_index].type) ngMusicID = 1;
					}
				}
			}
			// Cheatsurfer <--
	...
	trace("bAddonContent " + bAddonContent);
}
...
void NewGame()
{
	...
	CreateEntity(&reload_fader, "fader");
	SendMessage(&reload_fader, "ls", FADER_PICTURE0, "loading\ImgBack.tga");
	//SendMessage(&reload_fader, "ls",FADER_PICTURE, RandPhraseSimple("loading\battle_" + rand(1) + ".tga", "loading\Start_Loading_" + rand(1) + ".tga"));
	SendMessage(&reload_fader, "ls",FADER_PICTURE, "loading\Start_Loading_" + rand(1) + ".tga");
	SendMessage(&reload_fader, "lfl", FADER_IN, RELOAD_TIME_FADE_IN, true);
...
void NewGame_continue()
{
	...
	if (Pchar.questTemp.GothicMonbar != true) //21/07/07 homo для Монбара даем другое начало
	{
		InterfaceStates.startGameWeather = FindWeather("21 Hour");
    ...
    else
    {
		...
		InterfaceStates.startGameWeather = FindWeather("21 Hour");
		LoadMainCharacterInFirstLocationGroup("Xsardas_Dungeon", "quest", "quest3");
    }
...
void ProcessControls()
{
	...
	case "Sea_CameraSwitch": SeaCameras_Switch(); 
	PlaySound("ship\CameraSwitch_"+rand(14)+".mp3");break;
	case "Ship_Fire": Ship_DoFire(); 
 	PlaySound("ship\Fire_"+rand(32)+".mp3"); break;
...
  	{
        if (loadedLocation.type == "underwater" && !CheckAttribute(pchar, "CSM.TimeScaleEnable")) return; //запрет ускорения под водой.
		if (CheckAttribute(pchar, "questTemp.NoFast") && !CheckAttribute(pchar, "CSM.TimeScaleEnable")) return; //запрет ускорения
...
//			}
		...
		/*	if(CheckCharacterItem(PChar, "Map_Best") || bBettaTestMode)*/ if (sti(Pchar.Ship.Type) != SHIP_NOTUSED) LaunchBestMapScreen();
		break;
		...
		case "OfficersCharge":
			...
			PlaySound("interface\OfficersCharge.mp3");
			}	
		break;
		...
		case "OfficersFollow":
			...
			PlaySound("interface\OfficersFollow.mp3");
			}	
		break;
		...
		case "OfficersHold":
			...
			pchar.OfficerAttRange = 7.0;
			OfficersHold();
			Log_SetStringToLog("Офицеры, стоять на месте!");
			PlaySound("interface\OfficersHold.mp3");
			}	
		break;
		...
		case "Dolly":
			...
			if (CheckAttribute(location, "id") && location.id == "Providence_Alcovee")
			{
				bOk = (IsCharacterInLocator(pchar, "teleport", "teleport0")) || (IsCharacterInLocator(pchar, "teleport", "teleport1")) || (IsCharacterInLocator(pchar, "teleport", "teleport2")) || (IsCharacterInLocator(pchar, "teleport", "teleport3")) || (IsCharacterInLocator(pchar, "teleport", "teleport4")) || (IsCharacterInLocator(pchar, "teleport", "teleport5")) || (IsCharacterInLocator(pchar, "teleport", "teleport6"))
				if (bOk)
				{
...
*/			
	...
	break;
		...
		case "BOAL_SetCamera":
			...
			{
		       CSMLaunchMenuChars(); // --> CheatSurfMenu
	...
	break;
		...
		case "BOAL_ControF5":		
			...
			{
				CSMHotKeyF5(); // --> CheatSurfMenu // Statistic_AddValue(PChar, "Cheats.F5", 1);
			}			
		break;
		...
		case "BOAL_ControF7":
			...
			{
				CSMHotKeyF7(); // --> CheatSurfMenu // Statistic_AddValue(PChar, "Cheats.F7", 1);
			}			
		break;
		...
		case "CSM_MenuOfficers":
			CSMLaunchMenuOfficers();
        break;
		
		case "CSM_MenuShipsTuning":
			CSMLaunchMenuShipsTuning();
        break;
