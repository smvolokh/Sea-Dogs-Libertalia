...
void SetCoastalGuardPursuit()
{
	...
	for (i = 1; i <= 3; i++)
    {
		...
		Smuggler = GetCharacter(NPC_GenerateCharacter("Coastal_Captain0" + i, Model, "man", "man", 5, iNation, 365, true, "officer")); // 3 дн€, потом сами пропадут
		SetFantomParam(Smuggler);
		SelectCoastalGuardShip(Smuggler);
		// 1.2.3 хардкор! Smuggler.Coastal_Captain = true; // если по нему палить, Ќ«√ не будет
		Smuggler.GenQuest.CrewMilitiaMode = true;
		Smuggler.Ship.Mode = "militia";
		Smuggler.AlwaysEnemy = true;
		SetCharacterPerk(Smuggler, "MusketsShoot"); 
