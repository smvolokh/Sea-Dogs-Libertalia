...
void SetOfficerParam(ref Npchar, int _type)
{
	...
	Npchar.quest.MinepickingModify      = 1;
    ...
    Npchar.skill.Minepicking       = MiddleK - frandSmall(10*MOD_SKILL_ENEMY_RATE / 1.5) + 7;
    ...
    Npchar.skill.Minepicking      = makeint(stf(Npchar.skill.Minepicking)      + 10*stf(Npchar.quest.MinepickingModify));
...
void LandEnc_OfficerHired()
{
	...
	if (CheckAttribute(pchar, "CSM.OfficersQuit")) sld.OfficerWantToGo.DontGo = true; // --> CSM
}
