...
void DailyRatsEatGoodsUpdate(ref chref)
{
	...
	if(IsCharacterEquippedArtefact(chref, "talisman9")) return; // крысобог
	if(IsCharacterEquippedArtefact(chref, "DOjeronRing")) return; // крысобог
	if(IsCharacterEquippedArtefact(chref, "rat_poison")) return; // мышьяк
	if (IsCharacterPerkOn(chref, "HT2")) return; // абилка торговца
...
void DailyEatCrewUpdate()   // сюда пихаю все что в 1 день
{
	...
	if (CheckAttribute(mainCh, "questTemp.ShipCapellan.Yes")) AddCrewMorale(mainCh, 5); //Jason, влияние корабельного капеллана на мораль
