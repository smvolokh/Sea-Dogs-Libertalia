...
void PGG_DebugLog(string _sLog)
{
	...
	if (bPGGLogShow) // --> CheatSurfMenu by Cheatsurfer
	{
...
void PGG_BecomeHiredOfficer_1()
{
	...
	chr.PGGAi.IsPGG = false;
	chr.Dialog.CurrentNode = "hired";
	chr.Dialog.FileName = "Enc_OfficerGirl.c";
	chr.OfficerWantToGo.DontGo = true; 
	chr.loyality = MAX_LOYALITY;  
	DeleteAttribute(chr, "alignment"); 
	//офицер только раз.
	chr.PGGAi.Officer = 1;
	// учет зп -->
	chr.Money   = 0;
...
void PGG_FireOfficer()
{
	...
	chr.Dialog.CurrentNode = "exit_228";
	chr.Dialog.TempNode = "exit_228";
	chr.Dialog.FileName = "PGG_Dialog.c";
	chr.location = "none";
...
void PGG_Q1FightOnShore()
{
	...
	{
		chr = CharacterFromID(PChar.GenQuest.PGG_Quest.PGGid);
		int iTribe = sti(chr.nation);
		PlaceCharacter(chr, "goto", "random_must_be_near");
		LAi_SetWarriorType(chr);
		LAi_SetImmortal(chr, true);
		LAi_group_MoveCharacter(chr, LAI_GROUP_PLAYER);

		iRnd = 10 + rand(MakeInt(Pchar.rank/2));
		PChar.GenQuest.PGG_Quest.GrpID.Qty = iRnd;
		PChar.GenQuestFort.FarLocator = false;
		for (i = 1; i < iRnd; i++)
		{
		SetFantomGuardsMods("goto", "", PIRATE, "PGGTmp");
		}
	...
	for (i = 1; i < iRnd; i++)
	{
		chr = SetFantomGuardsMods("officers", sLoc, iNation, "ShoreGuard");
		chr.id = "pirate_" + i;
	}
	//натравим.
	LAi_group_SetHearRadius("ShoreGuard", 100.0);
	LAi_group_FightGroupsEx("ShoreGuard", LAI_GROUP_PLAYER, true, Pchar, -1, false, false);
	LAi_group_SetRelation("ShoreGuard", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);	
	LAi_group_FightGroupsEx("ShoreGuard", "PGGTmp", true, Pchar, -1, false, false);
	LAi_group_SetRelation("ShoreGuard", "PGGTmp", LAI_GROUP_ENEMY);
	LAi_group_SetAlarmReaction(LAI_GROUP_PLAYER, "PGGTmp", LAI_GROUP_FRIEND, LAI_GROUP_FRIEND);

	LAi_group_SetCheck("ShoreGuard", "PGG_Q1AfterShoreFight");
	LAi_SetFightMode(pchar, true);
	PChar.GenQuest.PGG_Quest.GrpID = "ShoreGuard";
}
