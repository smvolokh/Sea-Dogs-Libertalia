...
//============= ПГГ ===============
		...
		case "LSCC_rumourElisHusband":
			LSCC_rumourElisHusband();
		break;
		//линейка ГПК, базар о Лейтоне Декстере
		case "LSCC_rumourLostDecster":
			LSCC_rumourLostDecster();
		break;
		case "LSCC_rumourAdmiralLostKey":
			LSCC_rumourAdmiralLostKey();
		break;
	}
...
{                              // key - ключ спец. слуха
    ...
    if (i > 0)// есть ли подходящие слухи
    {
        ...
        if (it == 7) 
		{
			...
			if (key == "LSCC")
				return NO_RUMOUR_LSCC_TEXT[rand(4)];// нету слухов
			else
	...
	else
	{
		switch (key)
		{
			...
			case "PGGG": return PGGG_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)] break;
